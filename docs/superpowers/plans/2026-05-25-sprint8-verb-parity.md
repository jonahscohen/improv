# Sidecoach Sprint 8: Sidecoach Parity + Teach Rebuild - Implementation Plan
...
**New (6):**
- `sidecoach/src/verb-command-registry.ts` - 22-entry registry with full typed metadata.
...
- `sidecoach/src/__tests__/sprint8-verb-parity.test.ts` - parameterized parity test.
...
- `sidecoach/src/sidecoach-orchestrator.ts` - rewrite teach dispatch to V2, add guidance-append callback for sidecoach verbs, dispatch document command.
...
## Sidecoach command list (canonical)
...
Reference file path pattern: `~/.claude/plugins/cache/sidecoach/sidecoach/3.1.1/skills/sidecoach/reference/<verb>.md`.
...
**Files:**
- Create: `sidecoach/src/verb-command-registry.ts`
...
```typescript
import { VERB_REGISTRY, getSidecoachVerbs, getVerbEntry } from '../verb-command-registry';

async function run() {
  const checks: Array<[string, boolean]> = [];

  // T1.1: registry exports 5 prototype entries
  const verbs = getSidecoachVerbs();
  ...
  for (const v of expected) {
    ...
    checks.push([`T1.2: ${v} has skillRefPath`, !!entry && typeof entry.skillRefPath === 'string' && entry.skillRefPath.includes('sidecoach')]);
...
Expected: FAIL with `Cannot find module '../verb-command-registry'`.

- [ ] **Step 3: Read sidecoach's source for the 5 prototype verbs**
...
for v in craft polish audit critique document; do
  echo "=== $v ==="
  cat ~/.claude/plugins/cache/sidecoach/sidecoach/3.1.1/skills/sidecoach/reference/$v.md | head -60
  echo ""
done
```
...
```typescript
// Sidecoach sidecoach-parity command registry.
// Each entry declares an sidecoach verb, the sidecoach flow chain it orchestrates,
// and the parity-plus checklist that proves sidecoach matches or exceeds sidecoach.
...
const SKILL_REF = '~/.claude/plugins/cache/sidecoach/sidecoach/3.1.1/skills/sidecoach/reference';
...
NOTE: the `parityChecklist` strings above are PLACEHOLDERS for the 5 prototype entries; T1 step 3 instructs the implementer to derive real ones from the sidecoach .md files. The final registry strings must come from sidecoach's actual content, not made up.
...
cd /Users/spare3/Documents/Github/claude-dotfiles && git add sidecoach/src/verb-command-registry.ts sidecoach/src/__tests__/sprint8-registry-shape.test.ts .claude/memory/session_2026-05-25_sprint8_execution.md
cd /Users/spare3/Documents/Github/claude-dotfiles && git commit -m "feat(sidecoach): verb-command-registry skeleton + 5 prototype entries (Sprint 8 T1)"
```
...
async function run() {
  ...
  for (const verb of ['craft', 'polish', 'audit', 'critique', 'document']) {
    ...
    checks.push([`T2.1: ${verb} reason mentions sidecoach-parity`, typeof m.reason === 'string' && m.reason.includes('sidecoach-parity')]);
  }
...
cd /Users/spare3/Documents/Github/claude-dotfiles && git commit -m "feat(sidecoach): slash-router branch for sidecoach verb commands (Sprint 8 T2)"
```
...
async function run() {
  ...
  // Scenario 1: full brief -> no gaps -> PRODUCT.md written
  {
    ...
    if (fs.existsSync(productPath)) {
      ...
      checks.push(['S1: NO legacy sidecoach reference', !content.includes('/sidecoach')]);
    }
...
**Files:**
- Modify: `sidecoach/src/verb-command-registry.ts` (add 17 entries)
...
- [ ] **Step 1: Read sidecoach's source for each remaining verb**

For each verb in the 17-verb list, read the sidecoach skill file and extract the canonical checklist items, required output sections, and expected deliverables. Use these to populate `parityChecklist` strings accurately.
...
for v in shape onboard animate bolder colorize delight layout overdrive quieter typeset harden adapt clarify distill optimize extract live; do
  echo "=== $v ==="
  cat ~/.claude/plugins/cache/sidecoach/sidecoach/3.1.1/skills/sidecoach/reference/$v.md
  echo ""
done | less
```

Read these files in full. For each verb, derive a parityChecklist of 3-5 strings that MUST appear in sidecoach's output. The strings should be the OBJECTIVE CHECKLIST items from the canonical reference, not summary words.

- [ ] **Step 2: Map each verb to sidecoach flow handlers**

Reference mapping (the implementer may refine these based on actual sidecoach .md content):
...
Open `sidecoach/src/verb-command-registry.ts` and add the 17 entries inside `VERB_REGISTRY`. Each entry follows the same shape as the 5 prototype entries from T1. Example for `shape`:
...
```typescript
  shape: {
    ...
    parityChecklist: [
      // EXACT strings from ~/.claude/plugins/cache/sidecoach/sidecoach/3.1.1/skills/sidecoach/reference/shape.md
...
Fill in all 17 entries with REAL parityChecklist strings from the sidecoach source.
...
// dedupe + assert each is in getSidecoachVerbs()
```
...
cd /Users/spare3/Documents/Github/claude-dotfiles && rm -f /Users/spare3/.claude/.needs-verification
cd /Users/spare3/Documents/Github/claude-dotfiles && git add sidecoach/src/verb-command-registry.ts sidecoach/src/__tests__/sprint8-registry-shape.test.ts .claude/memory/session_2026-05-25_sprint8_execution.md
cd /Users/spare3/Documents/Github/claude-dotfiles && git commit -m "feat(sidecoach): add 17 remaining sidecoach verb registry entries (Sprint 8 T5)"
```
...
**Files:**
- Create: `sidecoach/src/__tests__/sprint8-verb-parity.test.ts`
...
```bash
cd /Users/spare3/Documents/Github/claude-dotfiles/sidecoach && npx ts-node src/__tests__/sprint8-verb-parity.test.ts
```
...
This task surfaces which flow handlers need guidance refinement to match the verb vocabulary. The implementer identifies failures and decides whether to:
...
```bash
cd /Users/spare3/Documents/Github/claude-dotfiles && rm -f /Users/spare3/.claude/.needs-verification
cd /Users/spare3/Documents/Github/claude-dotfiles && git add sidecoach/src/__tests__/sprint8-verb-parity.test.ts sidecoach/src/verb-command-registry.ts .claude/memory/session_2026-05-25_sprint8_execution.md
cd /Users/spare3/Documents/Github/claude-dotfiles && git commit -m "test(sidecoach): parameterized parity test across all 22 sidecoach verbs (Sprint 8 T6)"
```
...
Look for the place in `sidecoach-orchestrator.ts` where `commandMatch.flowIds` is iterated and each flow is executed. This is the path used by the existing phase commands (research, craft, review). The sidecoach verb commands also flow through here because `getVerbEntry` returns flowIds via the router.
...
```typescript
// Sprint 8: append sidecoach-verb guidance after chain execution
const sidecoachEntry = getVerbEntry(commandMatch.command);
if (sidecoachEntry) {
  // Append guidance to the result
  const guidanceLines = [
    ...(result.guidance || []),
    '',
    `## ${commandMatch.command} (verb command)`,
    ...sidecoachEntry.guidanceAppend,
    '',
    '### Sidecoach additions',
    ...sidecoachEntry.parityPlus.map(p => `- ${p}`),
  ];
...
cd /Users/spare3/Documents/Github/claude-dotfiles && git commit -m "feat(sidecoach): orchestrator appends sidecoach-verb guidance after chain execution (Sprint 8 T7)"
```
...
async function run() {
  ...
  checks.push(['T8.1: list mentions sidecoach verbs heading', /sidecoach/i.test(listOutput)]);
  for (const verb of ['craft', 'polish', 'audit', 'critique', 'document']) {
...
- [ ] **Step 3: Update getAvailableCommands to include sidecoach verbs**

In `sidecoach/src/slash-command-router.ts`, update the existing `getAvailableCommands` function so it returns BOTH phase commands AND sidecoach verbs. Also export a new function that splits them by category for the list command:
...
Find the existing `if (commandMatch.command === 'list')` block in the orchestrator. Replace its return-construction so the guidance contains BOTH phase headings and sidecoach verb headings:

```typescript
if (commandMatch.command === 'list') {
  const phaseCommands = getAvailableCommands();
  const sidecoachCommands = getVerbCommandInfo();
  const guidance: string[] = [
    ...
    '',
    '## Verb command verbs',
    ...Object.entries(sidecoachCommands).map(([cmd, info]) => `- /sidecoach ${cmd} - ${info.description}`),
  ];
  return {
    success: true,
...
```typescript
// Before the sidecoach registry branch, after the composite branch
if (command === 'help') {
  return {
    isCommand: true,
    command: 'help',
...
if (commandMatch.command === 'help') {
  ...
  return {
    ...
    guidance: [
      ...
      '',
      `**Phase:** ${entry.phase}`,
      `**Verb command reference:** ${entry.skillRefPath}`,
      '',
      '**Flow chain:**',
      ...entry.flowIds.map(f => `- ${f}`),
      '',
      '**Parity checklist (sidecoach equivalent produces these):**',
...
Add the import for `getVerbEntry` at the top of the orchestrator if not already present.
...
- Sidecoach now has 22 sidecoach-parity verbs alongside its phase commands (full list)
...
cd /Users/spare3/Documents/Github/claude-dotfiles && git commit -m "docs(sidecoach): document Sprint 8 verb command + teach rebuild + document command (Sprint 8 T9)"
```
...
description: Sprint 8 (Verb command + Teach rebuild) closed. 22 verb-based slash commands + brief-driven teach + new document command. Sidecoach now matches and exceeds sidecoach on every command. 70 tests green.
type: project
...
- T1 <sha> - verb-command-registry.ts skeleton + 5 prototype entries (craft, polish, audit, critique, document). Typed table with 8 fields per entry including skillRefPath and parityChecklist.
...
- T3 <sha> - teach-command-handler-v2.ts with hybrid brief parsing + gap-question fallback. 7-scenario test. Old stub + task9-teach-command test deleted. NO self-attribution. NO legacy sidecoach references.
...
- T5 <sha> - 17 remaining registry entries with real parityChecklist strings derived from sidecoach skill files.
...
- T8 <sha> - /sidecoach list expanded (Phase commands + Verb command verbs sections). New /sidecoach help <verb> shows per-verb detail. 3-assertion test.
...
- /sidecoach <verb> for any of 22 sidecoach verbs routes via the registry to a sidecoach flow chain. After the chain executes, the orchestrator appends the registry's guidanceAppend and parityPlus strings to the result so the parityChecklist gets satisfied.
- /sidecoach list shows both phase commands (sidecoach native) and verb command verbs in separate sections.
- /sidecoach help <verb> shows per-verb detail: description, phase, skill reference path, flow chain, parityChecklist, parityPlus.

## Sidecoach is now strictly more capable than sidecoach

- Every sidecoach command verb has a sidecoach equivalent
- Every command output includes sidecoach's checklist items (parityChecklist) plus sidecoach-specific additions (parityPlus) like BuildReport, validation results, memory entries
- Teach is brief-driven AND interactive (sidecoach's teach is only interactive)
- Document writes Google-spec DESIGN.md (sidecoach's document is similar)
- Sidecoach phase commands (research, craft, review, etc) provide an alternate vocabulary that sidecoach does not have

## Out of scope / future

- Drift-audit command that diffs registry entries against current sidecoach source (future sprint)
...
```markdown
- [Sprint 8 closed (2026-05-25)](session_2026-05-25_sprint8_closed.md): Sidecoach now matches and exceeds sidecoach - 22 verb commands, brief-driven teach, new document command; 70 tests green.
```
...
cd /Users/spare3/Documents/Github/claude-dotfiles && git commit -m "docs(sidecoach): close Sprint 8 - verb command + teach rebuild"
...
- **`parityChecklist` strings might drift** if sidecoach updates its skill files. Each entry's `skillRefPath` documents where to re-derive. No automated drift detection in this sprint.
- **Phase commands and sidecoach verbs sometimes overlap** (e.g. `/sidecoach craft` is BOTH a phase command and an sidecoach verb). The router branch order (composite -> help -> sidecoach -> SLASH_COMMANDS) ensures sidecoach wins. If a phase command shares a name with an sidecoach verb, the sidecoach verb's flowIds + guidance applies. Documented in T8's test.
