...
| [Motion review](motion-review.md) | Judging animation code: frequency gate, easing and duration budgets, escalation triggers, fix hierarchy, block/approve verdicts |

Native scroll-driven animations (`animation-timeline`) and the View Transitions API are documented in the motion-reference skill - prefer them over a full animation library for simple reveals and state morphs.
