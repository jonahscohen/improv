...
- **Impeccable parity verbs** - 22 verb commands that mirror impeccable's vocabulary 1:1 and route to the same underlying flows. The orchestrator appends per-verb parity guidance (sections from the corresponding impeccable reference plus sidecoach extensions) so output matches impeccable's voice while keeping sidecoach's validators, BuildReport, taste validation, and memory.
...
| `/sidecoach list` | Shows both phase commands and the 22 impeccable parity verbs grouped by phase |
| `/sidecoach help <verb>` | Shows registry detail for a verb: description, phase, impeccable reference path, flow chain, parity checklist, sidecoach parity-plus additions |
...
Every verb routes to a sidecoach flow chain and the orchestrator appends the verb's parity guidance (impeccable's section names verbatim) plus sidecoach's parity-plus extensions. Same flows underneath - different vocabulary on top.
