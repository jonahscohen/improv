...
3. **Side-by-side verification required.** If building from a design reference (Figma, screenshot, spec), you MUST compare your implementation against the source. Check dimensions, colors, spacing, typography, border radius, and states. **The interaction model is part of the design source, not yours to choose.** When a UI plugin, form builder, component library, or framework ships a default presentation that diverges from the artboard's interaction model (a stepper where the artboard shows an accordion, tabs where it shows stacked always-visible sections, a modal where it shows an inline panel), the artboard wins. You do not substitute the tool's default because it is the path of least resistance, and you do not grant yourself artistic license the design source did not give you. So the side-by-side is never per-field styling alone: put the artboard's overall structure and interaction next to the rendered build (how sections are grouped, revealed, advanced, collapsed, and navigated) and confirm the STRUCTURE matches, not just the field chrome, before calling the build done. If something doesn't match, fix it before reporting. **Values come from the design source, measured, not eyeballed.** Where the design source exposes exact values (Figma via get_design_context, design tokens, a spec), pull each element's exact color, size, weight, spacing, and dimension and set the implementation to that literal value; an eyeballed "looks about right" is not a measured match, and a screenshot is a reference, not a ruler. **Reuse the project's existing canonical component.** When the design system already defines a component (a button, a card, an input), use its treatment - do not re-style an off-system variant of a component the system already owns. (2026-08-27: a Gravity Forms multi-page stepper shipped over a six-bar accordion artboard because only per-field styling was ever side-by-sided; the section structure never was. 2026-08-28: the same UGC form's hero, inputs, labels, and like/heart buttons shipped off-Figma and off-system because values were guessed from screenshots and the buttons were built fresh instead of reusing the theme's canonical button; the Figma-fidelity gate's dom_equivalence escape hatch was also abused to claim a match without a measured reading, now closed to gate-verified equivalences only.)
...
6. **No false positives.** Running `npx tsc --noEmit` with no errors does not mean the feature works. A 200 HTTP status does not mean the page looks right. A passing CSS selector check does not mean the design matches Figma. Verify with your eyes - and where a design source exists, verify the match by its measured values (item 3), not by eye alone: the eye confirms it renders and reads right, the measured value confirms it matches the source.
...
11. **Open the deliverable, do not just render it.** A FILE deliverable - a document, spreadsheet, slide deck, PDF, or exported asset you built for the user - is not delivered until it is OPENED in its application on the user's machine (`open <path>`). Rendering it to images/PDFs that you Read to check your own work is VERIFICATION, not PRESENTATION: those renders are for YOU, opening the actual file is for the USER. Concretely: if your closing message tells the user a file is somewhere ("it's on your desktop", "saved to X", "the report is ready at Y"), you must have opened that exact file this session. The artifact-open mandate catches many creations, but it provably CANNOT see a file a build SCRIPT wrote - the output path never appears in the command text - so opening what you make is a standing habit, not something a hook can guarantee. This was learned 2026-08-20 when a 26-page document sat closed on the Desktop while the work was declared done; the `artifact-announce-stop` hook now backstops it by blocking the turn when you announce a file's location while it sits closed and unopened.

12. **Comb the whole surface, not just the flagged element (design from a source).** When you build or fix UI against a design source (an artboard, a Figma frame, a spec), you own the WHOLE surface, not only the element you were pointed at. Do not fix the one flagged defect, verify that single item, and report done. Before "done": enumerate every element and section the artboard set contains, OPEN every collapsed or conditional state (accordion panels, tabs, modals, empty vs filled, hover, mobile), and MEASURE each against its design-source value in one comb, up front. Fixing reactively - one user-caught defect at a time - is the exact failure this rule prevents: it makes the user your QA loop. Operationalize it with the Figma-fidelity gate: arm EVERY node of the artboard set (get_design_context each) at the start, and require the gate armed and GREEN across all of them, not just the nodes you touched, before reporting done - the gate then mechanically forces the full comb. Arming the complete set is behavioral (a hook cannot enumerate an artboard for you); the coverage gate is the mechanical backstop once they are armed. A "done" report on a design surface carries the full element inventory measured, each tied to a design-source value, not just the list of items you changed. (2026-08-28: across a full day of the ppai UGC rebuild the work shipped "done" element-by-element - the form's full-width band "glossed over every time", the collapsed accordion sections never combed, a button fixed while the wrong set was measured - so the user had to open the page and catch the next defect ~15 times in a row. Each fix was correct; the miss was never combing the whole surface first.)
