...
4. If `shutdown_request` does NOT terminate the teammate (it keeps emitting idle notifications after two asks), do NOT close a pane by ELIMINATION - the justify-watch daemon ALSO renders as an unnamed `general-purpose` pane, and pane titles carry the agent TYPE, not its name, so "the other general-purpose pane must be my leftover" is exactly what killed the daemon on 2026-07-12. Identify by NAME instead: `ps -Ao pid,command | grep 'claude.exe --agent-id'` prints every live agent with its name (`--agent-id <name>@session...`). Find YOUR teammate's line, confirm its PID is NOT a `justify` one, `kill <pid>`, and verify (`ps -p <pid>` gone, justify-watch still alive). cmux then closes the now-dead pane on its own; if it lingers it is dead AND positively identified, so the cmux-close-guard accepts a literal confirmed close: `CMUX_CLOSE_CONFIRM=surface:<N> cmux close-surface --surface surface:<N>` - spelled out, on its own line (the guard blocks any close carrying a variable, and even trips on a `grep` that merely mentions the close token inside a compound command, so keep identify/kill/close each on separate lines). Verified live 2026-08-01: a stuck homepage-resp teammate whose shutdown never took was resolved this way; the guard had refused the elimination-close and thereby protected justify-watch, which was the other unnamed general-purpose pane.
