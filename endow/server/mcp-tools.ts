...
export function registerTools(mcp: McpServer, ws: WsServer): void {
  ...
  const PROMPT_FILE = join(homedir(), '.claude', 'endow', 'prompts.json');
  ...
  mcp.tool(
    'endow_activate',
    'Activate the Endow overlay in the browser. If no browser is connected, returns a script tag to inject.',
    {},
    async () => {
      ...
      return text(
        `No browser connected. Run \`endow-init\` in the project directory to add the script to the page, ` +
        `or add \`<script src="/endow-core.js"></script>\` to the page's HTML.\n\n` +
  ...
  mcp.tool(
    ...
    async () => {
      ...
      return text('Selection capture not yet implemented - use endow_get_annotations for annotated elements');
    },
  );

  // Tool: endow_get_pending_changes
  mcp.tool(
    'endow_get_pending_changes',
  ...
  // Tool: endow_get_annotations
  mcp.tool(
    'endow_get_annotations',
  ...
  mcp.tool(
    'endow_watch',
    'Block until a prompt arrives from the browser, then return its full content ready for processing. Broadcasts working status to the browser immediately on receipt. Call this in a loop. When it returns prompt data, process it and call endow_respond, then call endow_watch again.',
    {
      timeout: z
        .number()
        .optional()
    ...
    async ({ timeout = 120 }) => {
      ...
      for (const p of prompts) {
        ws.broadcastToClients('endow_working', { promptId: p.id, timestamp: Date.now() });
      }
  ...
  // Tool: endow_get_prompts
  mcp.tool(
    'endow_get_prompts',
    'Return and clear pending prompts from the browser. Each prompt includes an id you must pass back to endow_respond.',
    {},
    async () => {
      ws.recordMcpActivity();
      ws.recordMcpActivity();
  ...
  mcp.tool(
    ...
    {
      promptId: z.string().describe('The prompt id from endow_get_prompts'),
    ...
    async ({ promptId, summary, filesChanged, changes, status, question }) => {
      ws.recordMcpActivity();
      ws.broadcastToClients('endow_response', {
        promptId,
        summary,
        filesChanged,
        changes,
  ...
  // Tool: endow_get_components
  mcp.tool(
    'endow_get_components',
  ...
  // Tool: endow_end_watch
  mcp.tool(
    'endow_end_watch',
    'End the watch session. Call this when the user says "end endow", "stop watching", or similar. Signals the browser that Claude is no longer watching.',
    {},
    async () => {
