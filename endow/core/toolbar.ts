import type { EndowMode } from './types';
import {
  iconManipulate,
  iconPrompt,
  iconLayout,
...
type ModeCallback = (mode: EndowMode | null) => void;
type ActionCallback = () => void;

const MODES: EndowMode[] = ['prompt', 'manipulate'];
...
export class Toolbar {
  el: HTMLDivElement;
  activeMode: EndowMode | null = null;
  ...
  modeButtons = new Map<EndowMode, HTMLButtonElement>();
  ...
  constructor(shadowRoot: ShadowRoot) {
    ...
    animSheet.replaceSync(`
      @keyframes endow-pill-in {
        from { opacity: 0; transform: translateY(12px) scale(0.92); }
        to { opacity: 1; transform: translateY(0) scale(1); }
      }
      @keyframes endow-icon-in {
        from { opacity: 0; transform: scale(0.8); filter: blur(2px); }
        to { opacity: 1; transform: scale(1); filter: blur(0); }
      }
      @keyframes endow-panel-in {
        from { opacity: 0; transform: translateY(8px) scale(0.97); filter: blur(3px); }
        to { opacity: 1; transform: translateY(0) scale(1); filter: blur(0); }
      }
      @keyframes endow-input-glow {
        0% { box-shadow: 0 0 4px 0px var(--endow-glow-color, #D97757); }
        50% { box-shadow: 0 0 8px 1px var(--endow-glow-color, #D97757); }
        100% { box-shadow: 0 0 4px 0px var(--endow-glow-color, #D97757); }
      }
      @keyframes endow-glow-pulse {
        0% { opacity: 0.6; }
        50% { opacity: 1; }
        100% { opacity: 0.6; }
      }
      @keyframes endow-toast-slide-in {
        from { transform: translateY(-100%) translateX(-50%); opacity: 0; }
        to { transform: translateY(0) translateX(-50%); opacity: 1; }
      }
      @keyframes endow-toast-slide-out {
        from { transform: translateY(0) translateX(-50%); opacity: 1; }
        to { transform: translateY(-100%) translateX(-50%); opacity: 0; }
      }
      @keyframes endow-toast-progress {
        0% { width: 0%; }
        100% { width: 100%; }
      }
      @keyframes endow-toast-check-draw {
        from { stroke-dashoffset: 20; }
        to { stroke-dashoffset: 0; }
      }
      @keyframes endow-send-pulse {
      ...
      @keyframes endow-badge-pop {
        0% { transform: scale(0.5); }
        60% { transform: scale(1.15); }
        100% { transform: scale(1); }
      }
      @keyframes endow-msg-wiggle {
      ...
      @keyframes endow-icon-hover-spin {
        from { transform: rotate(0deg); }
        to { transform: rotate(120deg); }
      }
      @keyframes endow-icon-hover-nudge {
      ...
      @keyframes endow-icon-hover-shake {
      ...
      @keyframes endow-icon-hover-rotate {
      ...
      *:focus-visible {
        outline: 2px solid var(--endow-marker, #D97757);
        outline-offset: 2px;
      }
    ...
    applyStyles(this.el, {
      ...
      animation: 'endow-pill-in 0.35s cubic-bezier(0.23, 1, 0.32, 1) forwards',
      overflow: 'hidden',
    ...
    for (const mode of MODES) {
      ...
      btn.style.animation = `endow-icon-in 0.2s cubic-bezier(0.23, 1, 0.32, 1) ${btnDelay}ms both`;
  ...
  createToolbarButton(iconBuilder: IconBuilder, tooltip: string): HTMLButtonElement {
    ...
    btn.addEventListener('mouseenter', () => {
      ...
      if (tooltip === 'Prompt') {
        icon.style.animation = 'endow-msg-wiggle 0.5s cubic-bezier(0.23,1,0.32,1)';
        ...
        const _anim: Record<string, string> = {
          Settings: 'endow-icon-hover-spin',
          Send: 'endow-icon-hover-nudge',
          Clear: 'endow-icon-hover-shake',
        };
  ...
  toggleSettingsPanel(): void {
    ...
    applyStyles(panel, {
      ...
      fontFamily: "'EndowSans', system-ui, sans-serif",
      pointerEvents: 'all',
      animation: 'endow-panel-in 0.25s cubic-bezier(0.23, 1, 0.32, 1) both',
    });
  ...
  updateModeButtonStyles(): void {
    ...
    this.el.style.setProperty('--endow-marker', _mc);
  ...
  setActiveMode(mode: EndowMode | null): void {
    this.activeMode = mode;
    this.updateModeButtonStyles();
  }

  getActiveMode(): EndowMode | null {
  ...
  setBadge(count: number): void {
    if (count > 0) {
      ...
      if (wasHidden || this.badgeEl.textContent !== String(count)) {
        ...
        this.badgeEl.style.animation = 'endow-badge-pop 0.3s cubic-bezier(0.23, 1, 0.32, 1)';
      }
    } else {
