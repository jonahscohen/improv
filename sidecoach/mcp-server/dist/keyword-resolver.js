...
exports.blankInformational = blankInformational;
exports.segmentClauses = segmentClauses;
exports.evaluateLane = evaluateLane;
exports.detectVerb = detectVerb;
exports.classifyIntent = classifyIntent;
exports.intentEligible = intentEligible;
...
const CONJUNCTION_RE = new RegExp('(?:' + CONJUNCTION_BOUNDARIES.map(cb => cb.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|') + ')(?![\\w])', 'iy');
const TERMINATORS = new Set(['.', '!', '?', ';', '\n']);
function loadRegistry(p) {
    const reg = JSON.parse(fs.readFileSync(p, 'utf-8'));
    if (!reg || !Array.isArray(reg.lanes) || !reg.lanes.length)
        throw new Error('lane registry has no lanes');
    if (!reg.scope || !reg.scope.ui_evidence || !reg.scope.negative_filters)
        throw new Error('scope incomplete');
    for (const k of ['route_floor', 'route_margin', 'classify_floor'])
        if (!(k in (reg.scoring || {})))
            throw new Error(`scoring missing ${k}`);
    return reg;
}
...
function blankInformational(text) {
    for (const f of INFO_FRAMES)
        text = text.replace(f, blank);
    text = text.replace(/["“][^"”\n]{0,400}["”]/g, blank);
    return text;
}
function segmentClauses(text) {
    ...
    for (const a of ABBREVIATIONS)
        masked = masked.split(a).join(a.replace(/\./g, ' '));
    ...
    for (let i = 0; i + 1 < b.length; i++)
        if (b[i + 1] > b[i])
            spans.push([b[i], b[i + 1]]);
    return spans;
}
const wb = (p) => new RegExp(`(?<![\\w-])(?:${p})(?![\\w-])`, 'gi');
function compileAll(pats) { const o = []; for (const p of pats) {
    try {
        o.push(wb(p));
    }
    catch { /* isolate one bad regex */ }
} return o; }
function clauseBounds(pos, spans) { for (const [a, b] of spans)
    if (a <= pos && pos < b)
        return [a, b]; return spans.length ? spans[spans.length - 1] : [0, 0]; }
function hasNegator(prefix) { return NEGATORS.some(n => new RegExp(`(?<![\\w-])${n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?![\\w-])`, 'i').test(prefix)); }
const anyMatch = (res, s) => res.some(r => { r.lastIndex = 0; return r.test(s); });
function evaluateLane(lane, prompt, reg) {
    ...
    for (const entry of lane.lexicon) {
        ...
        while ((m = rx.exec(text)) !== null) {
            ...
            if (hasNegator(text.slice(a, m.index)))
                continue;
            if (anyMatch(negs, clause)) {
                nOos++;
                continue;
            }
            if (anyMatch(ui, clause))
                nIn++;
            else
                nUnknown++;
...
function laneIsInformational(text, pattern) {
    const frames = [
...
function detectVerb(text, verbs) {
    for (const v of verbs) {
        ...
        let rx;
        try {
            rx = new RegExp(`(?<![\\w-])(?:${patt})(?![\\w-])`, 'i');
        }
        catch {
            continue;
        }
        if (rx.test(text) && !laneIsInformational(text, patt))
            return name;
    }
    return null;
}
function classifyIntent(prompt, reg, verbs, opts) {
    const res = { outcome: 'SILENT', winningLane: null, verbMatch: null, diagnosticLane: null, laneScores: [], schemaVersion: exports.SCHEMA_VERSION };
    if (!prompt || !prompt.trim())
        return res;
    if (/^\s*\/sidecoach\b/i.test(prompt))
        return res;
    const text = blankInformational(laneSanitize(prompt));
    const scores = reg.lanes.map((l) => evaluateLane(l, prompt, reg));
    res.laneScores = scores;
    const ranked = [...scores].sort((a, b) => b.score - a.score);
    const top = ranked[0];
    const second = ranked[1] ? ranked[1].score : 0;
    const verb = detectVerb(text, verbs);
    res.verbMatch = verb;
    ...
    if (verb) {
        if (routeGrade) {
            ...
            res.outcome = 'VERB';
            const diag = ranked.find(r => r.score > 0);
            res.diagnosticLane = diag ? diag.lane : null;
        }
        return res;
    }
    ...
    if (unknown) {
        res.outcome = 'CONTEXT_CHECK';
        res.winningLane = unknown.lane;
        return res;
    }
    const oos = scores.some((r) => r.scope === 'OUT_OF_SCOPE');
    if (opts && opts.intentEligible) {
        res.outcome = 'NUDGE_ELIGIBLE';
        return res;
    }
    res.outcome = oos ? 'OUT_OF_SCOPE' : 'SILENT';
    return res;
}
function hookEligibilitySanitize(text) {
    return text
        .replace(/```[\s\S]*?```/g, ' ')
        .replace(/`[^`\n]*`/g, ' ')
...
function hookEligibilityIsInformational(text, pattern) {
    const frames = [
        `\\bwhat\\s+(?:is|are|was|were|does|did)\\s+(?:the\\s+|a\\s+|an\\s+)?${pattern}(?![\\w-])`,
        `\\bwhat['\\u2019]s\\s+(?:the\\s+|a\\s+|an\\s+)?${pattern}(?![\\w-])`,
        `\\bhow\\s+to\\s+(?:use\\s+)?(?:the\\s+)?${pattern}(?![\\w-])`,
...
function intentEligible(prompt, intentReg) {
    if (!intentReg)
        return false;
    const sanitized = hookEligibilitySanitize(prompt);
    const arr = (k) => (Array.isArray(intentReg[k]) ? intentReg[k] : []);
    ...
    const subst = (pats) => mlist(pats).filter((p) => !hookEligibilityIsInformational(sanitized, p));
    ...
    if (hasExempt && !hasNewBuild && !hasStandalone)
        fires = false;
    return fires;
}
