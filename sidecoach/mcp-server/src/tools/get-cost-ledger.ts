...
import { getSessionLedger, summarizeLedger } from '../../../dist/model-routing';
import { SidecoachToolError } from '../errors';
import { getCostLedgerShape, type GetCostLedgerInputT } from '../schemas';
import type { ToolDefinition, ToolHandler } from './types';

export const definition: ToolDefinition<typeof getCostLedgerShape> = {
  name: 'sidecoach_get_cost_ledger',
  description:
    'Return the current session cost ledger from sidecoach\'s model-routing module. ' +
  ...
  inputSchema: getCostLedgerShape,
  timeoutMs: 5_000,
};

export const handler: ToolHandler<GetCostLedgerInputT> = async (input, _deps) => {
  let entries;
  try {
    entries = getSessionLedger();
  } catch (err) {
    throw new SidecoachToolError(
      'VALIDATOR_FAILURE',
      'failed to read session ledger from model-routing module',
      { validator: 'getSessionLedger', errorMessage: err instanceof Error ? err.message : String(err) },
    );
  }
