...
import { getCheatsheetShape, type GetCheatsheetInputT } from '../schemas';
import type { ToolDefinition, ToolHandler } from './types';

export const definition: ToolDefinition<typeof getCheatsheetShape> = {
...
function extractSection(markdown: string, section: NonNullable<GetCheatsheetInputT['section']>): string {
  const startRe = SECTION_HEADERS[section];
  if (!startRe) return markdown;
...
export const handler: ToolHandler<GetCheatsheetInputT> = async (input, deps) => {
  if (!deps.registries.cheatsheet) {
    throw new SidecoachToolError(
      'DOWNSTREAM_UNAVAILABLE',
      'cheatsheet not loaded (CHEATSHEET.md missing or unreadable at startup)',
      { resource: 'claude/skills/sidecoach/CHEATSHEET.md' },
    );
  }
  ...
  if (!content) {
    throw new SidecoachToolError(
      'DOWNSTREAM_UNAVAILABLE',
      `cheatsheet section "${section}" was not found in the document`,
      { resource: 'CHEATSHEET.md#' + section },
    );
  }
