...
export const listVerbsShape = {
  phase: z
    ...
    .optional()
    .describe(
      'Optional phase filter. Sidecoach phases include shape-strategy, build, review, tone, docs.',
    ),
};
...
function isValidRenderUrl(v: string): boolean {
  try { return ['http:', 'https:', 'file:', 'data:'].includes(new URL(v).protocol); } catch { return false; }
}
