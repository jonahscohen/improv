...
export class LspClient {
  ...
  private readonly opts: Required<Omit<LspClientOptions, 'log'>> & {
    log?: LspClientOptions['log'];
  };
  private nextId = 1;
  private dead = false;
  ...
  private log(msg: string, ctx?: Record<string, unknown>): void {
    if (this.opts.log) this.opts.log(msg, ctx);
  }
  ...
  private dispatch(msg: unknown): void {
    ...
    if ('id' in obj && (('result' in obj) || ('error' in obj)) && !('method' in obj)) {
      ...
      if ('error' in obj && obj.error) {
        ...
        pending.reject(
          new SidecoachToolError('VALIDATOR_FAILURE', `LSP ${pending.method} error: ${e.message ?? 'unknown'}`, {
            validator: 'lsp',
  ...
  private die(reason: string): void {
    ...
    for (const [, pending] of this.pending) {
      clearTimeout(pending.timer);
      pending.reject(
        new SidecoachToolError('DOWNSTREAM_UNAVAILABLE', `language server unavailable: ${reason}`, {
          resource: 'lsp',
        }),
      );
    }
  ...
  private sendRaw(obj: unknown): void {
    if (this.dead) return;
    try {
      this.transport.write(encodeMessage(obj));
    } catch (err) {
  ...
  public request(method: string, params?: unknown, timeoutMs?: number): Promise<unknown> {
    if (this.dead) {
      return Promise.reject(
        new SidecoachToolError('DOWNSTREAM_UNAVAILABLE', `language server unavailable: ${this.deadReason ?? 'dead'}`, {
          resource: 'lsp',
        }),
      );
    }
    ...
    return new Promise<unknown>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id);
        reject(
          new SidecoachToolError('TIMEOUT', `LSP ${method} exceeded ${budget}ms`, { timeoutMs: budget }),
        );
      }, budget);
   ...
   */
  public async initialize(rootUri: string): Promise<unknown> {
    const params = {
 ...
 */
export function spawnChildTransport(
  command: string,
  args: string[],
  cwd: string,
  log?: (msg: string, ctx?: Record<string, unknown>) => void,
  ...
  child.stderr?.on('data', (buf: Buffer) => {
    if (log) log('lsp server stderr', { bytes: buf.length });
  });
  ...
  return {
    ...
    onData: (cb) => {
      child.stdout?.on('data', (buf: Buffer) => cb(buf));
    },
    onExit: (cb) => {
