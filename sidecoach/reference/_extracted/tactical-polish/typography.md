# tactical-polish - Typography (extracted)

This file extracts `typography.md` from the tactical-polish skill. Source verbatim lift comes first, then Extension (operational specificity), then What's missing (typography gaps).
...
From `/Users/spare3/.agents/skills/tactical-polish/typography.md`.
...
## What's missing - typography gaps in tactical-polish

The typography section covers wrapping, smoothing, and tabular numbers. Significant typography concerns it does not cover:

1. **Font pairing.** No guidance on display + body combinations, when to use a single typeface family, how many weights to load. (Covered by sidecoach's `fontshare-reference` skill, but worth noting tactical-polish is silent here.)
...
4. **Letter spacing (tracking).** No guidance. Common defaults: -0.02em for large display, -0.01em for headings, 0 for body, +0.05em for ALL CAPS. The default letter-spacing on most fonts is calibrated for body text at 16px; large headings need negative tracking, ALL CAPS needs positive tracking. Silent in tactical-polish.

5. **Optical sizing.** Variable fonts (Inter, Recursive, etc.) ship an `opsz` axis that adjusts the design for the rendered size. tactical-polish doesn't mention `font-optical-sizing: auto`, which costs nothing and significantly improves rendering on variable fonts.

6. **Font-feature settings beyond tabular-nums.** Ligatures (`liga`, `dlig`), stylistic sets (`ss01-ss20`), small caps (`smcp`), fractions (`frac`), slashed zero (`zero`). Several of these have aesthetic impact and zero performance cost. Inter's `ss01` (single-story `a`) is a common stylistic choice; nothing in tactical-polish.
...
9. **CJK and complex script considerations.** `text-wrap: balance` and `pretty` behave differently on Chinese/Japanese/Korean (which lack word boundaries) and on Arabic/Hebrew (which are RTL). Silent in tactical-polish.
...
12. **Color contrast on text.** WCAG 2.1 AA requires 4.5:1 for body, 3:1 for large text (18pt+ or 14pt+ bold). tactical-polish doesn't mention. Light gray text on white is a common violation.
