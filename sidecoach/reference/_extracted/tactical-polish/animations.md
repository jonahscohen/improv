...
The enter blur animates from `blur(4px)` to `blur(0)` - inside the animated-blur budget (stay at or under `8px`; `2-4px` is the sweet spot). See the blur budget in [motion-review.md](motion-review.md) for the single rule set every animated and transitional blur follows.
