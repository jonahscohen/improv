...
The three-tier mental model (system / fundamentals / vertical) and the explicit conflict-resolution priority. Oracle's commands implicitly operate across all three tiers without naming the distinction. Tactical-polish is tier 2 (fundamentals) tactical-only. TypeUI separates the layers so when an audit flags a conflict, you know which tier overrides.
