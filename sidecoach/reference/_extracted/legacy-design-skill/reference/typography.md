# Impeccable typography.md (extracted)
...
**Bound your clamp()**: keep `max-size at most ~2.5x min-size`. Wider ratios break the browser's zoom and reflow behaviour and make large viewports feel like the page is shouting.
...
### Line-height by font-size (concrete table)

| Font size | Suggested line-height | Note |
|---|---|---|
| 12-13px | 1.4 to 1.5 | Captions, dense UI |
| 14-15px | 1.45 to 1.55 | Secondary text |
| 16-18px | 1.5 to 1.65 | Body prose |
| 20-24px | 1.4 to 1.5 | Lead, subheadings |
| 28-40px | 1.2 to 1.35 | Headings (h2-h3) |
| 48-72px | 1.05 to 1.15 | Large headings (h1) |
| 80px+ | 0.95 to 1.05 | Display |
| Light-on-dark | add 0.05 to 0.10 | Compensates for perceived weight |

### Font-pairing reference list (concrete, not banned)

A starter palette of pairings that don't appear on the reflex-reject list (current as of 2026):

- **Editorial / luxury:** Tiempos Display + Founders Grotesk; ABC Diatype + Marfa; Migra + Söhne
- **Tech / developer:** Söhne (sans only); JetBrains Mono + Söhne; Berkeley Mono + Inter Tight
- **Consumer / warm:** GT Walsheim + Reckless Neue; Recoleta + Söhne (gentler than Cormorant)
- **Brutalist / utility:** Helvetica Now + Helvetica Now Mono; Akzidenz-Grotesk; Suisse Int'l
- **Creative studio:** PP Editorial Old + ABC Diatype; Beausite Classic; Suisse Works

Verify availability and licensing before commit; the reflex-reject list updates faster than this catalog.

### Web font loading: decision tree

```
Are you brand-critical for above-the-fold? (e.g., a display serif logotype)
  +-- YES: preload + font-display: optional (avoid swap-in flash)
  +-- NO: font-display: swap (show fallback, swap when ready)

Are you using 3+ weights?
  +-- YES: variable font (single file, smaller than 3 static)
  +-- NO: static fonts in only the weights used

Do you have an extensive char set (multilingual)?
  +-- YES: unicode-range subsetting per script
  +-- NO: single file is fine
```

## WHAT'S MISSING

- **No vertical-rhythm worked example.** "Multiples of 24px" but no end-to-end token system showing how that propagates through padding, gaps, margins.
- **No relationship to fluid grids.** Type scales should align with spacing scales; not addressed.
- **No advice on hand-tuning specific letter pairs.** Kerning is referenced but no "tighten Ty, AW" guidance.
- **No font-as-brand-asset workflow.** Where do custom-drawn or licensed fonts live? How are they versioned? Not addressed.
- **Reflex-reject list is in brand.md only.** Typography.md cross-references but doesn't duplicate, which means non-brand readers might miss it.
