# Impeccable critique.md (extracted)
...
### Independence enforcement: why and how

The two-assessment isolation is load-bearing. Without it, the LLM review pre-anchors to whatever the detector finds, the detector's findings get treated as gospel, and the LLM review collapses to "yeah, the detector is right."

To enforce independence:
- Spawn Assessment A in a separate subagent/Task. Pass it the file path / URL and the impeccable context, nothing else.
- Spawn Assessment B as a separate subagent or just the deterministic CLI; do NOT pass it any output from A.
- Collect both as text/JSON, THEN synthesize in the main thread.

If you can't spawn subagents, do A first with no foreknowledge of what the detector would find, write A's findings into a local note, THEN run B. The temporal separation provides imperfect but real independence.

### Persona auto-selection table (from personas.md)

| Interface type | Auto-select |
|---|---|
...
```markdown
# Ignored findings (do not re-raise)

# Format: plain text per line; case-insensitive substring match against rule name or snippet
# Lines starting with # are comments

side-stripe border on .alert-warning  # designer chose this, accepted tradeoff
nested cards in /pricing               # intentional Stripe-style layout
generic-font Inter                     # licensed locally, this is the brand font
```

## WHAT'S MISSING

- **No "this was already audited, here's the diff" view.** Each critique is independent; no way to see "issues you fixed since last run" highlighted.
- **No way to attach screenshots to critique snapshots.** The persisted file is text-only; visual evidence isn't preserved.
- **No multi-author critique.** What if the user wants to run 3 critiques (LLM review, detector, human teammate's notes) and synthesize? Only 2 inputs supported.
- **No critique scope hint.** A 5000-line page critique vs a single component critique should produce different report shapes; same template for both.
- **No critique-for-PRs workflow.** Run critique against `git diff` only? Not supported.
- **Persona selection table is hardcoded.** No way to declare "this is a dashboard, but onboarded users only" or other context that would change persona priorities.
