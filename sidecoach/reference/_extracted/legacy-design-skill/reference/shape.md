...
### Interview question budget

| Round | Question count | Anchored to |
|---|---|---|
| Round 1 | 2-3 questions | Purpose, audience, content, visual direction |
| Round 2 (optional) | 2-3 questions | Whatever Round 1 left ambiguous |
| Round 3+ | Don't run | If 6 questions haven't anchored the brief, the brief is wrong; restart |

### Compact vs full brief decision rubric

Use compact when ALL of these hold:
- Prompt has scope baked in ("build a pricing page", "build a settings panel")
- PRODUCT.md answers register + voice + anti-references
- DESIGN.md exists and covers the relevant tokens
- Brief fits in 3-5 bullets without omitting decisions
- Visual direction is either a clear default or one named reference

Use full structured (10 sections) when ANY of these hold:
- Multi-screen or multi-flow task
- User asked for shape as a standalone deliverable
- Stakeholder review is implied (the brief will be shared with non-Claude readers)
- The task has unresolved trade-offs that need to be made explicit (e.g., "do we charge here or later", "is this an admin-only surface")
- Anti-references are missing from PRODUCT.md and this brief must establish them

### Scene-sentence quality rubric

Vague (doesn't force dark vs light): "an observability dashboard"
Better: "an observability dashboard for SREs"
Concrete: "an SRE glancing at incident severity on a 27-inch monitor at 2am in a dim NOC"
Forces the answer: "...in a dim NOC, eyes already adjusted to dark surroundings, glancing every 30 seconds between this and Slack" -> dark mode required

The test: read the sentence aloud. If a reasonable person could still defend either dark or light, add more detail.

## WHAT'S MISSING

- **No worked example of a compact brief.** The 10-section structure is documented but no example shows what a good 3-5 bullet compact brief looks like in practice.
- **No "shape ran without PRODUCT.md" recovery.** What if there's no PRODUCT.md and the user refused teach? Brief structure doesn't degrade gracefully.
- **No version control on briefs.** If shape iterates 3 times, where do the earlier briefs go? Lost in conversation context.
- **No template for "open questions" formatting.** "Don't list 'open questions' you've already recommended a default for" but no positive example of what real open questions look like.
- **Visual direction probes are referenced but not bounded.** "2 to 4 probes" but no guidance on cost (image generation is slow and tokens-heavy). When is 4 too many?
- **No brief-handoff format for non-Claude consumers.** A human designer reading the brief later doesn't have the implicit context Claude does.
