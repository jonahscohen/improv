# Impeccable live.md (extracted)

Note: long dashes and arrows in source replaced with regular hyphens / ASCII arrows per repo policy.
...
**While planning each variant, also name its 2-3 parameter knobs** (per the section 7 budget table). Parameters are part of the design, not a decoration added afterward. If the variant explores density, expose a density knob. If it explores color commitment, expose a color-amount range. Deciding "what's tunable" during planning produces better knobs than retrofitting them onto finished HTML.
...
5. **While planning each direction, also name its 2-3 parameter knobs** (per the section 7 budget table). The same principle as default mode: decide "what's tunable" during planning, not after writing the HTML. A departure-mode hero with 0 parameters is not "bold creative vision," it's a missed opportunity for the user to fine-tune the direction they pick.
...
- **Small composition**: labeled input, simple card, short callout (at most ~5 visual children): **0-1** params when one dominant axis is obvious; otherwise **0.**
...
Don't run `live-accept --discard` for this; that's a pure file mutator, the browser doesn't see it, and the bar gets stuck on the GENERATING dots forever (the user has to refresh). `--discard` is only correct when the **browser** initiated the discard (user clicked the close button during CYCLING) and the agent is just running source-side cleanup the browser already triggered.
...
### The 7-step contract, distilled

```
1. live.mjs            -> boot helper
2. navigate to pageFile -> open browser at served URL
3. live-poll.mjs       -> wait for events (long timeout, never short)
4. handle generate     -> read screenshot if present, wrap, plan 3 variants, write atomically, --reply done
5. handle accept       -> check carbonize, do cleanup, live-complete.mjs
6. handle discard      -> ack
7. handle exit         -> cleanup helper, remove inject
```

Recovery: if interrupted, `live-status.mjs` -> `live-resume.mjs` -> resume from durable checkpoint.

### Param-kind cheatsheet

| Kind | Drives | CSS access | Use for |
|---|---|---|---|
| range | --p-<id> CSS var | `var(--p-foo, default)` | Density, color amount, scale, motion intensity |
| steps | data-p-<id> attr | `:scope[data-p-foo="x"] .child` | Pairing, structure, palette role, theme |
| toggle | both | either | On/off (serif display, dark mode) |

### Carbonize cleanup checklist (5 steps)

1. Find carbonize block (markers + `<style>`)
2. Move CSS rules to real stylesheet
3. Bake param values, rewrite scope selectors to semantic classes
4. Unwrap the variant wrapper div, drop data-impeccable-params and data-p-* attrs
5. Delete inline style, param-values comment, both carbonize markers, dead @scope rules

Then `live-complete.mjs --id ID` and verify `phase: "completed"`.

### Identity sentence template (Phase A)

`The dominant surface is [hex/token], accent [hex/token]; type pairs [primary font] with [body font]; layout is [topology]; corners [radius scale], borders [hairline/none], shadows [none/subtle/heavy]; copy tone reads as [observed tone].`

Example: "The dominant surface is `#f7f5ef` cream, accent `#3a2418` espresso; type pairs Tiempos Display 600 with Untitled Sans 400; layout is asymmetric two-column with overlapping image; corners 4px, hairline 1px borders, no shadows; copy tone reads as plain-spoken and confident."

That's the identity lock. Every variant must read as the same brand by that sentence.

## WHAT'S MISSING

- **No multi-user live mode.** What if two designers want to iterate on the same page simultaneously? No concurrency model.
- **No live-mode-history viewer.** Accepts accumulate; no UI for browsing past changes.
- **No way to fork a variant.** User picks variant 2 and wants to tune it further with a second Go; flow not described.
- **No mobile testing in live mode.** The picker is desktop-only; no story for testing variants on phones.
- **No prescription for diff visualization at accept time.** What changed in source? User trusts the carbonize cleanup blindly.
