...
### The 4 craft gates (codex-available harness), enumerated as verifiable checks

| Gate | Pass criteria | Failure consequence |
|---|---|---|
| 1. Shape brief confirmed | User has explicitly said "yes / proceed / confirmed" OR overridden parts of the brief. Implicit silence does NOT count. | Restart shape with new info |
| 2. Direction questions answered | Codex Step A questions answered with palette + atmosphere + named references | Loop Step A |
| 3. Palette confirmed | User has approved a single palette artifact | Iterate Step B |
| 4. Mock direction approved | One direction named, OR explicit delegation ("you pick") | Loop Step C/D |

If any gate is skipped, the implementation will reliably drift from the user's intent.

### Step 4 production-bar checklist (testable items)

For each item, what to run / how to verify:

| Item | Verification |
|---|---|
| No placeholder copy | `grep -ri "lorem ipsum\|placeholder\|TODO\|FIXME" src/` returns empty |
| Semantic landmarks | `<header>`, `<nav>`, `<main>`, `<footer>` present; one `<h1>` per page; headings descend without skipping levels |
| All 8 interaction states | For each interactive element, screenshot at default/hover/focus/active/disabled/loading/error/success |
| No layout-property animations | `grep -ri "animation\|transition" src/styles/ | grep -E "(width|height|top|left|margin)" returns empty (excluding container-query rules) |
| Reduced-motion respected | `prefers-reduced-motion` media query present with motion-shortened or motion-disabled fallbacks |
| Image URLs verified | curl each image src; expect 200 not 404 |
| No console errors | Open in browser, check DevTools console; expect 0 errors and 0 warnings |
| No CLS | Lighthouse CLS score < 0.1 |
| Build passes | `npm run build` exits 0 |

### Image generation gate decision tree

```
Does this harness have native image_gen (Codex)?
  |
  +-- YES: load codex.md, run gates 2-4 in order, do not start code until gate 4 passes
  |
  +-- NO: announce skip in one line, proceed direct from shape to code, use brief alone as visual reference
```

The announcement is required because silent skipping makes future debugging harder; "I skipped this because the harness lacks image_gen" is auditable, silent skipping is not.

## WHAT'S MISSING

- **No design system integration.** Craft assumes a clean slate; doesn't address "this project already has a design system, build the feature inside it" workflows.
- **No multi-page craft workflow.** Step 5 says "long-form brand surfaces, inspect major sections individually" but there's no flow for "this is 5 connected pages, craft them as one coherent system."
- **No PR description / handoff format.** Step 6 "present to user" is conversational; no template for what gets written into the PR description, commit message, or handoff doc for a teammate.
- **No regression testing prescription.** "Re-inspect" is mentioned but no visual regression test setup (Chromatic, Playwright snapshots, Percy).
- **No collaboration model.** What if the user wants to push back on a craft decision mid-implementation? No protocol for graceful pivot.
- **No "what to do if shape was skipped" recovery.** If the user runs `/impeccable craft` and shape didn't happen (somehow), the flow doesn't describe recovery beyond "run shape first."
