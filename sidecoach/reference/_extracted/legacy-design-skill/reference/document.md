# Impeccable document.md (extracted)
...
**The [Rule Name] Rule.** [Short, forceful prohibition or doctrine, e.g. "The One Voice Rule. The primary accent is used on at most 10% of any given screen. Its rarity is the point."]
...
- **Body** ([weight], [size], [line-height]): [Purpose. Include max line length like 65 to 75ch if relevant.]
...
1. **Color strategy.** Pick one:
   - Restrained: tinted neutrals + one accent at most 10%
   - Committed: one saturated color carries 30 to 60% of the surface
   - Full palette: 3 to 4 named color roles, each deliberate
...
- **Cite PRODUCT.md anti-references by name** in the Do's and Don'ts section. If PRODUCT.md lists "SaaS landing-page cliches" or "generic AI tool marketing" as anti-references, the DESIGN.md Don'ts should repeat those phrases verbatim so the visual spec enforces the strategic line.
...
### Stitch frontmatter schema cheatsheet

Allowed top-level keys: `name`, `description`, `colors`, `typography`, `rounded`, `spacing`, `components`. That's it. Anything else triggers Stitch lint errors.

For non-Stitch keys (motion, breakpoints, shadows, etc.), use the `.impeccable/design.json` sidecar's `extensions` field.

### Color format choice tree

```
Project doctrine?
  +-- "OKLCH only" or uses P3 colors -> OKLCH in frontmatter, accept Stitch lint warning
  +-- "Strict Stitch compliance" or plans to export via Tailwind/DTCG -> hex in frontmatter, OKLCH only in prose
  +-- No doctrine stated -> hex in frontmatter, document why in Overview
```

### The 8-prop component schema (Stitch limit)

Components in frontmatter can ONLY use: `backgroundColor`, `textColor`, `typography`, `rounded`, `padding`, `size`, `height`, `width`.

For shadows, motion, focus rings, backdrop-filter, transitions, gradients, hover variants beyond color/text/padding swap: use the sidecar's `components[]` array with full HTML/CSS snippets.

### Named Rule template

`**The [Name] Rule.** [Short, forceful doctrine. Active voice. Names a specific thing to do or not do.]`

Examples (good):
- The One Voice Rule. The primary accent is used on at most 10% of any given screen.
- The Flat-By-Default Rule. Surfaces are flat at rest. Shadows appear only as a response to state.
- The No-Italic-Body Rule. Italic is for emphasis only, never body type.

Examples (bad):
- The Cohesion Rule. We try to keep things consistent across the design system. (Too vague, hedged)
- The Color Rule. Use brand colors. (Tautological)

## WHAT'S MISSING

- **No DESIGN.md diff / migration tool.** When DESIGN.md is regenerated, no diff is shown; user just sees overwrite.
- **No multi-theme DESIGN.md.** Light/dark mode is mentioned in Elevation prose but the frontmatter schema has one set of color values. Theme variants need a sidecar approach not documented.
- **No DESIGN.md authoring without scan/seed.** What if the user has DESIGN.md in their head but no code yet, and they want a third mode (interview-then-write)? Not provided.
- **No way to validate DESIGN.md is internally consistent.** If `colors.primary` is referenced by `button-primary.backgroundColor` but doesn't exist, no linter inside this skill catches it; you have to run Stitch's own linter.
- **No tonal-ramp generation prescription.** "Synthesize in OKLCH" is the prescription; no exact algorithm (lightness step size, chroma adjustment per step) is given.
- **No Stitch alternative format support.** If a project uses Material Design tokens, Tailwind config, Style Dictionary, W3C DTCG, etc. as its source of truth, document.md doesn't say how to keep DESIGN.md in sync with those.
