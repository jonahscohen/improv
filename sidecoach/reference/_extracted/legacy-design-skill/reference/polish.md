# Impeccable polish.md (extracted)
...
### Drift root-cause taxonomy

Polish's classification of drift causes is load-bearing; without it, every fix is symptomatic. Expanded:

| Drift cause | Symptom | Fix |
|---|---|---|
| Missing token | A value (e.g., a specific blue, a 12px radius) is hardcoded in N places | Define the token in DESIGN.md + token file, then replace all N occurrences |
| One-off implementation | A `<CustomButton>` was built when `<Button>` exists | Swap to shared component; delete the custom |
| Conceptual misalignment | A flow's IA doesn't match neighbors (e.g., modal-edit where the rest uses inline-edit) | Rework the flow shape, not the styling |
| Stale convention | The whole design system shifted (new spacing scale) and this corner wasn't updated | Update; mark the migration in changelog |
| Accidental coupling | A "design token" is actually used semantically in two unrelated places that need to diverge | Split the token into two named-by-purpose tokens |

### Quality-bar decision

| User signal | Bar | Time budget |
|---|---|---|
| "MVP, ship today" | Functional clean: states, accessibility, no broken paths | 30-60 min |
| "Feature for the next release" | Production: above + drift fixes + consistent transitions | 2-4 hr |
| "Flagship / hero feature" | Studio: above + optical alignment + delight moments + perfect at all breakpoints | 4+ hr |
| "Public marketing surface" | Brand: above + signature moves + cross-section coherence | 8+ hr |

Ask the user once; don't grind for studio quality on an MVP.

### Polish in adversarial review mode

For studio / flagship quality, run a final pass as if you're a hostile reviewer:

1. Find the one thing you'd be embarrassed about. Fix it.
2. Squint test: blur the page in your mind. Does hierarchy survive?
3. Use the keyboard only. Are all flows accessible?
4. Toggle dark mode (if applicable). Anything broken?
5. Resize to 320px wide. Anything overflowing?
6. Open DevTools console. Anything red?

If all 6 pass, ship.

## WHAT'S MISSING

- **No polish-as-CI flow.** Polish is human-triggered; no automated "polish bot" that runs on every PR.
- **No design-system gap reporting.** When polish finds a missing token or component, it fixes the local instance but doesn't open a tracking issue for adding the token/component to the design system.
- **No "polish budget" framework.** "2-4 hours" is in the extension; the original lacks any time-to-quality mapping.
- **No team handoff.** Polish is a solo activity in this doc; no way to delegate cosmetic fixes vs functional fixes across multiple people.
- **No visual regression integration.** Final Verification suggests "compare to design" but no Chromatic/Percy/Playwright snapshot integration.
- **No "what to polish first" priority within a session.** All polish areas are listed equally; no recommended ordering for tight time budgets.
