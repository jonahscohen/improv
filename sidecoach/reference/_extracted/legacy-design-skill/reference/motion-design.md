# Impeccable motion-design.md (extracted)
...
:root {
  ...
  /* Exit durations (~75% of enter) */
  --duration-instant-exit: 75ms;
  --duration-base-exit:    180ms;
  --duration-slow-exit:    300ms;
...
### Banned easings (do not use)

```css
/* Banned: bounce, draws attention to the animation */
/* cubic-bezier(0.34, 1.56, 0.64, 1) */

/* Banned: elastic, the same plus overshoot */
/* cubic-bezier(0.68, -0.6, 0.32, 1.6) */

/* Banned: CSS default `ease`, asymmetric and dated */
/* (any property: ease) */
```

### Animation-property matrix (cost-aware)

| Property animated | Cost | Notes |
|---|---|---|
| transform: translate | cheap | GPU compositor, no layout |
| transform: scale | cheap | GPU compositor, no layout |
| transform: rotate | cheap | GPU compositor, no layout |
| opacity | cheap | GPU compositor |
| filter: blur (small radius) | medium | GPU, but bound to layer |
| filter: blur (large radius) | expensive | Can drop frames |
| backdrop-filter | expensive | Re-rasters underneath every frame |
| background-color | cheap | Paint only, no layout |
| color | cheap | Paint only |
| box-shadow | medium | Paint, no layout |
| clip-path | medium | Paint |
| width / height | expensive | Layout reflow |
| top / left / margin | expensive | Layout reflow |
| height (grid-template-rows OK) | use grid instead | Reflows the layout |

### Stagger pattern (CSS-only, custom-property driven)
...
Per-item delay 50ms; cap at 10 items (500ms total). For longer lists, reduce to 25-30ms per item.

### Reduced-motion full pattern (the strict version)
...
This nukes ALL animation. For an alternative where you preserve some animations (loading spinners, progress) but disable spatial movement, use targeted overrides on specific classes instead.

## WHAT'S MISSING

- **No motion taxonomy by intent (feedback / state / entrance / decorative).** Animations are categorized; intents are implicit.
- **No spring-physics catalog.** Spring is mentioned for "natural motion"; no actual `stiffness/damping/mass` recipes.
- **No relationship to DESIGN.md.** Motion tokens should live in DESIGN.md sidecar; not addressed here.
- **No "is this animation worth it?" decision rubric.** Animation budget per surface not defined.
- **No mobile-specific motion guidance.** Touch interactions, momentum-scroll behavior, swipe gestures - not covered.
