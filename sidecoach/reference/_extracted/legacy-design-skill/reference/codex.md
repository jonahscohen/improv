...
### The 4 stop points, enumerated

| Step | Stop until... | Common compression failure |
|---|---|---|
| A | User answers direction questions | Treating shape's "confirm" as Step A's answer |
| B | User confirms the palette artifact | Generating mocks against an unconfirmed palette |
| C | User picks one direction (or delegates) | Picking based on personal preference |
| D | Direction is locked AND assets inventoried | Skipping Step E inventory |

Each compression doubles the chance the implementation drifts from intent.

### Mock-fidelity inventory template

For each major ingredient in the approved mock:

```
Ingredient: [name, e.g. "hero photograph of espresso machine"]
Implementation path:
  - [ ] Semantic HTML/CSS (e.g., text headline)
  - [ ] Sourced raster (e.g., Unsplash photo ID + URL)
  - [ ] Generated raster (impeccable_asset_producer subagent)
  - [ ] SVG (inline path)
  - [ ] Canvas/WebGL (component)
  - [ ] Icon from library (e.g., Lucide "coffee")
  - [ ] Accepted omission (user signed off on dropping)
Dimensions: WxH
Transparency: yes/no
Notes: [what should NOT become CSS scenery]
```

The inventory IS the contract for the build phase. Missing this list = the build drifts.

### Asset producer subagent passing template

```
Task: Produce N raster assets matching the approved mock.

Mock reference: /path/to/approved-mock.png
Output directory: ./public/images/feature/

Crops needed:
  - hero-espresso.webp, 1600x900, no transparency
  - product-shot-1.webp, 800x800, transparent background
  - texture-grain.png, 400x400, ~30% opacity

Style notes:
  - Color palette: oklch(95% 0.005 30) cream + oklch(25% 0.10 30) espresso
  - Lighting: morning natural, soft shadows
  - Don't: include any text in images (text is rendered live)
  - Don't: include people (this is a product brand, not lifestyle)

Avoid list:
  - Glossy / "fashion-magazine" lighting (brand is mechanical, not glamorous)
  - Tilted angles (always orthogonal or 30-degree max)
```

## WHAT'S MISSING

- **No image-quality grading.** What constitutes a "good" generated mock? No rubric.
- **No fallback if image generation fails.** If the API errors or produces unusable output, what's the recovery?
- **No cost / time estimates.** Image generation isn't instant; user has no expectation of how long Step C takes.
- **No format prescription for the palette artifact.** "Typography pairing + swatches + motif" but no canonical layout.
- **No relationship to design references.** Generated mocks could be informed by real-world references from a design-reference catalog; not addressed.
