# Impeccable overdrive.md (extracted)
...
### Overdrive proposal template

Before writing any code, present 2-3 directions in this shape:

```markdown
**Direction 1: [name]**
- What it looks like: [1-2 sentences]
- Technique: [main API or library]
- Browser support: [matrix]
- Performance cost: [low / medium / high]
- Fallback: [how it degrades]
...
Which direction matches what you have in mind?
```

The pause + question is non-negotiable. Skipping it produces work that gets thrown away.

### Browser-support cheat sheet

| API | Chrome / Edge | Firefox | Safari |
|---|---|---|---|
| View Transitions (same-doc) | yes | yes | yes |
| View Transitions (cross-doc) | yes | no | yes (partial) |
| Scroll-driven animations | yes | flag only | yes |
| @starting-style | yes | yes | yes |
| @property | yes | yes | yes |
| WebGPU | yes | flag only | partial |
| WebGL2 | yes | yes | yes |
| OffscreenCanvas | yes | yes | yes (16.4+) |
| WASM | yes | yes | yes |
| Web Audio | yes (gesture-gated) | yes | yes |

Always provide a fallback path for anything not in all-green.

### Reduced-motion fallback patterns

| Overdrive effect | Reduced-motion replacement |
|---|---|
| Shader background | Static screenshot of the shader |
| Scroll-driven parallax | Static layered image |
| Spring physics motion | Crossfade or cut |
| View Transition morph | Crossfade |
| Particle system | Static decorative SVG |
| Live filter chain | Pre-rendered effect |

The static fallback should be designed deliberately, not auto-derived.

## WHAT'S MISSING

- **No "is this project ready for overdrive?" gate.** If the design fundamentals are weak, overdrive amplifies the wrongness. Doc says don't use ambition to mask weak fundamentals, but no precondition check.
- **No cost-budget guidance.** Some overdrive effects cost a week to build well; no time estimates.
- **No "stop here" criterion.** When is the polish good enough? "Inevitable" is the bar but no test.
- **No measurement post-launch.** Did the overdrive move engagement metrics? Doc treats it as terminal.
- **No collaboration with designers.** Overdrive often needs custom assets (shader textures, illustration); workflow not described.
