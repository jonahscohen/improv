...
export class FlowExecutionEngine {
  ...
  async process(utterance: string, context: Partial<FlowExecutionContext> = {}): Promise<SidecoachResult> {
      ...
      for (const flowId of commandMatch.flowIds) {
        ...
        try {
          ...
          if (handler.canExecute(enrichedCtx)) {
            ...
            if (result.status === 'success') {
              const validatorsForFlow = getValidatorsForFlow(flowId);
              if (validatorsForFlow.length > 0) {
                const validations = this.compositionEngine.validateMultipleDomains(validatorsForFlow, result);
    ...
    const laneResolution = resolveSidecoachInput(utterance, path.resolve(__dirname, '..', 'data', 'sidecoach-lanes.json'));
    ...
    if (flowAResult.status === 'success') {
      const validatorsForFlowA = getValidatorsForFlow('flowA_brand_verify');
  ...
  private toRenderedAuditResult(audit: RenderedAuditResult, command: string): SidecoachResult {
    ...
    const byRuleMap = new Map<string, { rule: string; lens: string; count: number }>();
    ...
    const metricOf = (d?: string): string | undefined => {
    ...
    const numMetric = (m?: string): number => {
      if (!m) return Infinity;
      const r = m.match(/(\d+(?:\.\d+)?)\s*:\s*1/); if (r) return parseFloat(r[1]);   // contrast: lower worse
      const px = m.match(/(\d+(?:\.\d+)?)px/); if (px) return parseFloat(px[1]);       // px: smaller worse
      const d = m.match(/(\d+(?:\.\d+)?)\/100/); if (d) return -parseFloat(d[1]);      // density: higher worse
      return Infinity;
    };
    const sevRank = (s: string) => (s === 'blocking' ? 0 : 1);
    ...
    const fixMap = new Map<string, { selector?: string; metric?: string; rule: string; severity: string; n: number }>();
    for (const f of audit.findings) {
      ...
      if (better) fixMap.set(key, { selector: f.selector, metric: m, rule: f.rule, severity: f.severity, n });
    }
    ...
    const counts: SeverityCounts = audit.severityCounts;
    const verdict = computeVerdict(counts); // clean ONLY when blocking+warning are 0 (and we DID render)
    const findings: FindingEntry[] = audit.findings.map((f) => ({
      severity: f.severity,
    ...
    const letter: LetterGrade = passRateToLetter(passRate);
...
export interface SidecoachResult {
  ...
  audit?: {
    ...
    // Grouped counts over ALL findings (for the report's category/rule breakdown).
    byRule: { rule: string; lens: string; count: number }[];
    // Top concrete fixes (blocking first): full selector + a compact metric, never truncated mid-string.
    topFixes: { selector?: string; metric?: string; rule: string; severity: string }[];
