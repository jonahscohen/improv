 ...
 */
export const LAW_CRAFT: Record<string, CraftNote> = {
  // ==================== TYPOGRAPHY ====================
 ...
 */
export function lawKeysForDomain(domain: string): string[] {
  const prefix = `law/${domain}/`;
  return Object.keys(LAW_CRAFT).filter((k) => k.startsWith(prefix));
}

/** Every domain the law corpus covers. */
export function lawDomains(): string[] {
  return [...new Set(Object.keys(LAW_CRAFT).map((k) => k.split('/')[1]))].sort();
}

/** All law keys for a list of domains, in domain order then declaration order. */
export function lawKeysForDomains(domains: string[]): string[] {
  const out: string[] = [];
  for (const d of domains) out.push(...lawKeysForDomain(d));
  return out;
}
