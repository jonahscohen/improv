...
import {
  ...
  PolishCraftNote,
  banCraftNote,
  normalizeCraftKey as normalizePolishKey,
} from './polish-craft';
import { CRAFT_SEVERITY_RANK, CRAFT_UNRANKED } from './craft-probe';
 ...
 * defect name.
 */
export const REGISTRY_CRAFT: Record<string, CraftNote> = {
...
let _lawCraft: Record<string, CraftNote> | null = null;
function lawCraft(): Record<string, CraftNote> {
  if (_lawCraft === null) {
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      _lawCraft = require('./craft-laws').LAW_CRAFT as Record<string, CraftNote>;
    } catch (e) {
      _lawCraft = {};
      process.stderr.write(
        `craft-corpus: law corpus unavailable (${e instanceof Error ? e.message : String(e)}). ` +
...
let _registrySeverity: Map<string, string> | null = null;
/** canonicalRuleKey -> registry severity, read from the registry so the mapping cannot drift. */
function registrySeverities(): Map<string, string> {
  if (_registrySeverity === null) {
    _registrySeverity = new Map();
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { RULES } = require('./product-rule-registry');
      for (const r of RULES as Array<{ canonicalRuleKey: string; severity: string }>) {
        _registrySeverity.set(r.canonicalRuleKey, r.severity);
      }
    } catch (e) {
      process.stderr.write(
        `craft-corpus: product-rule-registry unavailable (${e instanceof Error ? e.message : String(e)}). ` +
 ...
 */
export function resolveCraftNote(rule: string | number | null | undefined): CraftNote | undefined {
  if (rule === null || rule === undefined) return undefined;
  const raw = String(rule).trim();
  if (!raw) return undefined;
  if (REGISTRY_CRAFT[raw]) return REGISTRY_CRAFT[raw];
  const laws = lawCraft();
  if (laws[raw]) return laws[raw];
  // Delegate every polish shape (canonical key, number, polish-standard:N, ban-*) to the reference impl.
  const polishKey = normalizePolishKey(rule);
  if (polishKey) {
    return POLISH_CRAFT[polishKey] || POLISH_FINDING_CRAFT[polishKey] || banCraftNote(polishKey);
  }
  return undefined;
}

/** The severity used to rank a note: the registry's, else the note's own, else unranked. */
export function craftSeverityRank(note: CraftNote): number {
  const fromRegistry = registrySeverities().get(note.ruleKey);
  const sev = fromRegistry ?? note.severity;
  return sev ? (CRAFT_SEVERITY_RANK[sev] ?? CRAFT_UNRANKED) : CRAFT_UNRANKED;
}
 ...
 */
export function registryCraftGaps(): string[] {
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { RULES } = require('./product-rule-registry');
    return (RULES as Array<{ canonicalRuleKey: string }>)
...
export const MAX_BRIEF_NOTES = 6;
export const MAX_BRIEF_EXAMPLES = 2;
...
export function selectCraftNotes(
  ...
  limit: number = MAX_BRIEF_NOTES,
): CraftNote[] {
  const seen = new Set<string>();
  const picked: CraftNote[] = [];
  for (const subject of subjects) {
    const note = resolveCraftNote(subject);
    if (!note || seen.has(note.ruleKey)) continue;
...
export function craftBriefLines(
  subjects: Array<string | number>,
  opts: CraftBriefOptions = {},
): string[] {
  const mode = opts.mode ?? 'findings';
  const limit = opts.limit ?? MAX_BRIEF_NOTES;
  const exampleBudget = opts.examples ?? MAX_BRIEF_EXAMPLES;
  const notes = selectCraftNotes(subjects, limit);
  if (notes.length === 0) return [];

  const distinct = new Set(
    subjects.map((s) => resolveCraftNote(s)?.ruleKey).filter((k): k is string => !!k),
  ).size;
  ...
  if (mode === 'findings') {
    ...
    lines.push(
      `Selected from the ${distinct} rule${distinct === 1 ? '' : 's'} that FAILED here, hardest-first` +
    ...
    lines.push(
      `This flow runs before there is a finished artifact to measure, so the ${distinct} point${distinct === 1 ? '' : 's'} ` +
