    ...
    try {
      ...
      if (hasDesignMd) {
        ...
        if (yamlMatch) {
          ...
          const sectionHeader = (line: string): { indent: number; key: string } | null => {
          ...
          const nestedHeader = (line: string): { indent: number; key: string } | null => {
            const m = line.match(/^(\s*)(\w+):\s*(#.*)?$/);
            return m ? { indent: m[1].length, key: m[2] } : null;
          };
          // A real token: `key: value` with a non-empty, non-comment value.
          const leafToken = (line: string): { indent: number; key: string } | null => {
          ...
          const listLeaf = (line: string): { indent: number; key: string } | null => {
          ...
          const listHeader = (line: string): { indent: number; key: string } | null => {
          ...
          const scalarListLeaf = (line: string): { indent: number } | null => {
            ...
            if (/^("[\w.-]+"|'[\w.-]+'|[\w.-]+):(\s|$)/.test(content)) return null;
          ...
          yamlLines.forEach((line, i) => {
            ...
            const scalarIndex = new Map<string, number>(); // parent path -> next [n]
            for (let j = i + 1; j < yamlLines.length; j++) {
              const cur = yamlLines[j];
              const trimmed = cur.trim();
              if (!trimmed || trimmed.startsWith('#')) continue;
              const curIndent = cur.length - cur.trimStart().length;
              if (curIndent <= header.indent) break; // walked out of the subtree
              ...
              if (leaf) {
                ...
                if (sub) pathStack.push({ indent: sub.indent, key: sub.key });
              }
            }
