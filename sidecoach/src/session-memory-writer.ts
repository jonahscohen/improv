...
import { formatFlowMemoryAsMarkdown, FlowMemoryEntry } from './flow-memory-schema';
...
export class SessionMemoryWriter {
   ...
   */
  private buildSessionMemory(flows: FlowHistoryEntry[]): string {
    const lines: string[] = [];
    const today = new Date().toISOString().split('T')[0];
    ...
    flows.forEach((flow) => {
      ...
      const memoryFlow = flow as any;
      if (
        memoryFlow.appliedRules ||
    ...
    const allDecisions = flows
      .flatMap((f) => (f as any).userDecisions || [])
      .filter((d) => d);
    ...
    const allValidations = flows
      .flatMap((f) => (f as any).validationResults || [])
      .filter((v) => v);
   ...
   */
  private formatFlowMemory(flow: any): string {
    const lines: string[] = [];
    ...
    if (flow.appliedRules && flow.appliedRules.length > 0) {
      ...
      flow.appliedRules.forEach((rule: any) => {
    ...
    if (flow.userDecisions && flow.userDecisions.length > 0) {
      ...
      flow.userDecisions.forEach((decision: any) => {
        lines.push(`- ${decision.decision}`);
      });
      lines.push('');
    }
    ...
    if (flow.metrics && flow.metrics.length > 0) {
      lines.push('**Metrics:**');
      flow.metrics.forEach((metric: any) => {
    ...
    if (flow.validationResults && flow.validationResults.length > 0) {
      const failed = flow.validationResults.filter((v: any) => v.result === 'fail');
      if (failed.length > 0) {
        lines.push('**Validation Issues:**');
        failed.forEach((v: any) => {
   ...
   */
  private formatFlowBasic(flow: FlowHistoryEntry): string {
    const lines: string[] = [];
   ...
   */
  private updateMemoryIndex(sessionFilename: string): void {
 ...
 */
export function persistSessionMemory(projectPath?: string): string {
