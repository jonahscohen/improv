...
import { SidecoachPanelModel } from './panel-model';
...
export interface PanelRenderOpts {
  color?: boolean;
  width?: number; // progress-bar cell count
}

export function renderSidecoachPanel(model: SidecoachPanelModel, opts: PanelRenderOpts = {}): string {
  const color = opts.color ?? !process.env.NO_COLOR;
  const paint = (rgb: string, s: string) => (color ? `\x1b[38;2;${rgb}m${s}\x1b[0m` : s);
  const fg = (s: string) => paint(C.fg, s);
  const dim = (s: string) => paint(C.dim, s);
  const faint = (s: string) => paint(C.faint, s);
  const orange = (s: string) => paint(C.orange, s);
  ...
  const gateCells = model.gates.map((g) => {
    if (g.ok === null) return faint(`${G.dot} ${g.name}`);
