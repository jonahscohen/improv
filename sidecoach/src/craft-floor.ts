...
import { resolveCraftNote, CraftNote } from './craft-corpus';
 ...
 */
export const FLOOR_KEYS: string[] = [
 ...
 */
export const FLOOR_REFUSALS: Array<{ refuse: string; instead: string }> = [
  {
...
export function floorCoverageGaps(): string[] {
  return FLOOR_KEYS.filter((k) => !resolveCraftNote(k));
}

/** The resolved floor notes, in declaration order. */
export function floorNotes(): CraftNote[] {
  return FLOOR_KEYS.map((k) => resolveCraftNote(k)).filter((n): n is CraftNote => !!n);
}
 ...
 */
export function craftFloorLines(opts: FloorOptions = {}): string[] {
  const form = opts.form ?? 'compact';
  const notes = floorNotes().slice(0, opts.limit ?? FLOOR_KEYS.length);
  const lines: string[] = [];

  lines.push('CRAFT FLOOR - the standing minimum for UI work in this project.');
  lines.push(
    'This is a FLOOR, not findings. It loaded because a UI file is being edited' +
  ...
  lines.push(
    'A pinned brief or the project\'s own DESIGN.md tokens override any value here. Your habit does not. ' +
  ...
  lines.push(
    'The floor holds the mechanics; it never picks the direction. With these clear, spend the page on ' +
...
export function craftFloorText(opts: FloorOptions = {}): string {
...
export function isUiPath(filePath: string): boolean {
  ...
  if (!UI_EXTENSIONS.some((e) => lower.endsWith(e))) return false;
  // Test files and generated output are not UI work being authored.
  if (/(^|\/)(node_modules|dist|build|coverage)\//.test(lower)) return false;
