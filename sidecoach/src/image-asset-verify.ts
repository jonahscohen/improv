...
import { decodePng, readPngHeader } from './image-png-codec';

// ---------------------------------------------------------------------------
// Format sniffing (magic bytes - the extension is a claim, not evidence)
// ---------------------------------------------------------------------------
 ...
 */
export function readDimensions(buf: Uint8Array, format: ImageFormat): Dimensions | null {
...
// ---------------------------------------------------------------------------
// Color math. The sRGB linearization breakpoint is 0.04045, matching the SHIPPING rendered scanner
// (src/validators/objective-rendered-scanner.ts, CSS Color 4). That copy lives inside a function serialized
// into the browser page and cannot be imported here, so the constant is restated rather than forked.
// ---------------------------------------------------------------------------
...
function over(src: Rgb & { a: number }, dst: Rgb): Rgb {
  return {
...
export function analyzePixels(
  ...
  width: number,
  height: number,
  opts: { region?: Region; backdrop?: Rgb } = {},
): PixelStats {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (x + 1 < w) {
        pairs++;
        if (Math.abs(l - lum[y * w + x + 1]) > EDGE_DELTA) edges++;
      }
      if (y + 1 < h) {
        pairs++;
        if (Math.abs(l - lum[(y + 1) * w + x]) > EDGE_DELTA) edges++;
      }
    }
  }
 ...
 */
export function contrastAgainstPixels(
  rgba: Uint8Array,
  width: number,
  height: number,
  ink: Rgb,
  opts: { region?: Region; backdrop?: Rgb } = {},
): { worst: number; mean: number; sampled: number } {
  const backdrop = opts.backdrop || { r: 255, g: 255, b: 255 };
  const r = opts.region || { x: 0, y: 0, width, height };
  const x0 = Math.max(0, Math.min(width - 1, Math.floor(r.x)));
  const y0 = Math.max(0, Math.min(height - 1, Math.floor(r.y)));
...
export interface VerifyReport {
  ...
  contrast: { worst: number; mean: number; sampled: number } | null;
...
export const SYNTHETIC_MARKER_KEY = 'sidecoach-synthetic';

function fold(checks: AssetCheck[]): AssetVerdict {
 ...
 */
export function verifyAsset(buf: Uint8Array, contract: AssetContract): VerifyReport {
  ...
  let contrast: VerifyReport['contrast'] = null;
  let synthetic = false;
  let decodeReason = '';
  ...
  if (sniffed === 'png') {
    ...
    if (decoded.ok) {
      ...
      if (pixels.uniqueColors < th.minUniqueColors) blankFailures.push(`${pixels.uniqueColors} unique colors < ${th.minUniqueColors}`);
      if (pixels.stdDevLuminance < th.minLuminanceStdDev) blankFailures.push(`luminance stddev ${pixels.stdDevLuminance.toFixed(5)} < ${th.minLuminanceStdDev}`);
      if (pixels.edgeDensity < th.minEdgeDensity) blankFailures.push(`edge density ${pixels.edgeDensity.toFixed(5)} < ${th.minEdgeDensity}`);
      checks.push(
      ...
      if (contract.placement) {
        ...
        if (!ink || !backdrop) {
          ...
          if (outOfBounds) {
            ...
            checks.push(
              got >= contract.placement.minContrast
                ? {
                    id: 'contrast-at-placement',
                    status: 'pass',
...
export function summarizeReport(report: VerifyReport): string {
  ...
  if (report.dimensions) parts.push(`${report.dimensions.width}x${report.dimensions.height}`);
  if (report.sniffedFormat) parts.push(report.sniffedFormat);
  if (failed.length) parts.push(`failed=[${failed.join(',')}]`);
  if (unver.length) parts.push(`unverified=[${unver.join(',')}]`);
  return parts.join(' ');
}
