...
import {
  ...
  SUBJECTIVE_RULES,
  BUZZ_MIN_DISTINCT_PEAK,
  DEFAULT_STACK_GROUND_GATED,
  DEFAULT_STACK_SHARE,
  TYPEFACE_MIN_CONTENT_CHARS,
  nestedCardsFindingFromScore,
  typefaceFindingFromScore,
  type NestedCardsScore,
  type NestedCardsPair,
  type TypefaceScore,
} from '../validators/subjective-rendered-scanner';
import { listRenderedManifest } from '../product-rule-registry';

const SC = path.resolve(__dirname, '..', '..');
// eslint-disable-next-line @typescript-eslint/no-var-requires
const detect = require(path.join(SC, 'bin', 'sidecoach-detect.js')) as {
  collapseForDisplay: (f: { rule: string; lens: string; severity: string; selector?: string; detail?: string }[]) =>
    { rule: string; lens: string; severity: string; selector?: string; detail?: string }[];
};
...
const asNumber = (v: number): number => v;
const asBoolean = (v: boolean): boolean => v;

/* ---- 1. the buzzword gate's inline copy must equal the exported constant --------------------------------- */
{
  const src = readFileSync(path.join(SC, 'src', 'validators', 'subjective-rendered-scanner.ts'), 'utf8');
  ...
  if (m) {
    check(Number(m[1]) === BUZZ_MIN_DISTINCT_PEAK,
      `inline BUZZ_MIN_DISTINCT_PEAK (${m[1]}) has drifted from the exported constant (${BUZZ_MIN_DISTINCT_PEAK})`);
  }
  const gate = asNumber(BUZZ_MIN_DISTINCT_PEAK);
  check(gate === 2,
    `the retune froze the buzzword gate at 2 distinct PEAK terms; it now reads ${gate}. Changing it is a re-tune and needs a fresh held-out measurement.`);
}

/* ---- 2. nested-cards: operating point UNCHANGED (the retune was rejected by its own held-out), ONE finding -- */
{
  const pair = (outerViewportWidthFrac: number): NestedCardsPair => ({
  ...
  const scoreWith = (frac: number): NestedCardsScore => ({ cardCount: 2, viewportWidth: 1280, pairs: [pair(frac)] });

  check(nestedCardsFindingFromScore(scoreWith(0.3)) !== null, 'nested-cards must fire on a nested pair');
  ...
  check(nestedCardsFindingFromScore(scoreWith(1.0)) !== null,
    'nested-cards must still fire on a full-bleed outer box - the viewport-width guard was rejected by its held-out measurement, so re-adding it needs a fresh one');
  check(nestedCardsFindingFromScore({ cardCount: 0, viewportWidth: 1280, pairs: [] }) === null,
    'nested-cards must not fire with no nested pairs');
  ...
  const many: NestedCardsScore = { cardCount: 40, viewportWidth: 1280, pairs: Array.from({ length: 25 }, () => pair(0.3)) };
  const f = nestedCardsFindingFromScore(many);
  check(f !== null, 'nested-cards must emit a finding for a 25-pair score');
  check(!!f && /25 card-in-card nesting/.test(f.detail || ''),
    `the single nested-cards finding must carry the COUNT (got: ${f ? f.detail : 'null'})`);
}

/* ---- 3. default-typeface ground A is gated off by default ------------------------------------------------- */
{
  const score: TypefaceScore = {
    contentChars: TYPEFACE_MIN_CONTENT_CHARS + 500,
    defaultStackChars: TYPEFACE_MIN_CONTENT_CHARS + 500,
    defaultStackShare: 1.0,
    families: [{ family: 'system-ui', chars: TYPEFACE_MIN_CONTENT_CHARS + 500 }],
    dominantFamily: 'system-ui', dominantShare: 1.0,
    declaredFamilies: [], defaultSelector: 'p',
  };
  const gated = asBoolean(DEFAULT_STACK_GROUND_GATED);
  check(gated === true,
    'default-typeface ground A must stay GATED until its real-page precision is measured (see the scanner note)');
  check(score.defaultStackShare >= DEFAULT_STACK_SHARE,
    'ANCHOR: the probe score must be above the ground-A threshold, or the silence below proves nothing');
  check(typefaceFindingFromScore(score, { enableDefaultStackGround: true }) !== null,
    'ANCHOR: ground A must still fire when explicitly opted in - the gate is a default, not a deletion');
  check(typefaceFindingFromScore(score, {}) === null,
    'default-typeface ground A must be SILENT with no opt-in');
  // ground B is unaffected by the gate and still fires on a known-but-absent committed family.
  const chosen: TypefaceScore = { ...score, defaultStackChars: 0, defaultStackShare: 0, dominantFamily: 'alluvium sans', families: [{ family: 'alluvium sans', chars: 700 }] };
  check(typefaceFindingFromScore(chosen, { brandFamilies: ['Verge Serif'] }) !== null,
    'ground B (brand mismatch) must be unaffected by the ground-A gate');
}

/* ---- 4. numbered-section-markers is gone ------------------------------------------------------------------ */
{
  check(!(SUBJECTIVE_RULES as readonly string[]).includes('numbered-section-markers'),
    'numbered-section-markers must be removed from SUBJECTIVE_RULES (it was inert: 0 fires / 6 positives)');
  check((SUBJECTIVE_RULES as readonly string[]).includes('marquee'),
    'ANCHOR: its sibling marquee must still be present, or the absence above proves only that the list broke');
  const manifest = listRenderedManifest().map((m) => m.scannerRule);
  check(!manifest.includes('numbered-section-markers'),
    'numbered-section-markers must be removed from the rendered registry manifest - an inert rule there is a claim nobody can cash');
...
/* ---- 5. the detect CLI collapses per-element objective findings for display -------------------------------- */
{
  ...
  check(!!line && /35 element\(s\)/.test(line.detail || ''),
    `the collapsed line must carry the element COUNT (got: ${line ? line.detail : 'missing'})`);
  check(!!line && /2\.91:1/.test(line.detail || ''),
    'the collapsed line must carry a representative measurement');
  check(collapsed.some((f) => f.rule === 'skipped-heading'),
    'ANCHOR: a non-collapsed rule must survive the collapse untouched');
  // a single finding is NOT rewritten into a "1 element(s)" line.
  const one = detect.collapseForDisplay([lc[0]]);
  check(one.length === 1 && one[0].detail === '2.91:1 (need 4.5:1)',
    'a lone low-contrast finding must pass through with its original detail');
}
...
const dedupeDoc = `<!doctype html><html><head><meta charset="utf-8"></head><body style="font-family:'Fixture Sans',sans-serif"><main>${
  Array.from({ length: 24 }, (_, i) => `<p style="font-size:1${i % 3}px">${S.repeat(2)}</p>`).join('')
}</main></body></html>`;

// four independent card-in-card nestings on one page
const nestedCard = (i: number) => `<div style="width:400px;height:300px;border-radius:12px;border:1px solid #ddd"><p>Card ${i}</p><div style="width:200px;height:120px;border-radius:8px;border:1px solid #ccc"><p>Inner ${i}</p></div></div>`;
...
async function run(): Promise<void> {
  ...
  try {
    const findings = await analyzeHtmlOnBrowserSubjective(browser, dedupeDoc);
    const tiny = findings.filter((f) => f.rule === 'tiny-text');
    check(tiny.length === 1,
      `tiny-text must emit exactly ONE page-level finding (got ${tiny.length}; it used to emit up to 20)`);
    check(tiny.length === 1 && /across \d+ element\(s\)/.test(tiny[0].detail || ''),
      `the single tiny-text finding must carry the offender COUNT (got: ${tiny[0] ? tiny[0].detail : 'none'})`);
    ...
    const nestedFindings = await analyzeHtmlOnBrowserSubjective(browser, nestedDoc);
    const nc = nestedFindings.filter((f) => f.rule === 'nested-cards');
    check(nc.length === 1,
      `nested-cards must emit exactly ONE finding for a page with 4 nestings (got ${nc.length}; it used to emit one per outer card)`);
    check(nc.length === 1 && /4 card-in-card nesting/.test(nc[0].detail || ''),
      `the production nested-cards finding must carry the COUNT (got: ${nc[0] ? nc[0].detail : 'none'})`);
  } finally {
    await browser.close();
  }
  if (failures.length) throw new Error(`taste-precision-gates FAILED (${failures.length}):\n  ${failures.join('\n  ')}`);
