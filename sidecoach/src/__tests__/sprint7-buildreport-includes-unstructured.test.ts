...
const historySandboxes: string[] = [];

function isolateHistory(tag: string) {
...
function stepRan(result: any, flowId: string): boolean {
  return (result?.flowResults || []).some(
    (f: any) => f.flowId === flowId && f.status === 'success',
  );
}
