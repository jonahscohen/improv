...
import { assemblePanelModel, laneStepToPanelModel, coverageForVerdict } from '../panel-model';
...
const renderAnsi: (result: any, utterance?: string) => string = present.render;
...
function absentRe(hay: string, re: RegExp, label: string): void {
  if (re.test(hay)) { console.error('FAIL ' + label + ': should not match ' + re); failures++; }
}
...
function assertNoBarePass(out: string, label: string): void {
...
ok(coverageForVerdict('clean') === 'verified-clean', 'coverageForVerdict: clean -> verified-clean');
ok(coverageForVerdict('warnings-only') === 'checked', 'coverageForVerdict: warnings-only -> checked');
ok(coverageForVerdict('blocked') === 'checked', 'coverageForVerdict: blocked -> checked');
ok(coverageForVerdict(undefined) === undefined, 'coverageForVerdict: no verdict -> undefined (no claim)');
...
const cleanModel = assemblePanelModel({ flowResults: chain, report: report({}), confidence: 0.9 });
ok(cleanModel.coverage === 'verified-clean', 'panel-model: clean verdict -> coverage verified-clean');
const cleanCard = renderSidecoachPanel(cleanModel, { color: false });
contains(cleanCard, 'VERIFIED CLEAN', 'panel clean card: shows VERIFIED CLEAN coverage line');

const blockedModel = assemblePanelModel({ flowResults: chain, report: report({ verdict: 'blocked', overallGrade: 'F', severityCounts: { blocking: 2, warning: 0, info: 0 } }) });
ok(blockedModel.coverage === 'checked', 'panel-model: blocked verdict -> coverage checked');
const blockedCard = renderSidecoachPanel(blockedModel, { color: false });
...
const unmeasured = assemblePanelModel({
...
const unmeasuredCard = renderSidecoachPanel(unmeasured, { color: false });
...
const laneInconclusive = laneStepToPanelModel({
  currentVerb: 'audit',
...
const laneIncCard = renderSidecoachPanel(laneInconclusive, { color: false });
...
const laneError = laneStepToPanelModel({
  currentVerb: 'audit', flowIds: ['flowK_multi_lens_audit'] as any,
  checklist: [{ id: 'flowK_multi_lens_audit:1', label: 'a11y', completed: true }],
...
const laneClean = laneStepToPanelModel({
  currentVerb: 'audit', flowIds: ['flowK_multi_lens_audit'] as any,
  checklist: [{ id: 'flowK_multi_lens_audit:1', label: 'a11y', completed: true }],
...
const partialCleanPanel = assemblePanelModel({ flowResults: chain, report: report({}), partial: true });
ok(partialCleanPanel.coverage === 'not-fully-checked', 'F4 panel: partial + clean verdict -> not-fully-checked');
const partialCleanCard = renderSidecoachPanel(partialCleanPanel, { color: false });
...
const laneZeroValidators = laneStepToPanelModel({
  currentVerb: 'shape',
  flowIds: ['flowA_brand_verify'] as any,
...
const laneZeroCard = renderSidecoachPanel(laneZeroValidators, { color: false });
...
const partialAuditPanel = assemblePanelModel({ flowResults: chain, report: partialAuditReport, auditPartial: true });
ok(partialAuditPanel.coverage === 'partially-checked', 'F6 panel: partial audit (a lens did not run) -> partially-checked');
const partialAuditCard = renderSidecoachPanel(partialAuditPanel, { color: false });
...
if (failures > 0) { console.error('coverage-honesty: ' + failures + ' failure(s)'); process.exit(1); }
