...
import {
  FLOOR_KEYS, FLOOR_REFUSALS, craftFloorLines, craftFloorText, floorCoverageGaps, floorNotes,
  isUiPath, UI_EXTENSIONS,
} from '../craft-floor';
import { craftBriefLines } from '../craft-corpus';
...
ok(FLOOR_KEYS.length >= 10, `the floor is substantive (got ${FLOOR_KEYS.length} keys)`);
ok(floorNotes().length === FLOOR_KEYS.length, 'every floor key produces a note');
ok(new Set(FLOOR_KEYS).size === FLOOR_KEYS.length, 'no duplicate floor keys');
// Each floor note must carry a source, or the floor asserts without being checkable.
for (const n of floorNotes()) {
...
const a = craftFloorText({ filePath: '/a/clean.css' });
const b = craftFloorText({ filePath: '/b/broken.css' });
ok(a !== b, 'the floor names the file being edited');
ok(a.replace('/a/clean.css', 'X') === b.replace('/b/broken.css', 'X'),
  'the floor content is otherwise identical regardless of the project');
// Never empty. The brief is silent on a clean page; the floor is never silent - that is the point.
ok(craftFloorLines().length > 0, 'the floor is never empty');
ok(craftFloorText({ refusals: false }).length > 0, 'the floor is non-empty without refusals');
ok(craftFloorText({ limit: 0 }).length > 0, 'the floor still frames itself with zero notes');
// Both render forms work and the full form carries the rationale the compact form drops.
const full = craftFloorText({ form: 'full' });
contains(full, 'Why:', 'the full form carries the rationale');
contains(full, 'Good:', 'the full form states the target state');
contains(craftFloorText({ form: 'compact' }), 'Do:', 'the compact form carries the action');
absent(craftFloorText({ form: 'compact' }), 'Why:', 'the compact form drops the rationale to save budget');
ok(full.length > craftFloorText({ form: 'compact' }).length, 'the full form is longer than the compact form');
// The limit binds.
ok(craftFloorText({ limit: 3 }).length < craftFloorText({ limit: 12 }).length, 'the note limit binds');

// ---- 5. REFUSALS CARRY REPLACEMENTS -----------------------------------------------------------
ok(FLOOR_REFUSALS.length >= 8, `the refusal list is substantive (got ${FLOOR_REFUSALS.length})`);
for (const r of FLOOR_REFUSALS) {
...
for (const p of ['a.html', '/x/styles.css', 'src/Button.tsx', 'App.vue', 'page.svelte', 'x.scss', 'i.astro', '/X/STYLES.CSS']) {
  ok(isUiPath(p), `isUiPath accepts ${p}`);
}
for (const p of ['logic.ts', 'README.md', 'data.json', 'run.py', 'Makefile', 'x.go']) {
  ok(!isUiPath(p), `isUiPath rejects ${p}`);
}
for (const p of ['node_modules/pkg/a.css', 'dist/out.css', 'build/x.css', 'coverage/a.html', 'Button.test.tsx', 'a.spec.tsx']) {
  ok(!isUiPath(p), `isUiPath rejects non-authored ${p}`);
}
ok(UI_EXTENSIONS.every((e) => e.startsWith('.')), 'every UI extension is dotted');
...
const brief = craftBriefLines(['a11y/form-control-labelled'], { mode: 'findings' }).join('\n');
...
const standardBrief = craftBriefLines(['law/motion/duration-budget'], { mode: 'standard' }).join('\n');
...
if (failures > 0) { console.error('craft-floor: ' + failures + ' failure(s)'); process.exit(1); }
