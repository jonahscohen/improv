...
// ---------------------------------------------------------------------------
function testDeckIntegrity(): void {
  assert(DIRECTION_DECK.length >= 12, `deck should be a small curated set (>=12), got ${DIRECTION_DECK.length}`);
  const ids = deckIds();
  assert(new Set(ids).size === ids.length, 'every direction id must be unique');
  assert(ids.includes(MODEL_DEFAULT_ID), 'MODEL_DEFAULT_ID must be a real deck entry');
  ...
  assert(directionById(MODEL_DEFAULT_ID) !== undefined, 'directionById resolves the default');
  assert(directionById('not-a-real-id') === undefined, 'directionById returns undefined for a non-deck id');
}

// ---------------------------------------------------------------------------
// 2. RNG + seed folding: deterministic, order-independent, seed-sensitive.
// ---------------------------------------------------------------------------
function testRngAndSeed(): void {
  ...
  assert(mixSeed(42, []) === (42 >>> 0), 'mixSeed with no excludes returns the base seed');
  assert(mixSeed(42, ['a', 'b', 'c']) === mixSeed(42, ['c', 'b', 'a']), 'mixSeed is order-independent over the exclusion set');
  assert(mixSeed(42, ['a', 'b']) !== mixSeed(42, ['a', 'b', 'c']), 'a different exclusion set changes the seed');
  assert(mixSeed(42, []) !== mixSeed(43, []), 'a different base seed changes the fold');
}

// ---------------------------------------------------------------------------
// 3. resolveModelTopId + unknownIds - fail-loud id validation.
// ---------------------------------------------------------------------------
function testResolveAndValidate(): void {
  assert(resolveModelTopId(undefined) === MODEL_DEFAULT_ID, 'no explicit model-top => the default-instinct id');
  assert(resolveModelTopId(null) === MODEL_DEFAULT_ID, 'null model-top => the default-instinct id');
  assert(resolveModelTopId('') === MODEL_DEFAULT_ID, 'empty model-top => the default-instinct id');
  const someOther = deckIds().find((id) => id !== MODEL_DEFAULT_ID)!;
  assert(resolveModelTopId(someOther) === someOther, 'a valid explicit model-top is respected');
  let threw = false;
  try { resolveModelTopId('does-not-exist'); } catch { threw = true; }
  assert(threw, 'an unknown explicit model-top must throw (fail loud)');

  assert(unknownIds([]).length === 0, 'no ids => nothing unknown');
  assert(unknownIds(deckIds()).length === 0, 'every deck id is known');
  assert(unknownIds([someOther, 'nope', 'also-nope']).sort().join(',') === 'also-nope,nope', 'unknownIds flags exactly the non-deck ids');
}
...
function testDeterminism(): void {
  const one = roll({ seed: 42, modelTopId: MODEL_DEFAULT_ID });
  const two = roll({ seed: 42, modelTopId: MODEL_DEFAULT_ID });
  ...
  for (let s = 0; s < 80; s++) seen.add(roll({ seed: s, modelTopId: MODEL_DEFAULT_ID }).draw!.id);
...
function testSeedDomain(): void {
  ...
  assert(roll({ seed: 2147483648, modelTopId: MODEL_DEFAULT_ID }).draw!.id === hi.draw!.id, 'high seed is deterministic');
  assert(roll({ seed: 0, modelTopId: MODEL_DEFAULT_ID }).status === 'drawn', 'seed 0 is a valid seed');
}

// ---------------------------------------------------------------------------
// 5. Outside-ranking: the draw is NEVER the model-top, default OR overridden.
// ---------------------------------------------------------------------------
function testOutsideRanking(): void {
  for (let s = 0; s < 300; s++) {
    const out = roll({ seed: s, modelTopId: MODEL_DEFAULT_ID });
    assert(out.draw!.id !== MODEL_DEFAULT_ID, `seed ${s}: draw must be outside the default model-top`);
    assert(!out.eligibleIds.includes(MODEL_DEFAULT_ID), 'the model-top must not appear in the eligible pool');
  }
  ...
  for (let s = 0; s < 300; s++) {
    const out = roll({ seed: s, modelTopId: top });
    assert(out.draw!.id !== top, `seed ${s}: draw must be outside the overridden model-top`);
    if (out.draw!.id === MODEL_DEFAULT_ID) sawDefaultDrawn = true;
  }
...
// ---------------------------------------------------------------------------
function testExclusion(): void {
  const first = roll({ seed: 42, modelTopId: MODEL_DEFAULT_ID }).draw!.id;
  for (let s = 0; s < 300; s++) {
    const out = roll({ seed: s, modelTopId: MODEL_DEFAULT_ID, exclude: [first] });
    assert(out.draw!.id !== first, `seed ${s}: a re-roll must not return the excluded prior draw`);
    assert(out.draw!.id !== MODEL_DEFAULT_ID, `seed ${s}: a re-roll must still never return the model-top`);
    assert(out.excluded.includes(first) && out.excluded.includes(MODEL_DEFAULT_ID), 'the applied exclusion set includes both the prior draw and the model-top');
  }
  // A larger exclusion set: a used id passed in any order is never returned.
  const twoDraws = [first, roll({ seed: 1, modelTopId: MODEL_DEFAULT_ID, exclude: [first] }).draw!.id];
  for (let s = 0; s < 200; s++) {
    const out = roll({ seed: s, modelTopId: MODEL_DEFAULT_ID, exclude: [...twoDraws].reverse() });
    assert(!twoDraws.includes(out.draw!.id), `seed ${s}: no excluded id may be redrawn regardless of order`);
    assert(out.draw!.id !== MODEL_DEFAULT_ID, 'model-top still excluded');
  }
}
...
// ---------------------------------------------------------------------------
function sweep(seed: number, modelTopId: string): { visited: string[]; steps: number } {
  const seen = new Set<string>();
  const visited: string[] = [];
  let exclude: string[] = [];
  let steps = 0;
  for (;;) {
    const out = roll({ seed, modelTopId, exclude });
    ...
    exclude = [...exclude, id];
    steps++;
    assert(steps <= DIRECTION_DECK.length + 1, 'sweep must terminate');
  }
  return { visited, steps };
}
function testFullSweep(): void {
  const expected = DIRECTION_DECK.length - 1; // everything except the model-top
  const s1 = sweep(42, MODEL_DEFAULT_ID);
  ...
  assert(!s1.visited.includes(MODEL_DEFAULT_ID), 'sweep never includes the model-top');

  // Independent of the seed, the SET of visited directions is the whole deck minus the model-top.
  const s2 = sweep(7, MODEL_DEFAULT_ID);
  assert([...new Set(s2.visited)].sort().join(',') === deckIds().filter((id) => id !== MODEL_DEFAULT_ID).sort().join(','),
    'a sweep covers exactly the deck minus the model-top');
  ...
  assert(s3.visited.includes(MODEL_DEFAULT_ID), 'with an override, the sweep includes the former default');
...
function testParseArgs(): void {
  ...
  const cli = require(BIN) as {
    parseArgs: (argv: string[]) => { seed: number | null; modelTop: string | null; exclude: string[]; list: boolean; quiet: boolean };
    EXIT_DRAWN: number; EXIT_USAGE: number; EXIT_EXHAUSTED: number;
  };
...
function testE2E(): void {
  ...
  const allButTop = deckIds().filter((id) => id !== MODEL_DEFAULT_ID);
...
// Reference the imported type so an unused-locals pass stays quiet; it documents the deck entry shape above.
