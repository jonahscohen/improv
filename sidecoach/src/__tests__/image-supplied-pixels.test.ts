...
import { verifyAsset, sniffFormat, SYNTHETIC_MARKER_KEY, type AssetContract } from '../image-asset-verify';
import { encodePng, decodePng } from '../image-png-codec';
import { decodeViaBrowser, bundledPlaywrightPath } from '../image-jpeg-pixels';
...
function buildImage(w: number, h: number): { rgba: Uint8Array; png: Buffer } {
  ...
  return { rgba, png: encodePng(w, h, rgba) };
}
...
function pngContract(over: Partial<AssetContract> = {}): AssetContract {
  return {
    width: W,
    height: H,
    format: 'png',
...
function checkOf(report: ReturnType<typeof verifyAsset>, id: string) {
...
function testNothingLoosened(): void {
  ...
  assert(sniffFormat(jpeg) === 'jpeg', 'the fixture must sniff as jpeg');

  // WITHOUT a supply: every pixel check unverified, reason named, verdict never verified.
  const bare = verifyAsset(jpeg, pngContract());
  ...
  const supplied = verifyAsset(jpeg, {
    ...pngContract(),
  ...
  const flatReport = verifyAsset(jpeg, {
    ...pngContract(),
...
function testSupplyIsChecked(): void {
  const jpeg = minimalJpeg(W, H);

  const wrongDims = verifyAsset(jpeg, {
    ...pngContract(),
  ...
  const malformed = verifyAsset(jpeg, {
    ...pngContract(),
  ...
  if (sniffFormat(headerless) === 'jpeg') {
    const unanchored = verifyAsset(headerless, {
      ...pngContract(),
      decodedPixels: { rgba: IMG.rgba, width: W, height: H, source: 'a test harness' },
    });
    assert(
      /supplied-pixels-unanchored/.test(unanchored.unverifiedReasons.join(' ')),
      'a supply with no header to check against is refused by name',
    );
  }
}
...
function testPlacementGeometryDrift(): void {
  ...
  const exact = verifyAsset(minimalJpeg(W, H), {
    ...pngContract(),
    decodedPixels: { rgba: IMG.rgba, width: W, height: H, source: 'a test harness' },
  });
...
function testProvenanceAndParity(): void {
  ...
  const markedPng = encodePng(W, H, IMG.rgba, { text: [[SYNTHETIC_MARKER_KEY, 'offline-deterministic']] });
  const marked = verifyAsset(markedPng, pngContract({ expectSynthetic: true }));
  ...
  const native = verifyAsset(IMG.png, pngContract());
  const viaSupply = verifyAsset(minimalJpeg(W, H), {
    ...pngContract(),
    decodedPixels: { rgba: IMG.rgba, width: W, height: H, source: 'a test harness' },
  });
  ...
  assert(
    Math.abs(native.contrast!.worst - viaSupply.contrast!.worst) < 1e-9,
    `the same pixels must measure identically either way: ${native.contrast!.worst} vs ${viaSupply.contrast!.worst}`,
  );
  assert(
    native.pixels!.uniqueColors === viaSupply.pixels!.uniqueColors,
    'the blank statistics are identical either way',
  );
}
...
async function testBrowserAgreesWithOracle(): Promise<void> {
  ...
  const decoded = await decodeViaBrowser(jpegBytes, 'jpeg', { playwrightPath: bundledPlaywrightPath() });
  if (!decoded.ok) {
    // A missing browser must degrade to a NAMED refusal, which is itself the property worth asserting.
    assert(
      ['browser-unavailable', 'browser-launch-failed'].includes(decoded.reason),
      `an unavailable browser must refuse by name, got ${decoded.reason}: ${decoded.detail}`,
    );
  ...
  execFileSync('sips', ['-s', 'format', 'png', jpgPath, '--out', oraclePath], { stdio: 'ignore' });
  const oracleResult = decodePng(fs.readFileSync(oraclePath));
  assert(oracleResult.ok, 'the oracle PNG must decode');
  if (!oracleResult.ok) return;
...
async function main(): Promise<void> {
  ...
  console.log(
    'image-supplied-pixels: OK (format still fails on a substitution, a drifted geometry refuses the placement check, no supply means unverified with a named reason, a supply is checked not trusted, provenance cannot be laundered, the PNG path measures identically, browser agrees with an independent decoder)',
  );
}
