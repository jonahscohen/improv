...
function run() {
  ...
  if (ANTI_PATTERN_CHECKS['anti-pattern/identical-card-grids']) throw new Error('identical-card-grids should be DELETED (Stage-2)');
  for (const [k, f] of Object.entries({ gt, gl, ss, mo })) if (!f) throw new Error(`missing ${k}`);
