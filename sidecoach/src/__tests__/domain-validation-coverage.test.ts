...
// append expression.
// ---------------------------------------------------------------------------
async function testDomainValidationAppends(): Promise<void> {
  ...
  check(
    'append: a pre-existing result.validationResults entry survives domain validation (not overwritten)',
    topVrs.some((v) => v.domain === 'handler-preexisting' && v.status === 'fail'),
    `topVrs=${JSON.stringify(topVrs)}`
  );

  // the domain-validation outcome is ALSO present (append added to, did not replace).
  check(
    'append: the domain-validation outcome is present alongside the pre-existing entry',
    topVrs.some((v) => v.domain === 'performance' && v.status !== 'pass'),
    `topVrs=${JSON.stringify(topVrs)}`
  );
  ...
  const memVrs: any[] = (entry && entry.domainValidationResults) || [];
  check(
    'append: BOTH the pre-existing entry and the domain outcome persist to memory',
    memVrs.some((v) => v.domain === 'handler-preexisting') && memVrs.some((v) => v.domain === 'performance'),
    `entryDomainVrs=${JSON.stringify(memVrs)}`
  );
}
