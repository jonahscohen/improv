...
import {
  resolveDirections,
  renderDeckMarkdown,
  renderDeckArtifactHtml,
} from '../direction-deck-present';
import { DIRECTION_DECK, deckIds } from '../direction-deck';
...
function testResolve(): void {
  const ok = resolveDirections(IDS);
  ...
  const unknown = resolveDirections([IDS[0], 'ghost-id', 'also-ghost']);
  ...
  const dup = resolveDirections([IDS[0], IDS[0], IDS[1]]);
...
function testMarkdown(): void {
  const dirs = resolveDirections(IDS).directions;
  const md = renderDeckMarkdown(dirs, { title: 'Pick a direction' });
  ...
  // Determinism.
  assert(renderDeckMarkdown(dirs, { title: 'Pick a direction' }) === md, 'markdown render is deterministic');
  ...
  const injected = renderDeckMarkdown(dirs, { title: 'Bad | Title\nsecond line' });
...
function testArtifact(): void {
  const dirs = resolveDirections(IDS).directions;
  const html = renderDeckArtifactHtml(dirs, { title: 'Pick a direction' });
  ...
  assert(renderDeckArtifactHtml(dirs, { title: 'Pick a direction' }) === html, 'artifact render is deterministic');
}
...
// ---------------------------------------------------------------------------
function testExclusionSelfScan(): void {
...
// ---------------------------------------------------------------------------
function testIdsFromRollJson(): void {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const cli = require(BIN) as { idsFromRollJson: (t: string) => { ids: string[]; malformed: string[] } };
...
function testParseArgs(): void {
  ...
  const cli = require(BIN) as {
    parseArgs: (argv: string[]) => { ids: string[]; surface: string; title: string | null; out: string | null; quiet: boolean };
    EXIT_OK: number; EXIT_USAGE: number;
  };
...
function testE2E(): void {
  ...
  for (const id of IDS) assert(text.stdout.includes(id), `e2e text: carries ${id}`);
  ...
  for (const id of IDS) assert(rich.stdout.includes(id), `e2e rich: carries ${id}`);
  for (const tok of FORBIDDEN) assert(!rich.stdout.includes(tok), `e2e rich: inert static HTML (no ${tok})`);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    let ok = true;
    try {
      out = execSync(pipeCmd, { encoding: 'utf8', cwd: SC, shell: '/bin/bash' });
    } catch (_e) {
      ok = false;
    }
    assert(ok, `e2e real pipe (run ${i + 1}/5): sidecoach-roll | sidecoach-deck must exit 0`);
    assert(
      out.includes('| # | Direction | Axis | Premise |') && !out.includes('no directions'),
      `e2e real pipe (run ${i + 1}/5): a rolled direction is presented, not dropped to "no directions"`,
    );
  }
  ...
  const producer =
    'setTimeout(() => process.stdout.write(JSON.stringify({status:"drawn",draw:{id:process.env.DECK_TEST_ID}})+"\\n", () => process.exit(0)), 120);';
  const delayedCmd = `${q(process.execPath)} -e ${q(producer)} | ${q(process.execPath)} ${q(BIN)} --surface text --quiet`;
  for (let i = 0; i < 3; i++) {
    let out = '';
    let ok = true;
    try {
      out = execSync(delayedCmd, { encoding: 'utf8', cwd: SC, shell: '/bin/bash', env: { ...process.env, DECK_TEST_ID: IDS[0] } });
    } catch (_e) {
      ok = false;
    }
    assert(
      ok && out.includes(IDS[0]) && !out.includes('no directions'),
      `e2e delayed producer (run ${i + 1}/3): a slow producer's roll JSON is read and presented, not lost to a non-blocking-fd race`,
    );
  }
}
