...
const cap = require(CAP) as {
  captureScan: (result: unknown, opts?: Record<string, unknown>) => boolean;
  buildEntry: (result: unknown, nowIso?: string) => AuditEntry;
  summarizeFindings: (findings: unknown) => Array<{ rule: string; severity: string; lens: string }>;
  DEFAULT_PATH: string;
...
interface AuditEntry {
  ...
  findings: Array<{ rule: string; severity: string; lens: string }>;
}
...
function runDetect(args: string[], extraEnv: Record<string, string>): { code: number; stdout: string } {
  let code = 0;
  let stdout = '';
  try {
    stdout = execFileSync('node', [BIN, ...args, '--quiet'], {
      encoding: 'utf8',
      cwd: SC,
...
function main(): void {
  ...
  const first = e.findings[0] as Record<string, unknown>;
  assert(
    Object.keys(first).sort().join(',') === 'lens,rule,severity',
    `a captured finding is exactly {rule,severity,lens}, got keys [${Object.keys(first).join(',')}]`,
  );
  assert((first as { detail?: unknown }).detail === undefined, 'the heavy detail field is NOT persisted');
  assert((first as { selector?: unknown }).selector === undefined, 'the selector field is NOT persisted');
  ...
  const junk = cap.summarizeFindings([null, { rule: 42 }, 'nope', { severity: 'x', lens: 'y' }] as unknown);
  ...
  assert(cap.summarizeFindings('not-an-array' as unknown).length === 0, 'a non-array findings value yields []');
  ...
  // ---------------------------------------------------------------------
  {
    ...
    const l0 = JSON.parse(lines[0]) as AuditEntry;
    const l1 = JSON.parse(lines[1]) as AuditEntry;
  ...
  // ---------------------------------------------------------------------
  {
    ...
    try {
      ...
      if (prev === undefined) delete process.env.SIDECOACH_NO_AUDIT_HISTORY;
      else process.env.SIDECOACH_NO_AUDIT_HISTORY = prev;
    }
    fs.rmSync(dir, { recursive: true, force: true });
  }
  ...
  // ---------------------------------------------------------------------
  {
    const hostileFinding: Record<string, unknown> = { severity: 'blocking', lens: 'static-check' };
    Object.defineProperty(hostileFinding, 'rule', { enumerable: true, get() { throw new Error('boom'); } });
    const hostile = {
      target: 'h.html',
      targetKind: 'file',
      verdict: 'blocked',
      severityCounts: { blocking: Symbol('x') as unknown, warning: 2, info: 0 },
      findings: [hostileFinding],
    };
    let entry: AuditEntry | undefined;
    let threw = false;
    try { entry = cap.buildEntry(hostile, '2026-08-23T00:00:00.000Z'); } catch { threw = true; }
    ...
    const withHostileIter = { target: 'i.html', targetKind: 'file', verdict: 'blocked', severityCounts: { blocking: 1 }, findings: hostileIter as unknown };
    let iterThrew = false;
    let iterEntry: AuditEntry | undefined;
    try { iterEntry = cap.buildEntry(withHostileIter, '2026-08-23T00:00:00.000Z'); } catch { iterThrew = true; }
  ...
  // ---------------------------------------------------------------------
  {
    ...
    assert(
      entry.findings.some((f) => f.rule.includes('gradient-text') && f.severity === 'blocking'),
      `the fired gradient-text rule is captured at blocking, got [${entry.findings.map((f) => `${f.rule}/${f.severity}`).join(', ')}]`,
    );
  ...
  // ---------------------------------------------------------------------
  {
    ...
    const j = JSON.parse(forced.stdout) as { verdict: string; lenses: unknown };
