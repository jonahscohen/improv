...
function blocks(report: string): string[] {
  return report.split(/\n(?=#### )/).filter((b) => b.startsWith('#### '));
}
function nonEmptyLines(s: string): string[] { return s.split('\n').map((l) => l).filter((l) => l.trim().length > 0); }
...
function assertReportShape(report: string, label: string): void {
  ...
  bs.forEach((block, i) => {
    ...
    const divIdx = bl.indexOf(divider as string);
...
if (failures > 0) { console.error('executive-report: ' + failures + ' failure(s)'); process.exit(1); }
