...
function run() {
  ...
  for (const cat of ['clean', 'findings'] as const) {
    const res = getValidatorRegistration('polish-standard')!.validateProduct!({ projectPath: path.resolve(SC, `fixtures/polish-standard/${cat}`) });
  ...
  const apClean = getValidatorRegistration('anti-pattern')!.validateProduct!({ projectPath: path.resolve(SC, 'fixtures/anti-pattern/clean') });
