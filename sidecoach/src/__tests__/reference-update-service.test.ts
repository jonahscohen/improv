...
import {
  ...
  mergeBundle,
  validateBundle,
} from '../reference-update-service';

function fail(msg: string): never {
  console.error(`FAIL: ${msg}`);
  process.exit(1);
}
function ok(cond: any, msg: string): void {
  try {
    assert.ok(cond);
  } catch {
    fail(msg);
...
function eq(a: any, b: any, msg: string): void {
  if (a !== b) fail(`${msg} (got ${JSON.stringify(a)}, expected ${JSON.stringify(b)})`);
}

// The two required top-level keys per system (must match the service's validation).
const REQ: Record<ReferenceSystemName, [string, string]> = {
...
function makeBundle(system: ReferenceSystemName, version: string, marker = 'base'): any {
  const b: any = {
  ...
  for (const key of REQ[system]) b[key] = { _marker: marker };
  return b;
}

function writeBundle(dir: string, system: ReferenceSystemName, bundle: any): void {
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, `${system}.json`), JSON.stringify(bundle, null, 2) + '\n');
}

function writeUpstream(dir: string, version = '1.0.0'): void {
  for (const system of REFERENCE_SYSTEMS) writeBundle(dir, system, makeBundle(system, version));
}

function plantCapture(capturesDir: string, slug: string, title: string): void {
...
function makeWorkspace(): { root: string; upstream: string; local: string; captures: string; designMd: string } {
...
(function testHelpers() {
  ok(validateBundle(makeBundle('fontshare', '1.0.0'), 'fontshare'), 'valid bundle should validate');
  const missing = makeBundle('fontshare', '1.0.0');
  delete missing.useCases;
  ok(!validateBundle(missing, 'fontshare'), 'missing required key should fail validation');
  const noMeta = makeBundle('fontshare', '1.0.0');
  delete noMeta.metadata;
  ok(!validateBundle(noMeta, 'fontshare'), 'missing metadata should fail validation');
  ...
  const { merged, mergedCount } = mergeBundle('design-references', upstream, existingLocal, catalog);
  ...
  const mm = mergeBundle('motion-reference', makeBundle('motion-reference', '2.0.0'), mLocal, catalog);
...
(function testCheckEmpty() {
  ...
  const svc = new ReferenceUpdateService({ upstreamDir: w.upstream, localDir: w.local, capturesDir: w.captures, designMdPath: w.designMd });
