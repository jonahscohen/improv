...
function run() {
  ...
  if (a11y.browserRuleIds.includes('a11y.color-contrast')) throw new Error('contrast rule must NOT be a browser rule after Stage 6 migration');
  if (!a11y.renderedRuleIds.includes('a11y.color-contrast')) throw new Error('contrast rule must be generated as a rendered rule');
