...
async function testBlock3() {
  ...
  try {
    ...
    const handler = handlers.get(targetFlowId) as { execute: (...args: any[]) => any } | undefined;
    ...
    if (!handler) {
      ...
      const fromInjection = errored.filter(
        r => (r.message || '').includes(INJECTED) || (r.error || '').includes(INJECTED)
      );
      results.push({
