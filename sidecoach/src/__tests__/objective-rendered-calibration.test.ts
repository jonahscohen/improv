...
import { analyzeHtmlOnBrowser, type ObjectiveRule } from '../validators/objective-rendered-scanner';

// Grows as each step lands: S1 broken-image, S2 skipped-heading, S3 low-contrast+gray-on-color, S4 justified-text.
const IMPLEMENTED_RULES: ObjectiveRule[] = ['broken-image', 'skipped-heading', 'low-contrast', 'gray-on-color', 'justified-text'];

interface Fixture { name: string; html: string; expect: ObjectiveRule | null; note?: string; }
const doc = (head: string, body: string) => `<!doctype html><html><head><meta charset="utf-8">${head}</head><body>${body}</body></html>`;
...
async function run(): Promise<void> {
  const impl = new Set<ObjectiveRule>(IMPLEMENTED_RULES);
  const browser = await chromium.launch({ headless: true });
  const failures: string[] = [];
  let asserted = 0, pending = 0;
  try {
    for (const f of FIXTURES) {
      const findings = await analyzeHtmlOnBrowser(browser, f.html, 20000);
      ...
      if (f.expect === null) {
        ...
        asserted++;
        if (firedImplemented.length) failures.push(`${f.name}: expected CLEAN but fired [${firedImplemented.join(',')}]${f.note ? ' (' + f.note + ')' : ''}`);
      } else if (impl.has(f.expect)) {
        asserted++;
        if (!findings.some((x) => x.rule === f.expect)) failures.push(`${f.name}: expected ${f.expect} not detected${f.note ? ' (' + f.note + ')' : ''}`);
      } else {
  ...
  if (failures.length) throw new Error(`objective-rendered calibration FAILED (${failures.length}):\n  ${failures.join('\n  ')}`);
