...
import { resolveAuditTarget } from '../audit-rendered';
import { generateBuildReport } from '../build-report-aggregator';
import { countMetricStatus } from '../flow-memory-schema';
...
const bcheck = (label: string, cond: boolean) => {
  browserAssertionsRun++;
  check(label, cond);
};
const bskip = (group: string, count: number, reason: string) => {
...
const stripAnsi = (s: unknown) => String(s ?? '').replace(/\x1b\[[0-9;]*m/g, '');
...
async function targetResolution() {
  const file = resolveAuditTarget(path.join(FIX, 'clean.html'));
  ...
  const dir = resolveAuditTarget(path.join(FIX, 'site-with-index'));
  ...
  const noIndex = resolveAuditTarget(path.join(FIX, 'dir-without-index'));
  ...
  const missing = resolveAuditTarget(path.join(FIX, 'nope-does-not-exist.html'));
  ...
  const source = resolveAuditTarget(path.join(REPO, 'src/audit-rendered.ts'));
  ...
  const bare = resolveAuditTarget('SomeComponentName');
  ...
  const url = resolveAuditTarget('http://localhost:4830/page');
  ...
  const bareHost = resolveAuditTarget('localhost:3000');
  ...
  for (const r of [noIndex, missing, source, bare]) {
    check(`resolve: unrenderable "${(r as any).kind}" carries a remedy`, !r.renderable && r.remedy.length > 10);
  }
}
...
async function selfCheckSuppression() {
  const withSelfCheck = (measures: string | undefined): FlowExecutionResult => ({
  ...
  } as unknown as FlowExecutionResult);

  const suppressed = generateBuildReport({ source: 'flow-results', flowResults: [withSelfCheck('flow-output')] });
  ...
  const artifact = generateBuildReport({ source: 'flow-results', flowResults: [withSelfCheck('artifact')] });
  ...
  const unset = generateBuildReport({ source: 'flow-results', flowResults: [withSelfCheck(undefined)] });
...
async function countMetrics() {
  check('count-metric: zero problems -> pass (no finding)', countMetricStatus(0) === 'pass');
  check('count-metric: zero problems -> pass even when severity is fail', countMetricStatus(0, 'fail') === 'pass');
  check('count-metric: one problem -> warning', countMetricStatus(1) === 'warning');
  check('count-metric: many problems -> warning', countMetricStatus(9) === 'warning');
  check('count-metric: one problem at fail severity -> fail', countMetricStatus(1, 'fail') === 'fail');
  ...
  const withMetric = (value: number, status: string): FlowExecutionResult => ({
    flowId: 'flowJ_tactical_polish',
    flowName: 'tactical polish',
    status: 'success',
    message: 'ok',
  ...
  const zero = generateBuildReport({ source: 'flow-results', flowResults: [withMetric(0, countMetricStatus(0))] });
  check('count-metric: a zero-violation run emits NO violation-count finding', !zero.findings.some((f) => f.rule === 'violation-count'));
  const nine = generateBuildReport({ source: 'flow-results', flowResults: [withMetric(9, countMetricStatus(9))] });
...
async function acceptanceBar() {
  ...
  const audit = async (target: string) =>
    engine.process(`/sidecoach audit ${target}`, { projectPath: REPO } as any);
  ...
  const ca0 = (clean as any).audit || {};
  const launchFailed = ca0.rendered !== true
    && (ca0.unavailableReasons || []).some((r: string) => /launch|executable|Permission denied|browserType|spawn|ENOENT/i.test(r));
  if (launchFailed) {
  ...
  const ca = (clean as any).audit || {};
  const ba = (broken as any).audit || {};
  const ea = (empty as any).audit || {};
  const da = (dir as any).audit || {};
  ...
  const rank = (g: string) => 'ABCDF'.indexOf(g);
  ...
  const rules = new Set<string>((ba.byRule || []).map((r: { rule: string }) => r.rule));
  ...
  bcheck('ground truth: the clean page reports NONE of them', !['skipped-heading', 'broken-image', 'low-contrast'].some((r) => (ca.byRule || []).some((x: { rule: string }) => x.rule === r)));
  ...
  for (const [name, res] of [['clean', clean], ['catastrophic', broken]] as const) {
  ...
  bcheck('acceptance: a 0-byte file is NOT success', (empty as any).success === false);
  bcheck('acceptance: a 0-byte file gets NO build report', !(empty as any).buildReport);

  // 7. The proof the original measurement used: the two payloads are not the same bytes.
  const strip = (r: unknown) => JSON.stringify(r)
...
async function constantsGoneFromChain() {
  ...
  const engine = createExecutionEngine();
  const res: any = await engine.process('/sidecoach audit', { projectPath: REPO } as any);
  const flowI = (res.flowResults || []).find((f: { flowId: string }) => f.flowId === 'flowI_accessibility');
  const flowK = (res.flowResults || []).find((f: { flowId: string }) => f.flowId === 'flowK_multi_lens_audit');
  check(
    `chain: flowK_multi_lens_audit actually SUCCEEDED (status=${flowK?.status}) - it is the flow the performance validator attaches to`,
    flowK?.status === 'success',
  );
  ...
  check(
    `chain: flowI_accessibility actually SUCCEEDED (status=${flowI?.status}) - else the assertions below prove nothing`,
    flowI?.status === 'success',
  );
  check('chain: a build report was produced (else there are no findings to inspect)', !!res.buildReport);

  const findings = (res.buildReport?.findings || []) as { rule: string }[];
  ...
  check('chain: no domain grade comes from a guidance-only performance validator', !(res.buildReport?.domainGrades || []).some((g: { domain: string }) => g.domain === 'performance'));
  ...
  const messages = (res.flowResults || []).map((f: { message?: string }) => f.message || '').join('\n');
  ...
  const factories: [string, () => { domain: string; measures?: string }][] = [
    ['accessibility', createAccessibilityValidator],
    ['performance', createPerformanceValidator],
    ['design_system', createDesignSystemValidator],
    ['semantic', createSemanticValidator],
    ['content_quality', createContentQualityValidator],
  ];
  ...
  const vrs = (res.flowResults || []).flatMap((f: { validationResults?: unknown[] }) => f.validationResults || []) as { domain: string; measures?: string }[];
...
async function nonPageTargetsKeepTheirChain() {
  ...
  for (const [label, target] of [
    ...
    const res: any = await engine.process(`/sidecoach audit ${target}`, { projectPath: REPO } as any);
...
async function brokenImageShapes() {
  ...
  const engine = createExecutionEngine();
  const audit = async (t: string) => engine.process(`/sidecoach audit ${t}`, { projectPath: REPO } as any);

  const emptySrc: any = await audit(path.join(FIX, 'broken-image-empty-src.html'));
  const missingFile: any = await audit(path.join(FIX, 'broken-image-missing-file.html'));
  ...
  const fired = (res: any) => (res.audit?.byRule || []).some((r: { rule: string }) => r.rule === 'broken-image');
...
async function fileUrlTargets() {
  ...
  const ok = resolveAuditTarget(cleanFileUrl);
  ...
  const src = resolveAuditTarget(`file://${path.join(REPO, 'src/audit-rendered.ts')}`);
  check('file-url: a source file URL is UNRENDERABLE (was renderable)', src.renderable === false);

  const missing = resolveAuditTarget(`file://${path.join(FIX, 'no-such-page.html')}`);
  check('file-url: a missing file URL is UNRENDERABLE', missing.renderable === false);

  const dirUrl = resolveAuditTarget(`file://${path.join(FIX, 'dir-without-index')}`);
  check('file-url: a directory URL with no entry document is UNRENDERABLE', dirUrl.renderable === false);

  const malformed = resolveAuditTarget('file://not a valid url at all');
...
async function panelNeverGradesAnUnrenderedTarget() {
  ...
  const rendered: any = await engine.process(`/sidecoach audit ${path.join(FIX, 'catastrophic.html')}`, { projectPath: REPO } as any);
  if (rendered.audit?.rendered === true) {
    ...
    const reasons: string[] = rendered.audit?.unavailableReasons || [];
    const launchFailure = reasons.some((r) => /launch|executable|Permission denied|browserType|spawn|ENOENT/i.test(r));
    if (!launchFailure) {
      throw new Error(
        `FAIL: the rendered-panel contrast case did not render, and NOT because the browser was missing. ` +
...
async function nonAuditVerbsFailClosedOnPageTargets() {
  ...
  for (const verb of ['critique', 'polish'] as const) {
    for (const [label, target] of [
      ...
      const res: any = await engine.process(`/sidecoach ${verb} ${target}`, { projectPath: REPO } as any);
  ...
  const cleanC: any = await engine.process(`/sidecoach critique ${path.join(FIX, 'clean.html')}`, { projectPath: REPO } as any);
  const cataC: any = await engine.process(`/sidecoach critique ${path.join(FIX, 'catastrophic.html')}`, { projectPath: REPO } as any);
  ...
  for (const [label, utterance] of [
    ...
    const res: any = await engine.process(utterance, { projectPath: REPO } as any);
  ...
  for (const [label, target] of [
    ...
    const res: any = await engine.process(`/sidecoach critique ${target}`, { projectPath: REPO } as any);
  ...
  const urlRes: any = await engine.process('/sidecoach critique http://localhost:4830/page', { projectPath: REPO } as any);
  ...
  for (const ext of ['shtml', 'xht', 'HTML', 'Htm'] as const) {
    const res: any = await engine.process(`/sidecoach critique some/page.${ext}`, { projectPath: REPO } as any);
...
async function everyUnmeasuredPathIsHonest() {
  ...
  for (const [label, utterance] of shapes) {
    ...
    const panel = stripAnsi(res.panel);
    check(
      `invariant (${label}): either NO panel, or a panel that SAYS nothing was rendered`,
      panel.length === 0 || /NO PAGE WAS RENDERED/.test(panel),
    );
    check(
      `invariant (${label}): no panel anywhere claims a verdict or grade`,
      !/verdict/i.test(panel) && !/\bgrade\s+[A-F]\b/.test(panel),
    );
  }
}
...
async function executiveReportNeverInventsAPass() {
  ...
  for (const [label, utterance] of [
    ...
    const res: any = await engine.process(utterance, { projectPath: authoringRoot } as any);
  ...
  }
  {
    ...
    try {
      const res: any = await engine.process('/sidecoach document', { projectPath: scratch } as any);
  ...
  try {
    ...
    for (const utterance of ['/sidecoach document', '/sidecoach teach'] as const) {
      const res: any = await engine.process(utterance, { projectPath: siteRoot } as any);
      check(
        `exec-report (${utterance} from a root containing index.html): still claims nothing about a page`,
        res.audit === undefined,
      );
    }
  } finally {
  ...
  check(
    'exec-report: a REAL clean measured run still reports checks passed',
    /Checks passed/i.test(stripAnsi(present.renderExecutiveReport(measuredClean, '/sidecoach polish x'))),
  );
...
async function unrenderedAuditIsNotAPrerequisite() {
  const entry = (over: Partial<FlowHistoryEntry>): FlowHistoryEntry => ({
  ...
  const unrendered = FlowPrerequisiteValidator.canExecute(
    'flowL_design_critique' as any,
    [entry({ rendered: false } as any)],
  );
  ...
  const rendered = FlowPrerequisiteValidator.canExecute(
    'flowL_design_critique' as any,
    [entry({ rendered: true } as any)],
  );
  ...
  const legacy = FlowPrerequisiteValidator.canExecute('flowL_design_critique' as any, [entry({})]);
  ...
  const nUnrendered = FlowPrerequisiteValidator.canExecute(
    'flowN_rapid_iteration_refined' as any,
    [entry({ rendered: false } as any)],
  );
  const nRendered = FlowPrerequisiteValidator.canExecute(
    'flowN_rapid_iteration_refined' as any,
    [entry({ rendered: true } as any)],
  );
  ...
  check(
    'prereq: flowN has no minSuccessfulPrerequisites (the reason it is ungated) - if this fails, flowN gained a gate and the two assertions above must be revisited',
    (FlowPrerequisiteValidator.getDependencies('flowN_rapid_iteration_refined' as any) as any)?.minSuccessfulPrerequisites === undefined,
  );

  // END TO END: the field must actually be PERSISTED, or the check above guards nothing.
  const engine = createExecutionEngine();
  await engine.process('/sidecoach audit PRODUCT.md', { projectPath: REPO } as any);
  const { getFlowHistory } = require('../flow-history');
  const seq = getFlowHistory().getFlowSequence() as FlowHistoryEntry[];
  ...
  check('persist: that entry records rendered:false', audits.some((e) => (e as any).rendered === false));
}
...
async function humanSurfacesFilterSelfChecks() {
  const resultWith = (measures: string | undefined) => ({
    success: true,
    message: 'ok',
...
async function measuresIsSafeByDefault() {
  ...
  const undeclared = generateBuildReport({ source: 'flow-results', flowResults: [withSelfCheck(undefined)] });
...
async function run() {
  ...
  if (accountedFor !== BROWSER_ASSERTION_COUNT) {
    console.error(
      `FAIL: browser-assertion accounting drifted - ran ${browserAssertionsRun} + skipped ` +
  ...
  console.log(
    `flow-target-render browser layer: ${browserAssertionsRun}/${BROWSER_ASSERTION_COUNT} ran, ` +
  ...
  if (browserAssertionsSkipped > 0) {
    ...
    console.log(
      `flow-target-render DEGRADED: ${browserAssertionsSkipped} of ${BROWSER_ASSERTION_COUNT} ` +
