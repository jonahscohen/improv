...
const renderExecutiveReport: (result: any, utterance?: string) => string = present.renderExecutiveReport;
...
if (first) {
  const viaNumber = normalizeCraftKey(first.n);
  ok(viaNumber === first.key, `number ${first.n} normalizes to ${first.key} (got ${viaNumber})`);
  ok(normalizeCraftKey(`polish-standard:${first.n}`) === first.key, 'bare alias normalizes');
  ok(normalizeCraftKey(`polish-standard:rule-${first.n}`) === first.key, 'BuildReport alias normalizes');
  ok(normalizeCraftKey(first.key) === first.key, 'canonical key normalizes to itself');
}
ok(normalizeCraftKey('not-a-rule-at-all') === undefined, 'unknown rule does not normalize');
ok(normalizeCraftKey(null) === undefined, 'null does not normalize');
ok(normalizeCraftKey(9999) === undefined, 'out-of-range rule number does not normalize');
...
const allNotes = [...Object.values(POLISH_CRAFT), ...Object.values(POLISH_FINDING_CRAFT)];
for (const note of allNotes) {
  ...
  const ownWords = new Set(
    `${note.ruleKey} ${note.title}`.toLowerCase().split(/[^a-z]+/).filter(Boolean),
  );
  const novel = note.fix.toLowerCase().split(/[^a-z]+/).filter(Boolean)
    .filter((w) => w.length > 2 && !STOPWORDS.has(w) && !ownWords.has(w));
  ok(novel.length >= 10,
    `${tag}: fix carries content beyond the rule's own vocabulary (${novel.length} novel words)`);
  ok(new Set(novel).size >= 8,
    `${tag}: that content is varied, not one word repeated (${new Set(novel).size} distinct)`);
}
// At least MAX_EXAMPLES notes carry an example, or the example budget can never be spent.
const withExamples = Object.values(POLISH_CRAFT).filter((n) => !!n.example);
ok(withExamples.length >= MAX_EXAMPLES, `at least ${MAX_EXAMPLES} notes carry an example (got ${withExamples.length})`);
...
const selected = selectCraftNotes(failing);
...
const withBlocker = selectCraftNotes([1, 18]); // scale-on-press (major), focus-visible (blocker)
ok(withBlocker[0]?.ruleKey === 'a11y/focus-visible', `blocker sorts before major (got ${withBlocker[0]?.ruleKey})`);
// Duplicates collapse.
ok(selectCraftNotes([1, 1, 'polish-standard:rule-1', 'polish/scale-on-press']).length === 1, 'duplicate rules collapse to one note');
// Unknown rules are dropped, not guessed at.
ok(selectCraftNotes(['who-knows', 4242]).length === 0, 'unknown rules select nothing');
// The cap holds.
...
ok(selectCraftNotes(allRuleNumbers).length === MAX_TAUGHT_NOTES,
  `all 24 failing rules select exactly MAX_TAUGHT_NOTES notes (cap ${MAX_TAUGHT_NOTES}, got ${selectCraftNotes(allRuleNumbers).length})`);
// Ordering is stable across a shuffled input.
const shuffled = [...allRuleNumbers].reverse();
ok(JSON.stringify(selectCraftNotes(shuffled).map((n) => n.ruleKey)) ===
   JSON.stringify(selectCraftNotes(allRuleNumbers).map((n) => n.ruleKey)),
  'selection order does not depend on input order');

// ---- 4. PROPORTIONALITY -----------------------------------------------------------------------
const briefOne = craftBriefLines([1]);
const briefAll = craftBriefLines(allRuleNumbers);
const briefNone = craftBriefLines([]);
ok(briefNone.length === 0, 'a page with no failures gets no brief');
ok(briefOne.length > 0, 'a page with one failure gets a brief');
ok(briefAll.length > briefOne.length * 2,
  `an 8-note brief is materially longer than a 1-note brief (${briefOne.length} vs ${briefAll.length} lines)`);
...
contains(craftBriefLines(allRuleNumbers, { limit: 5 }).join('\n'), 'capped at 5',
  'brief discloses the cap it actually applied');
ok(selectCraftNotes(allRuleNumbers, 5).length === 5, 'an explicit limit binds');
...
const exampleKeys = selectCraftNotes(allRuleNumbers).filter((n) => !!n.example);
const snippetsInBrief = exampleKeys.filter((n) => briefAllText.includes((n.example || '').split('\n')[0])).length;
ok(snippetsInBrief <= MAX_EXAMPLES, `at most ${MAX_EXAMPLES} examples emitted (got ${snippetsInBrief})`);
// A brief teaching only passed rules would be a constant; assert the brief for two DIFFERENT
// failure sets differs.
ok(craftBriefLines([1]).join('\n') !== craftBriefLines([8]).join('\n'),
...
ok(craftNote('anti-patterns:ban-not-a-real-ban') === undefined, 'an unknown ban does not resolve');
...
ok(craftRemediation('low-contrast') === undefined, 'audit rules are not in the craft corpus');
...
(async () => {
  ...
  const res: any = await handler.execute({ projectPath: tmp, flowId: 'flowJ_tactical_polish' } as any);
  ...
  ok(Array.isArray(res.validationResults) && res.validationResults.length >= 3,
    `live result keeps its validationResults (got ${(res.validationResults || []).length})`);
  ok(Array.isArray(res.checklist) && res.checklist.length >= 20,
    `live result keeps its checklist (got ${(res.checklist || []).length})`);
  ...
  const cleanRes: any = await handler.execute({ projectPath: cleanDir, flowId: 'flowJ_tactical_polish' } as any);
  const cleanPayload = (cleanRes.guidance || []).join('\n');
  ok(cleanPayload.length < payload.length,
    `a cleaner page gets a shorter payload (${cleanPayload.length} vs ${payload.length} chars)`);

  fs.rmSync(tmp, { recursive: true, force: true });
  fs.rmSync(cleanDir, { recursive: true, force: true });

  if (failures > 0) { console.error('polish-craft: ' + failures + ' failure(s)'); process.exit(1); }
