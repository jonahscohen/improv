...
interface BinRecord {
  ...
  purpose: string;
  invocation: string;
  flowWiring: string | null;
  flowWired: boolean;
}

function readSurface(): BinRecord[] {
  const stdout = execFileSync(process.execPath, [path.join(SC, 'bin', 'sidecoach.js'), 'list', '--json'], {
    encoding: 'utf8',
    maxBuffer: 8 * 1024 * 1024,
  });
  const parsed = JSON.parse(stdout) as { standaloneBins?: BinRecord[] };
...
function main(): void {
  ...
  assert(
    countRe.test(doc),
    `the skill document must state that ${totalWord} (${total}) self-contained CLIs ship in sidecoach/bin/. It does not, so its count has drifted from \`sidecoach list\`.`,
  );
  ...
  if (wrongCount) {
    ...
    assert(
      stated === totalWord || stated === String(total),
      `the skill document says "${stated}" self-contained CLIs but the surface has ${total} (${totalWord})`,
    );
  }
   ...
   */
  function negatedAt(sentence: string, index: number): boolean {
    return NEGATION.test(sentence.slice(Math.max(0, index - 24), index));
  }

  function affirmsWiring(sentence: string): boolean {
    AFFIRMATIVE.lastIndex = 0;
    let m: RegExpExecArray | null;
    while ((m = AFFIRMATIVE.exec(sentence)) !== null) {
      if (!negatedAt(sentence, m.index)) return true;
    }
    return false;
  }
  ...
  for (const b of wired) {
    ...
    assert(
      mentioning.some((s) => affirmsWiring(s)),
      `${b.bin} is auto-run by a flow (${b.flowWiring}) and no sentence in the skill document affirmatively says so. Sentences mentioning it: ${mentioning
  ...
  if (manualClaim) {
    ...
    assert(
      stated === manualWord || stated === String(manualCount),
      `the skill document says ${stated} tools are not auto-run by a flow, but ${manualWord} (${manualCount}) of ${total} are manual: ${bins
  ...
  for (const b of wired) {
    for (const s of sentences) {
      if (!s.includes(b.bin)) continue;
      AFFIRMATIVE.lastIndex = 0;
      let m: RegExpExecArray | null;
      while ((m = AFFIRMATIVE.exec(s)) !== null) {
        assert(
          !negatedAt(s, m.index),
          `a sentence names ${b.bin} and DENIES its flow wiring, which is false (${b.flowWiring}): ${s.trim().slice(0, 160)}`,
        );
      }
      assert(
        !/\binvoked (?:directly|manually|by hand)\b/i.test(s),
        `a sentence names ${b.bin} and says it is invoked directly, but a flow auto-runs it (${b.flowWiring}): ${s.trim().slice(0, 160)}`,
      );
    }
  }
  ...
  assert(
    /\bimage\b/i.test(description![1]),
    'the frontmatter description must name the image capability, or a request about generating an image will never select this skill',
  );

  console.log(
    `skill-surface-parity: OK (${total} bins named, count word "${totalWord}" stated, ${wired.length} flow-wired bin(s) declared as such, no false manual claim, description names the image capability)`,
  );
}
