...
function inconclusiveResult(): ProductValidationResult {
...
function erroredResult(): ProductValidationResult {
...
async function mustReject(fn: () => Promise<any>, re: RegExp): Promise<void> {
  let threw = false;
  ...
  if (!threw) throw new Error('expected a rejection matching ' + re);
}
...
async function run() {
  ...
  // --- CAP: maxIterations=1, one failing pass -> capped, terminal-pending ---
  {
    ...
    const d = deps(proj, () => fail('polish.no-transition-all'));
    const s = await startLane('lane_converge', 'project', { projectPath: proj }, 'req-cap', d);
    const cp0 = d.store.read(s.checkpointId);
    cp0.convergence!.limits.maxIterations = 1;   // cap after the first pass
    d.store.write(cp0);
    const { res } = await onePass(proj, d, s.checkpointId, s.revision, 0);
    if (!res.convergence || res.convergence.outcome !== 'capped') throw new Error('capped after 1 pass at maxIterations=1, got ' + res.convergence?.outcome);
    if (res.currentVerb !== 'critique') throw new Error('cap stays on the terminal boundary (critique), got ' + res.currentVerb);
    if (res.iteration !== 0) throw new Error('cap does not advance the iteration');
    const cp = d.store.read(s.checkpointId);
    if (cp.cursor !== 2 || cp.iteration !== 0 || cp.lease !== null) throw new Error('cap persisted terminal-pending: cursor 2, iteration 0, lease null');
    await mustReject(() => advanceLane(proj, s.checkpointId, { action: 'complete', report: rep('critique', 0), expectedRevision: res.revision }, d), /explicit retry or resume/);
    await mustReject(() => advanceLane(proj, s.checkpointId, { action: 'skip', reason: 'ordinary skip', expectedRevision: res.revision }, d), /explicit retry or resume/);
  }
  ...
  // --- RETRY: from capped, retry reruns the SAME boundary; clean validators converge ---
  {
    ...
    const s = await startLane('lane_converge', 'project', { projectPath: proj }, 'req-retry', d);
    const cp0 = d.store.read(s.checkpointId);
    cp0.convergence!.limits.maxIterations = 1;
    d.store.write(cp0);
    const { res } = await onePass(proj, d, s.checkpointId, s.revision, 0);
    if (res.convergence!.outcome !== 'capped') throw new Error('precondition: capped, got ' + res.convergence!.outcome);
    polish = () => clean();   // fix the product; retry must reach clean
    const retried = await advanceLane(proj, s.checkpointId, { action: 'retry', expectedRevision: res.revision }, d);
    if (retried.outcome !== 'converged') throw new Error('retry with clean validators converges, got ' + retried.outcome);
    if (!retried.convergence || retried.convergence.outcome !== 'converged') throw new Error('retry converges (convergence surface)');
    if (retried.lifecycle !== 'closed') throw new Error('retry-converged closes the lane');
    if (retried.iteration !== 0) throw new Error('retry stays on the same iteration (no advance)');
  }
  ...
  // --- RESUME: from stalled, resume begins the pending next pass ---
  {
    ...
    const d = deps(proj, () => fail('polish.no-transition-all'));
    const s = await startLane('lane_converge', 'project', { projectPath: proj }, 'req-resume', d);
    let rev = s.revision;
    let last: any;
    for (let i = 0; i < 3; i++) { const { res } = await onePass(proj, d, s.checkpointId, rev, i); rev = res.revision; last = res; }
    ...
    const resumed = await advanceLane(proj, s.checkpointId, { action: 'resume', expectedRevision: last.revision }, d);
    if (resumed.currentVerb !== 'polish') throw new Error('resume serves polish first, got ' + resumed.currentVerb);
    if (resumed.iteration !== pendingIter) throw new Error('resume moves to the pending iteration ' + pendingIter + ', got ' + resumed.iteration);
    const cp = d.store.read(s.checkpointId);
    if (cp.convergence!.outcome !== 'running') throw new Error('resume sets convergence outcome back to running');
    if (cp.cursor !== 0) throw new Error('resume resets cursor to 0');
    if (cp.convergence!.consecutiveNoProgress !== 0) throw new Error('resume resets consecutiveNoProgress');
  }
  ...
  // --- FRESH STORE/PROCESS CONTINUATION: a new store + deps continues to convergence ---
  {
    ...
    const s = await startLane('lane_converge', 'project', { projectPath: proj }, 'req-fresh', d1);
    const { res } = await onePass(proj, d1, s.checkpointId, s.revision, 0);
    if (res.convergence!.outcome !== 'running' || res.iteration !== 1) throw new Error('precondition: running with iteration 1 pending');
    ...
    if (cont.res.outcome !== 'converged') throw new Error('a fresh store/process continues iteration 1 to convergence, got ' + cont.res.outcome);
  }
  ...
  // --- INCONCLUSIVE/ERROR: neither converges; both iterations + signatures persist, lease cleared ---
  {
    ...
    const s = await startLane('lane_converge', 'project', { projectPath: proj }, 'req-incerr', d);
    const p0 = await onePass(proj, d, s.checkpointId, s.revision, 0);
    if (p0.res.convergence!.outcome === 'converged') throw new Error('an inconclusive boundary cannot converge');
    if (p0.res.convergence!.outcome !== 'running') throw new Error('inconclusive stays resumable (running)');
    if (d.store.read(s.checkpointId).lease !== null) throw new Error('lease cleared after the inconclusive boundary');
    const p1 = await onePass(proj, d, s.checkpointId, p0.res.revision, 1);
    if (p1.res.convergence!.outcome === 'converged') throw new Error('a typed-error boundary cannot converge');
    const cp = d.store.read(s.checkpointId);
    if (cp.convergence!.history.length !== 2 || cp.convergence!.signatures.length !== 2) throw new Error('both iterations and signatures persist');
    if (cp.lease !== null) throw new Error('lease cleared after the error boundary');
  }
  ...
  // --- skip of the FINAL step still runs the boundary gate (cannot bypass) ---
  {
    ...
    const d: LaneRunnerDeps = { ...base, runValidator: async (_vId) => { boundaryRan++; return clean(); } };
