...
const rulesOf = (s: TypographyExtremesScore): Set<string> => new Set(typographyExtremesFindingsFromScore(s).map((f: SubjectiveFinding) => f.rule));
...
check(!rulesOf(baseScore({ allCapsShare: ALLCAPS_SHARE_MIN - 0.01 })).has('all-caps-body'), 'all-caps must NOT fire just below the share floor');
check(!rulesOf(baseScore({ allCapsShare: 1, contentChars: TYPO_MIN_CONTENT_CHARS - 1 })).has('all-caps-body'), 'all-caps must NOT fire below the min-content guard');

// oversized-h1: fires at exactly H1_VW_RATIO; NOT guarded by content chars (an h1 on a sparse page still fires).
check(rulesOf(baseScore({ h1Ratio: H1_VW_RATIO, largestH1Px: H1_VW_RATIO * 1280 })).has('oversized-h1'), `h1 must fire at ratio=${H1_VW_RATIO}`);
check(!rulesOf(baseScore({ h1Ratio: H1_VW_RATIO - 0.005, largestH1Px: 130 })).has('oversized-h1'), 'h1 must NOT fire just below the ratio');
check(rulesOf(baseScore({ h1Ratio: H1_VW_RATIO, largestH1Px: 141, contentChars: 10 })).has('oversized-h1'), 'h1 must fire regardless of content-char count (no proportion guard)');

// sub-11px-ui: fires at exactly SUB11_MIN_CHARS; NOT guarded by content-char proportion.
check(rulesOf(baseScore({ sub11Chars: SUB11_MIN_CHARS, sub11MinPx: 9 })).has('sub-11px-ui'), `sub-11px must fire at ${SUB11_MIN_CHARS} chars`);
check(!rulesOf(baseScore({ sub11Chars: SUB11_MIN_CHARS - 1, sub11MinPx: 9 })).has('sub-11px-ui'), 'sub-11px must NOT fire just below the char floor');
check(rulesOf(baseScore({ sub11Chars: SUB11_MIN_CHARS, sub11MinPx: 8, contentChars: 5 })).has('sub-11px-ui'), 'sub-11px must fire regardless of content-char count');
...
const FIX = path.resolve(__dirname, '..', '..', 'eval', 'fixtures', 'typography-extremes');
...
async function run(): Promise<void> {
  ...
  if (failures.length) throw new Error(`typography-extremes FAILED (${failures.length}):\n  ${failures.join('\n  ')}`);
