...
const baseDeps = (store: LaneCheckpointStore, id: string, runValidator: LaneRunnerDeps['runValidator']): LaneRunnerDeps => ({
  store,
...
async function driveUntilValidator(proj: string, first: LaneStepResult, deps: LaneRunnerDeps, ran: () => boolean): Promise<void> {
  let step = first;
  let guard = 0;
  while (step.lifecycle === 'in_progress' && !ran() && guard++ < 50) {
    const verb = step.currentVerb;
    if (!verb) break;
    const rep: StepReport = { stepId: verb, iteration: step.iteration, reportId: `ru:${verb}:${guard}`, verb, summary: 'done', evidence: [{ kind: 'note', detail: 'x' }] };
...
async function run() {
  ...
  // 1. WITH renderUrl: threads onto the checkpoint (persisted) and into the validator ctx.
  {
    ...
    const deps = baseDeps(new LaneCheckpointStore(proj), 'lane-ru1', async (_v, ctx) => { seen.push(ctx.renderUrl); return cleanResult(); });
    const first = await startLane('lane_build', 'a free-text target', { projectPath: proj }, 'req-ru-1', deps, url);
    await driveUntilValidator(proj, first, deps, () => seen.length > 0);
    if (seen.length === 0) throw new Error('lane_build must run a validator (craft binds polish-standard)');
    if (!seen.every((u) => u === url)) throw new Error(`validator ctx.renderUrl must equal the started renderUrl, got ${JSON.stringify(seen)}`);
    const reloaded = new LaneCheckpointStore(proj).read('lane-ru1');
    if (reloaded.renderUrl !== url) throw new Error('renderUrl must persist on the checkpoint across reload');
  }
  ...
  // 2. NO renderUrl + free-text target: ctx.renderUrl undefined (browser rules dormant).
  {
    const proj = mkproj();
    const seen: Array<string | undefined> = [];
    const deps = baseDeps(new LaneCheckpointStore(proj), 'lane-ru2', async (_v, ctx) => { seen.push(ctx.renderUrl); return cleanResult(); });
    const first = await startLane('lane_build', 'just a free-text target, not a url', { projectPath: proj }, 'req-ru-2', deps);
    await driveUntilValidator(proj, first, deps, () => seen.length > 0);
    if (seen.length === 0) throw new Error('lane_build must run a validator (dormant case)');
    if (!seen.every((u) => u === undefined)) throw new Error(`a free-text target must yield undefined renderUrl, got ${JSON.stringify(seen)}`);
    if (new LaneCheckpointStore(proj).read('lane-ru2').renderUrl !== undefined) throw new Error('no renderUrl -> checkpoint renderUrl undefined');
  }

  // 3. An invalid (non-URL) renderUrl is rejected at start and stored undefined.
  {
    const proj = mkproj();
    const deps = baseDeps(new LaneCheckpointStore(proj), 'lane-ru3', async () => cleanResult());
    await startLane('lane_build', 't', { projectPath: proj }, 'req-ru-3', deps, 'not a url at all');
    if (new LaneCheckpointStore(proj).read('lane-ru3').renderUrl !== undefined) throw new Error('a non-URL renderUrl must be stored as undefined');
  }
  ...
  const realRun = async (id: string, renderUrl?: string) => {
    const proj = mkproj();
    const captured: ProductValidationResult[] = [];
    const deps = baseDeps(new LaneCheckpointStore(proj), id, async (vId, ctx, signal) => {
      const res = await makeProductValidator(vId)(ctx, signal);
      if (vId === 'polish-standard') captured.push(res);
      return res;
    });
    const first = await startLane('lane_build', 'a target', { projectPath: proj }, `req-${id}`, deps, renderUrl);
