...
// --- P4a-2: partial-static registry slice (exact 30-row assertion) ---
{
  ...
  if (RULES.length !== 59) throw new Error(`expected 59 canonical rules, got ${RULES.length}`); // 58 -> 59: +polish.marketing-buzzword (Stage 5a rendered-scan, 2026-06-25); 52 -> 58: +6 page-quality Tier-2 keepers (Stage 2, 2026-06-25); 41->52 = 11 more forms; 36->41 = first 5 forms; 31->36 = Stage 1 rendered
  const owners = (id: string) => RULES.filter((r) => r.ownerValidatorId === id);
  if (owners('polish-standard').length !== 23) throw new Error('polish-standard must own 23 rules');
