...
import {
  ...
  IMAGE_ROLES,
  IMAGE_SURFACES,
  validateImageComposition,
  validateImageCompositionCatalog,
  inkZoneToPixels,
  compositionById,
...
function tokens() {
  return parseDesignMd(DESIGN_MD);
}
...
function testCatalog(): void {
  const { errors, warnings } = validateImageCompositionCatalog();
  ...
  assert(IMAGE_COMPOSITION_CATALOG.length >= 12, 'the catalog must carry at least 12 stagings');

  for (const surface of IMAGE_SURFACES) {
    assert(IMAGE_COMPOSITION_CATALOG.some((c) => c.surface === surface), `a staging must serve ${surface}`);
  }
  for (const role of IMAGE_ROLES) {
    assert(IMAGE_COMPOSITION_CATALOG.some((c) => c.roles.includes(role)), `a staging must be able to hold a ${role}`);
  }
  ...
  const base = compositionById('quiet-left-third')!;
  const withColour: ImageComposition = {
    ...base,
    id: 'leaky-colour',
    label: 'Leaky colour',
  ...
  const colourErrors = validateImageComposition(withColour);
  assert(
    colourErrors.some((e) => /palette term/.test(e)),
    `a staging naming a colour must be rejected, got: ${colourErrors.join(' | ')}`,
  );

  const withType: ImageComposition = {
    ...base,
    id: 'leaky-type',
    label: 'Leaky type',
  ...
  assert(
    validateImageComposition(withType).some((e) => /type term/.test(e)),
    'a staging naming a typeface must be rejected',
  );

  // An ink zone must be big enough to measure contrast in; a sliver is not a measurement.
  const slivered: ImageComposition = { ...base, id: 'sliver', label: 'Sliver', inkZone: [0, 0, 0.01, 0.01] };
  assert(validateImageComposition(slivered).some((e) => /too small to measure contrast/.test(e)), 'a degenerate ink zone must be rejected');

  const outside: ImageComposition = { ...base, id: 'outside', label: 'Outside', inkZone: [0.8, 0.8, 0.5, 0.5] };
  assert(validateImageComposition(outside).some((e) => /outside the frame/.test(e)), 'an ink zone running off the frame must be rejected');

  // Pixel conversion clamps rather than producing a region that starts outside the bytes.
  const px = inkZoneToPixels([0.5, 0.5, 0.5, 0.5], 1024, 512);
  assert(px !== null && px.x + px.w <= 1024 && px.y + px.h <= 512, 'a converted ink zone must stay inside the image');
  assert(inkZoneToPixels([0, 0, 0.0001, 0.0001], 10, 10) === null, 'an unmeasurable region must be null rather than degenerate');
}
...
function testWeakBrief(): void {
  assert(!briefWantsRaster('a settings toggle with a disabled state'), 'a brief naming no raster wants no raster');
  assert(compileImageBrief({ text: 'a settings toggle' }) === null, 'a brief naming no raster compiles to nothing at all');
  assert(briefWantsRaster('hero image'), 'a two-word brief naming an image wants a raster');

  const c = compileImageBrief(
    { text: 'hero image' },
    { register: 'product', productName: 'Ledgerline', designTokens: tokens(), antiReferences: ['the usual purple gradient look'] },
  );
  ...
  for (const ban of MEDIUM_BANS) assert(bans.includes(ban), `the medium bans must all be present, missing: ${ban}`);
  ...
  const anon = compileImageBrief({ text: 'hero image' }, { designTokens: tokens() });
  assert(/No legible lettering anywhere/.test(anon!.sections.find((s) => s.name === 'TEXT')!.body), 'with no product name nothing is legible');
  assert(
    !anon!.sections.find((s) => s.name === 'DO NOT')!.body.includes('one permitted string'),
    'with no permitted string the bans must not reference a permission that was never granted',
  );
}
...
function testContractDerivation(): void {
  const c = compileImageBrief({ text: 'a full-bleed hero backdrop' }, { designTokens: tokens(), register: 'product' })!;
  const staging = c.composition;
  const dims = /^(\d+)x(\d+)$/.exec(c.contract.size)!;
  const expected = inkZoneToPixels(staging.inkZone, Number(dims[1]), Number(dims[2]));

  assert(c.contract.inkRegion !== undefined, 'a backdrop over real tokens must carry a contracted ink region');
  assert(
    JSON.stringify(c.contract.inkRegion) === JSON.stringify(expected),
    `THE central property: the contracted region must BE the staging's ink zone in pixels. got ${JSON.stringify(c.contract.inkRegion)} want ${JSON.stringify(expected)}`,
  );
  ...
  assert(
    textSection.includes(`${c.contract.inkRegion!.w} by ${c.contract.inkRegion!.h} pixels`),
    'the prompt must state the region it is told to keep quiet, in the same pixels the check will read',
  );

  // NOTHING IS INVENTED. No text colour in the project means no contracted contrast and an explicit gap.
  const noInk = compileImageBrief({ text: 'a full-bleed hero backdrop' }, {})!;
  ...
  assert(
    noInk.briefStrength.missing.some((m) => /names no text colour/.test(m)),
    'the missing report must name the gap and what it costs',
  );

  // An object role composites over the page, so alpha is required without being asked for.
  const obj = compileImageBrief({ text: 'a transparent cutout of the product' }, {})!;
...
function testResolution(): void {
  ...
  assert(detectRole('a tiling texture for the dashboard background')!.role === 'texture', 'texture outranks backdrop when both words appear');
  assert(detectRole('a full-bleed background behind the headline')!.role === 'backdrop', 'backdrop still resolves when no texture word is present');
  ...
  for (const text of ['hero image', 'a plate', 'a texture', 'a thumbnail', 'an og image', 'a portrait']) {
    const { concept } = resolveConcept({ text }, {});
    assert(concept.id !== MODEL_DEFAULT_ID, `an unspecified world must never resolve to ${MODEL_DEFAULT_ID} (got it for "${text}")`);
  ...
  const matched = resolveConcept({ text: 'hero image' }, { approach: 'restrained editorial' });
  ...
  assert(OPENAI_SIZES.includes(sizeForAspect('16:9', { allowed: OPENAI_SIZES })), 'the OpenAI policy only yields OpenAI geometries');
  assert(sizeForAspect('16:9', { maxEdge: 1024 }) === '1024x576', `a 1024 max-edge 16:9 must be 1024x576, got ${sizeForAspect('16:9', { maxEdge: 1024 })}`);
  assert(sizeForAspect('1:1', { maxEdge: 1024 }) === '1024x1024', 'a square staging at 1024 max edge is 1024x1024');
  assert(JSON.stringify(sizePolicyFor('openai')) === JSON.stringify({ allowed: OPENAI_SIZES }), 'openai gets its three geometries');
  assert(sizePolicyFor('nanobanana').maxEdge === 1024, 'nanobanana is held to a 1024 max edge, its cheapest and only universally supported tier');
  assert(
    JSON.stringify(sizePolicyFor('auto').allowed) === JSON.stringify(['1024x1024']),
    'auto must use the intersection every provider in the chain can serve',
  );

  // Calibration bans are suppressed when the project committed a palette, because the committed palette wins.
  const committed = compileImageBrief({ text: 'hero image' }, { designTokens: tokens() })!;
  const free = compileImageBrief({ text: 'hero image' }, {})!;
  for (const ban of CALIBRATION_BANS) {
...
function testReviewFindings(): void {
  ...
  for (const notARaster of [
    ...
    assert(
      detectRole(notARaster) === null,
      `"${notARaster}" names no raster asset, but role detection returned ${JSON.stringify(detectRole(notARaster))}`,
    );
    assert(compileImageBrief({ text: notARaster }) === null, `"${notARaster}" must compile to nothing`);
  }

  // The false-friend guard must not suppress a REAL request that also mentions a CSS sense.
  const both = detectRole('a hero backdrop plus a background color transition on the nav');
  ...
  assert(detectRole('improve the imagery guidelines') === null, '"imagery" must not match the keyword "image"');
  assert(detectRole('a plate for the launch page') !== null, 'a genuine keyword still matches');
  ...
  const dashed = harvestPaletteFromProse('- **Body text** - #2C2C2C - primary typography\n- **Teal** - #1B4D4D - blocks');
  assert(dashed !== null, 'a dash-separated prose colour line must be harvested');
  assert(flattenPalette(dashed, 10).length === 2, 'both dash-separated colours are harvested');
  const dashedInk = resolveInk(dashed);
  ...
  const nullWanted = assetProductionChecklistItem(null, true);
  ...
  const nullNotWanted = assetProductionChecklistItem(null, false);
...
function testTokenPrecedence(): void {
  ...
  const withParsed = resolveDesignTokens({
    utterance: 'a hero backdrop',
    projectPath: empty,
    projectContext: { design: { parsedTokens: tokens() } } as never,
  });
  assert(withParsed !== null, 'parsed tokens on the context must be used when no file is on disk');
  assert(resolveInk(withParsed)!.hex === '#14181c', 'the ink comes from the context tokens');

  // The other shape flow G already threads around: a bare token object on metadata.
  const withMeta = resolveDesignTokens({
    utterance: 'a hero backdrop',
    projectPath: empty,
    metadata: { designTokens: { colors: { text: { primary: '#101418' } } } },
  } as never);
  assert(withMeta !== null && resolveInk(withMeta)!.hex === '#101418', 'metadata tokens are used when nothing better exists');
  ...
  const fellThrough = resolveDesignTokens({
    utterance: 'a hero backdrop',
    projectPath: projectWithFile,
    projectContext: { design: { parsedTokens: { colors: {} } } } as never,
    metadata: { designTokens: { colors: {} } },
  } as never);
  assert(fellThrough !== null, 'an empty token object must not shadow a real file');
  assert(resolveInk(fellThrough)!.hex === '#14181c', 'the file supplies the ink when the in-memory sources are empty');

  assert(resolveDesignTokens({ utterance: 'x', projectPath: empty }) === null, 'with no tokens anywhere the result is null, not an invention');
}

function testProseHarvest(): void {
  const prose = [
    '## Colors',
    '',
  ...
  const harvested = harvestPaletteFromProse(prose)!;
  assert(harvested !== null, 'a prose colour list must be harvested rather than discarded');
  const flat = flattenPalette(harvested, 10);
  ...
  const ink = resolveInk(harvested);
  assert(ink !== null && ink.hex === '#2c2c2c', `the body-text colour must be found as the ink, got ${JSON.stringify(ink)}`);
  assert(harvestPaletteFromProse('no colours here at all') === null, 'a document with no hex yields null rather than an empty palette');
}
...
function testArgv(): void {
  const c = compileImageBrief({ text: 'a full-bleed hero backdrop' }, { designTokens: tokens() })!;
  const offline = buildImageArgv('/bin/img.js', c, '/out/a.png', '/cache', {});
  ...
  const live = buildImageArgv('/bin/img.js', c, '/out/a.png', '/cache', {
  ...
  const liveNoConsent = buildImageArgv('/bin/img.js', c, '/out/a.png', '/cache', { SIDECOACH_IMAGE_PROVIDER: 'nanobanana' });
  ...
  const obj = compileImageBrief({ text: 'a transparent cutout of the product' }, {})!;
  assert(buildImageArgv('/bin/img.js', obj, '/o.png', '/c', {}).includes('--alpha'), 'a required-alpha contract travels to the bin');
}
...
function testOffering(): void {
  assert(!isOfferableAsset(null), 'a lens that did not run offers nothing');
  for (const status of ['failed', 'unverified', 'needs-consent', 'no-key', 'budget', 'unavailable', 'not-needed'] as const) {
    assert(!isOfferableAsset({ status, path: '/tmp/x.png', detail: 'x' }), `a ${status} outcome is never offered, even with bytes on disk`);
  }
  assert(isOfferableAsset({ status: 'verified', path: '/tmp/x.png', detail: 'x' }), 'only a verified asset is offered');
  ...
  const placeholder = assetProductionChecklistItem({ status: 'failed', synthetic: true, detail: 'contrast 1.89:1' });
  ...
  const real = assetProductionChecklistItem({ status: 'failed', synthetic: false, detail: 'contrast 1.89:1' });
  assert(real.required === true && real.completed === false, 'a failing real render is a hard blocker');
  const notNeeded = assetProductionChecklistItem({ status: 'not-needed', detail: 'no raster named' });
...
async function testFlow(): Promise<void> {
  ...
  const context = (utterance: string): FlowExecutionContext => ({
    utterance,
    projectPath: project,
    projectContext: {
      projectPath: project,
  ...
  const handler = new FlowGComponentImplementationHandler();
  ...
  assert(
    /VERIFIED|FAILED|UNVERIFIED/.test((made.guidance || []).find((g) => g.startsWith('Asset production:'))!),
    'the flow reports a real verdict, never silence',
  );
