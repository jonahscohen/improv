...
export async function decodeViaBrowser(
  buf: Uint8Array,
  format: string,
  opts: { timeoutMs?: number; playwrightPath?: string } = {},
): Promise<RgbaResult> {
  ...
  let chromium: { launch(o?: Record<string, unknown>): Promise<BrowserLike> };
  try {
    const mod = require(opts.playwrightPath || 'playwright') as {
      chromium: { launch(o?: Record<string, unknown>): Promise<BrowserLike> };
    };
    chromium = mod.chromium;
    if (!chromium || typeof chromium.launch !== 'function') throw new Error('playwright exposed no chromium.launch');
  } catch (err) {
    return {
      ok: false,
  ...
  try {
    ...
    const result = (await page.evaluate(async (url: string) => {
      ...
      if (!w || !h) return { error: 'decoded to zero dimensions' };
      ...
      return { w, h, bytes: Array.from(data.data) as number[] };
    }, dataUrl)) as { error?: string; w?: number; h?: number; bytes?: number[] };

    if (result.error) return { ok: false, reason: 'decode-failed', detail: result.error };
...
/** The slice of playwright's surface this module uses, typed locally so playwright is not a load-time import. */
...
export function bundledPlaywrightPath(): string {
