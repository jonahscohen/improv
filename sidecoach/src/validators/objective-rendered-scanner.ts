...
export type ObjectiveScan =
  | { available: true; findings: ObjectiveFinding[] }
...
export function inPageObjective(): ObjectiveFinding[] {
  const findings: { rule: string; severity: string; selector?: string; detail?: string }[] = [];
  const add = (rule: string, el: Element, detail?: string, severity = 'error') => findings.push({ rule, severity, selector: sel(el), detail });
  ...
  const _cctx = (() => { try { const c = document.createElement('canvas'); c.width = c.height = 1; return c.getContext('2d', { willReadFrequently: true }) as CanvasRenderingContext2D | null; } catch { return null; } })();
  const _colorCache = new Map<string, { r: number; g: number; b: number; a: number } | null>();
  function parseColor(s: string): { r: number; g: number; b: number; a: number } | null {
    ...
    if (_cctx) {
      try {
        ...
        _cctx.fillStyle = '#000'; const before = _cctx.fillStyle;
        _cctx.fillStyle = s;
    ...
    if (!out) { // fallback: legacy rgb()/rgba()
      ...
      if (m) { const p = m[1].split(/[,/]/).map((x) => parseFloat(x.trim())); if (![p[0], p[1], p[2]].some((n) => Number.isNaN(n))) out = { r: p[0], g: p[1], b: p[2], a: Number.isNaN(p[3]) ? 1 : p[3] }; }
    }
    _colorCache.set(s, out);
    return out;
  }
  // straight-alpha "over" compositing of src onto opaque dst (CSS Color 4 simple-alpha-compositing)
  function over(src: { r: number; g: number; b: number; a: number }, dst: { r: number; g: number; b: number }) {
  ...
  function ratio(a: { r: number; g: number; b: number }, b: { r: number; g: number; b: number }): number {
    const l1 = lum(a), l2 = lum(b); const hi = Math.max(l1, l2), lo = Math.min(l1, l2); return (hi + 0.05) / (lo + 0.05);
  }
  ...
  function visuallyVisible(el: Element): boolean {
    ...
    if (!r.length) return false;                                   // display:none anywhere -> no box
    const box = (el as HTMLElement).getBoundingClientRect();
    if (box.width < 1 || box.height < 1) return false;             // sub-1px = no rendered box
    ...
    if (box.right <= 0 || box.bottom <= 0) return false;            // off-screen top/left
    ...
    if (parseFloat(cs.textIndent) <= -999) return false;           // text-indent off-screen
    ...
    if (clipM) { const [t, rr, b, l] = [parseFloat(clipM[1]), parseFloat(clipM[2]), parseFloat(clipM[3]), parseFloat(clipM[4])]; if (rr <= l || b <= t) return false; }
    if (/^inset\(\s*(100%|50%)\b/.test(cs.clipPath || '')) return false; // sr-only clip-path inset(100%)
    return true;
  }
  ...
  function ariaVisible(el: Element): boolean {
    ...
    for (let n: Element | null = el; n && n instanceof Element; n = n.parentElement) {
      ...
      if ((n.getAttribute('aria-hidden') || '').trim().toLowerCase() === 'true') return false;
  ...
  function resolvedBg(el: Element): { r: number; g: number; b: number } | null {
    ...
    for (let cur: Element | null = el; cur && cur instanceof Element; cur = cur.parentElement) {
      const cs = getComputedStyle(cur);
      if (parseFloat(cs.opacity) < 1) return null; // group opacity<1 -> indeterminate (axe-core "incomplete")
    ...
    const layers: { r: number; g: number; b: number; a: number }[] = []; // topmost (element) first
    for (let cur: Element | null = el; cur && cur instanceof Element; cur = cur.parentElement) {
      const cs = getComputedStyle(cur);
  ...
  function hasDirectText(el: Element): boolean {
    for (const n of Array.from(el.childNodes)) if (n.nodeType === 3 && (n.textContent || '').trim().length >= 2) return true;
    return false;
  }
  ...
  for (const img of Array.from(document.querySelectorAll('img'))) {
    ...
    if (srcAttr === null || srcAttr.trim() === '') add('broken-image', img, 'missing/empty src');
  }
  ...
  for (const el of all) {
    ...
    let effRole = '';
    for (const t of tokens) { if (KNOWN_ROLES.has(t)) { effRole = t; break; } } // first VALID token
    if (!effRole) effRole = isHTag ? 'heading' : '';                           // no valid explicit role -> implicit
    if (effRole !== 'heading') continue; // presentation/none/non-heading explicit role strips heading semantics
  ...
  for (const el of all) {
    if (!hasDirectText(el) || !visuallyVisible(el)) continue;
    const cs = getComputedStyle(el);
    const fg0 = parseColor(cs.color);
    if (!fg0) continue;
    const bg = resolvedBg(el);
    if (!bg) continue; // indeterminate (background-image)
    ...
    if (cr < threshold) {
      ...
      if (chroma(fg) <= 16 && chroma(bg) >= 24) add('gray-on-color', el, `gray text ${cr.toFixed(2)}:1 on colored bg`);
    }
  }
  ...
  for (const el of all) {
    if (!hasDirectText(el) || !visuallyVisible(el)) continue;
    const cs = getComputedStyle(el);
    if (cs.textAlign !== 'justify' && cs.textAlign !== 'justify-all') continue;
    if (!BLOCKISH.has(cs.display)) continue; // text-align justifies block containers, not inline boxes
    const text = (el.textContent || '').trim();
    const sentences = (text.match(/[.!?](\s|$)/g) || []).length;
    if (sentences >= 2 || text.length >= 120) add('justified-text', el, 'justified block of text (uneven word spacing)', 'warning');
  }

  return findings as ObjectiveFinding[];
}
 ...
 */
// Render transform. Defaults = the deterministic hermetic eval basis (scripts stripped, all external aborted,
// 1280x800). The options exist so a render-DECOUPLING test (and later the S5b genuine real-page render) can
// vary the transform and check whether detections are render-robust (genuine) vs coupled to the exact transform.
...
export async function analyzeHtmlOnBrowser(browser: Browser, html: string, timeoutMs = 30000, render: RenderOpts = {}): Promise<ObjectiveFinding[]> {
