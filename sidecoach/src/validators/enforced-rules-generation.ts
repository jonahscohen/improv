...
export const ENFORCE_GEN_MIN_HELDOUT_POSITIVES = 8;
export const ENFORCE_GEN_MIN_FIRES = 8;
...
export interface PrecisionRecordView {
  precisionDigest?: string;            // the digest STRING the cache stored
  buildStamp?: string;
  pass?: boolean;
  precision?: number | null;
  threshold?: number;
  ...
  minFires?: number;
  // Codex Caveat B: the digest RE-COMPUTED from the cache's OWN fields (spec hash + corpus fingerprint
  // + tp/fp/floor/threshold/pass) by the wrapper - exactly the fields the harness digests. The derive
  // requires THIS to equal the ledger's signed precision_digest, so a cache rewritten to CLAIM a
  // production floor while its signed digest was for a weakened floor fails the recomputation.
  recomputedPrecisionDigest?: string;
}

export interface DeriveInputs {
  records: EnforcedRecord[];
  ledgerByRuleId: Map<string, LedgerEntryView>;      // AUDIT-VERIFIED entries (HMAC already checked upstream)
  precisionByRuleId: Map<string, PrecisionRecordView>;
  currentBuildStamp: string;                          // current interpreter build stamp
  // The PRODUCTION floor the record must have MET (Codex MEDIUM #3): a record measured under a weaker
  // floor (e.g. MIN_FIRES=0, a tiny-denominator P=1.0) must NOT certify. The wrapper resolves these
  // (env overrides honored ONLY under a test root; production = the fixed constants) and passes them.
  requiredThreshold?: number;                         // default ENFORCE_GEN_PRECISION_THRESHOLD (0.90)
  requiredMinHeldoutPositives?: number;               // default ENFORCE_GEN_MIN_HELDOUT_POSITIVES (8)
  requiredMinFires?: number;                          // default ENFORCE_GEN_MIN_FIRES (8)
  threshold?: number;                                 // back-compat alias for requiredThreshold
}
...
function ruleBodyOf(obj: Record<string, unknown>): Record<string, unknown> {
  const r = obj && (obj as { rule?: unknown }).rule;
  return r && typeof r === 'object' ? (r as Record<string, unknown>) : obj;
}
 ...
 */
export function deriveEnforcedRules(inp: DeriveInputs): DeriveResult {
  const reqThreshold = typeof inp.requiredThreshold === 'number' ? inp.requiredThreshold
    : (typeof inp.threshold === 'number' ? inp.threshold : ENFORCE_GEN_PRECISION_THRESHOLD);
  const reqMinPos = typeof inp.requiredMinHeldoutPositives === 'number' ? inp.requiredMinHeldoutPositives : ENFORCE_GEN_MIN_HELDOUT_POSITIVES;
  const reqMinFires = typeof inp.requiredMinFires === 'number' ? inp.requiredMinFires : ENFORCE_GEN_MIN_FIRES;
  const certified: ProductRuleDefinition[] = [];
  const errors: string[] = [];
  ...
  for (const rec of inp.records) {
    ...
    if (!bodyId) { errors.push(`enforced rule "${id}": rule body has no ruleId`); continue; }
    if (bodyId !== id) { errors.push(`enforced rule "${id}": body ruleId "${bodyId}" does not match filename`); continue; }
    if (body.sourceVocabulary !== 'mined-taste') { errors.push(`enforced rule "${id}": sourceVocabulary is "${String(body.sourceVocabulary)}", not "mined-taste"`); continue; }
    if (!body.patternSpec || typeof body.patternSpec !== 'object') { errors.push(`enforced rule "${id}": no patternSpec (nothing to run)`); continue; }
    ...
    if (!entry) { errors.push(`enforced rule "${id}": NO enforcement-ledger entry (not enforced through the gate)`); continue; }
    if (!entry.content_digest) { errors.push(`enforced rule "${id}": ledger entry carries no content_digest (content was never bound - legacy/forged); refusing to certify`); continue; }
    ...
    if (!entry.precision_digest) { errors.push(`enforced rule "${id}": ledger entry carries no precision-digest`); continue; }
    ...
    if (!prec) { errors.push(`enforced rule "${id}": no precision record found (cannot confirm the ledgered precision out of sample)`); continue; }
    if (prec.pass !== true) { errors.push(`enforced rule "${id}": precision record is not a PASS`); continue; }
    if (prec.precisionDigest !== entry.precision_digest) { errors.push(`enforced rule "${id}": precision record digest does not match the ledger's signed precision-digest (record/ledger disagree)`); continue; }
    ...
    if (prec.recomputedPrecisionDigest !== entry.precision_digest) { errors.push(`enforced rule "${id}": precision record fields do not RE-COMPUTE to the ledger's signed precision-digest (the cache was rewritten - its recorded floor/precision disagree with the signed measurement)`); continue; }
    if (prec.buildStamp !== inp.currentBuildStamp) { errors.push(`enforced rule "${id}": precision record was measured against a DIFFERENT interpreter build (${prec.buildStamp || '(none)'} != current ${inp.currentBuildStamp}) - re-enforce against the current interpreter`); continue; }
    if (typeof prec.threshold !== 'number' || prec.threshold < reqThreshold) { errors.push(`enforced rule "${id}": precision record threshold ${prec.threshold ?? '(none)'} is weaker than the production threshold ${reqThreshold}`); continue; }
    if (typeof prec.minHeldoutPositives !== 'number' || prec.minHeldoutPositives < reqMinPos) { errors.push(`enforced rule "${id}": precision record was measured under a weakened held-out-positive floor (${prec.minHeldoutPositives ?? '(none)'} < ${reqMinPos})`); continue; }
    if (typeof prec.minFires !== 'number' || prec.minFires < reqMinFires) { errors.push(`enforced rule "${id}": precision record was measured under a weakened fires floor (${prec.minFires ?? '(none)'} < ${reqMinFires})`); continue; }
 ...
 */
function buildDefinition(id: string, body: Record<string, unknown>): ProductRuleDefinition {
  const s = (k: string, d: string): string => (typeof body[k] === 'string' && body[k] ? (body[k] as string) : d);
  const def: ProductRuleDefinition = {
    ruleId: id,
    sourceRuleAliases: Array.isArray(body.sourceRuleAliases) && (body.sourceRuleAliases as unknown[]).length
      ? (body.sourceRuleAliases as string[])
      : [`mined:${id}`],
    ...
    evidenceRequirements: (Array.isArray(body.evidenceRequirements) && (body.evidenceRequirements as unknown[]).length
      ? (body.evidenceRequirements as ProductRuleDefinition['evidenceRequirements'])
      : ['css-rule']),
    supportedSourceKinds: (Array.isArray(body.supportedSourceKinds)
      ? (body.supportedSourceKinds as ProductRuleDefinition['supportedSourceKinds'])
      : []),
    scope: (typeof body.scope === 'string' ? body.scope : 'file') as ProductRuleDefinition['scope'],
    narrowTargetBehavior: (typeof body.narrowTargetBehavior === 'string' ? body.narrowTargetBehavior : 'evaluate_expanded_context') as ProductRuleDefinition['narrowTargetBehavior'],
    applicability: (body.applicability === 'inconclusive' ? 'inconclusive' : 'not_applicable'),
    patternSpec: body.patternSpec as ProductRuleDefinition['patternSpec'],
  };
  return def;
}
 ...
 */
export function renderEnforcedRulesModule(certified: ProductRuleDefinition[]): string {
