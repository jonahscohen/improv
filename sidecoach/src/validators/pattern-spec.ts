...
export const PATTERN_SPEC_ENGINE = 'static-css-regex' as const;
...
export interface NumericGuard {
  predicateId: string;   // MUST be a key of NUMERIC_PREDICATES; unknown => fail-closed
  threshold: number;
}

export interface PatternSpec {
  specVersion: number;
  engine: string;                                   // 'static-css-regex' | anything-else => inconclusive
  applicability: { anyOf: string[]; scope?: PatternSpecScope };  // regex SOURCES; empty match => notApplicable
  defect: { anyOf: PatternSpecRegex[]; numericGuard?: NumericGuard };
  message: string;                                  // the finding message on a fail
  remediation?: string;
  evidenceScope?: PatternSpecScope;                 // where to scan/locate the defect (defaults to applicability.scope)
}
...
// ---------------------------------------------------------------------------
export const MAX_REGEX_SOURCE_LEN = 1000;   // a taste tell is a short pattern; a huge source is suspicious
export const MAX_SCAN_LEN = 200_000;        // input-length cap on scanned text (bounds linear work)
export const MAX_QUANTIFIERS = 60;          // pathological quantifier count => reject
export const MAX_GROUP_DEPTH = 25;          // pathological nesting depth => reject
export const MAX_DEFECT_MATCHES = 200;      // cap collected matches (bounds memory)
export const DEFAULT_APPLICABILITY_FLAGS = 'i';
 ...
 */
export function validateFlags(flags?: unknown): { flags: string } | { error: string } {
  ...
  if (flags === undefined || flags === '') return { flags: '' };
  if (typeof flags !== 'string') return { error: `regex flags must be a string (got ${flags === null ? 'null' : typeof flags})` };
  const seen = new Set<string>();
  for (const ch of flags) {
    if (!ALLOWED_FLAGS.has(ch)) return { error: `unsupported regex flag '${ch}' (allowed: ${[...ALLOWED_FLAGS].join('')})` };
...
let _re2Cache: RE2Ctor | null | undefined;
type RE2Ctor = new (source: string, flags?: string) => RE2Instance;
interface RE2Instance { test(s: string): boolean; exec(s: string): (RegExpExecArray | null); source: string; flags: string; lastIndex: number; }

function loadRe2(): RE2Ctor | null {
  if (_re2Cache !== undefined) return _re2Cache;
  if (process.env.SIDECOACH_DISABLE_RE2) { _re2Cache = null; return _re2Cache; }
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    _re2Cache = require('re2') as RE2Ctor;
  } catch (_e) {
    _re2Cache = null;
  }
  return _re2Cache;
...
export function re2Available(): boolean { return loadRe2() !== null; }

// ---------------------------------------------------------------------------
// STATIC ReDoS SCREEN (the load-bearing defense - runs before any execution)
// ---------------------------------------------------------------------------

interface ScreenFrame { hasUnbounded: boolean; }
 ...
 */
export function screenRegexSource(source: string): { safe: boolean; reason?: string } {
  if (typeof source !== 'string') return { safe: false, reason: 'regex source is not a string' };
  if (source.length === 0) return { safe: true };
  if (source.length > MAX_REGEX_SOURCE_LEN) return { safe: false, reason: `regex source exceeds ${MAX_REGEX_SOURCE_LEN} chars` };

  const root: ScreenFrame = { hasUnbounded: false };
  const stack: ScreenFrame[] = [];
  const top = (): ScreenFrame => (stack.length ? stack[stack.length - 1] : root);
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (c === '\\') {
      ...
      if (nxt && nxt >= '1' && nxt <= '9') return { safe: false, reason: 'backreference (\\1..\\9) is not linear-time' };
      if (nxt === 'k') return { safe: false, reason: 'named backreference (\\k<...>) is not linear-time' };
    ...
    if (c === '[') { inClass = true; lastWasGroupClose = false; continue; }

    if (c === '(') {
      stack.push({ hasUnbounded: false });
      if (stack.length > MAX_GROUP_DEPTH) return { safe: false, reason: `group nesting deeper than ${MAX_GROUP_DEPTH}` };
      lastWasGroupClose = false;
      ...
      if (source[i + 1] === '?') {
        ...
        if (p === ':' || p === '=' || p === '!') j += 1;
          ...
          if (source[j + 1] === '=' || source[j + 1] === '!') j += 2;
          else { j += 1; while (j < len && source[j] !== '>') j += 1; j += 1; } // named group up to '>'
        }
    ...
    if (c === ')') {
      ...
      if (had) parent.hasUnbounded = true; // propagate: a group "contains" an unbounded repeat at any depth
    ...
    if (c === '*' || c === '+') { isQuant = true; isUnbounded = true; isCompounding = true; }
    else if (c === '?') { isQuant = true; }
    else if (c === '{') {
    ...
    if (isQuant) {
      quantCount += 1;
      if (quantCount > MAX_QUANTIFIERS) return { safe: false, reason: `more than ${MAX_QUANTIFIERS} quantifiers` };
      ...
      if (isUnbounded) top().hasUnbounded = true;
      lastWasGroupClose = false;
      continue;
    }
...
export interface GuardedRegex { re: RE2Instance; source: string; flags: string; }
 ...
 */
export function compileGuarded(source: string, flags?: string): { re: GuardedRegex } | { error: string } {
  if (typeof source !== 'string') return { error: 'regex source is not a string' };
  if (source.length > MAX_REGEX_SOURCE_LEN) return { error: `regex source exceeds ${MAX_REGEX_SOURCE_LEN} chars` };
  const fr = validateFlags(flags);
  if ('error' in fr) return { error: fr.error };
  const RE2 = loadRe2();
  if (!RE2) return { error: 're2 (linear-time regex engine) is unavailable, so this patternSpec regex cannot be executed safely' };
  try {
 ...
 */
export function execCapped(source: string, flags: string, text: string, cap: number): { matches: { match: string; index: number }[]; error?: string } {
  const RE2 = loadRe2();
  if (!RE2) return { matches: [], error: 're2 unavailable' };
  const g = flags.includes('g') ? flags : flags + 'g';
  let re: RE2Instance;
  try { re = new RE2(source, g); } catch (e) { return { matches: [], error: `invalid regex (re2): ${e instanceof Error ? e.message : String(e)}` }; }
  const out: { match: string; index: number }[] = [];
  let m: RegExpExecArray | null;
  while (out.length < cap && (m = re.exec(text)) !== null) {
    out.push({ match: m[0], index: m.index });
    if (m[0].length === 0) re.lastIndex += 1; // guard against a zero-width infinite loop
  }
  return { matches: out };
}

// ---------------------------------------------------------------------------
// FIXED numeric-predicate allowlist (OUR code; the candidate only names one + a threshold)
// ---------------------------------------------------------------------------

export interface PredicateInput {
  text: string;      // the scoped, length-capped defect text
  cssText: string;   // full (capped) CSS
  markup: string;    // full (capped) markup
  matches: string[]; // the defect regex match strings
}
 ...
 */
export const NUMERIC_PREDICATES: Record<string, NumericPredicate> = {
  ...
  'cubic-bezier-overshoot': (input, threshold) => {
    ...
    for (const m of src.matchAll(new RegExp(CUBIC_BEZIER.source, 'gi'))) {
      ...
      if ([y1, y2].some((y) => Number.isFinite(y) && (y < -threshold || y > 1 + threshold))) return true;
    }
    return false;
  },
...
export function isKnownPredicate(id: unknown): id is string {
  return typeof id === 'string' && Object.prototype.hasOwnProperty.call(NUMERIC_PREDICATES, id);
}

export function normalizeScope(scope: unknown): PatternSpecScope {
 ...
 */
export function screenPatternSpec(spec: unknown): { ok: boolean; errors: string[] } {
  const errors: string[] = [];
  if (!spec || typeof spec !== 'object') return { ok: false, errors: ['patternSpec is not an object'] };
  const s = spec as Partial<PatternSpec> & { defect?: { anyOf?: unknown; numericGuard?: unknown } };

  if (s.engine !== PATTERN_SPEC_ENGINE) errors.push(`unknown patternSpec engine '${String(s.engine)}' (only '${PATTERN_SPEC_ENGINE}')`);
  ...
  const screenRegex = (label: string, src: string, flags: string | undefined): void => {
    const scr = screenRegexSource(src);
    if (!scr.safe) errors.push(`${label}: unsafe regex rejected (${scr.reason})`);
    const c = compileGuarded(src, flags);
    if ('error' in c) errors.push(`${label}: ${c.error}`);
  };
  ...
  if (!app || !Array.isArray(app.anyOf) || app.anyOf.length === 0) {
    ...
    app.anyOf.forEach((src, i) => {
      if (typeof src !== 'string') { errors.push(`applicability.anyOf[${i}] is not a string`); return; }
      screenRegex(`applicability.anyOf[${i}]`, src, DEFAULT_APPLICABILITY_FLAGS);
    });
  }

  const defect = s.defect;
  const anyOf = defect && (defect as { anyOf?: unknown }).anyOf;
  if (!defect || !Array.isArray(anyOf) || anyOf.length === 0) {
    ...
    (anyOf as unknown[]).forEach((d, i) => {
      const pat = d && typeof d === 'object' ? (d as PatternSpecRegex).pattern : undefined;
      if (typeof pat !== 'string') { errors.push(`defect.anyOf[${i}] is missing a string 'pattern'`); return; }
      screenRegex(`defect.anyOf[${i}]`, pat, (d as PatternSpecRegex).flags);
    });
  }

  const guard = defect && (defect as { numericGuard?: NumericGuard }).numericGuard;
  if (guard) {
    if (!isKnownPredicate(guard.predicateId)) errors.push(`numericGuard.predicateId '${String(guard.predicateId)}' is not in the allowlist [${Object.keys(NUMERIC_PREDICATES).join(', ')}]`);
    if (typeof guard.threshold !== 'number' || !Number.isFinite(guard.threshold)) errors.push('numericGuard.threshold must be a finite number');
  }

  if (typeof s.message !== 'string' || !s.message.trim()) errors.push('patternSpec.message must be a non-empty string');
