...
export class MotionReferenceImpl implements MotionReference {
  ...
  async validateMotionLaws(code: string): Promise<string[]> {
    ...
    if (
      /bounce|elastic|cubic-bezier.*1\.\d|cubic-bezier.*0\.\d\d\d/.test(code) &&
...
export function createMotionReference(): MotionReference {
