...
export const IMAGE_SURFACES: readonly ImageSurface[] = ['persuade', 'operate', 'read', 'experience'];
 ...
 */
export const COMPOSITION_GRAMMAR_PREFIXES: readonly string[] = ['Frame:', 'Focal:', 'Depth:', 'Adaptation:'];
 ...
 */
export const IMAGE_COMPOSITION_CATALOG: readonly ImageComposition[] = [
  {
    id: 'quiet-left-third',
 ...
 */
const COLOUR_LEAK_RE =
  /#[0-9a-f]{3,8}\b|\b(?:red|orange|yellow|green|blue|indigo|violet|purple|pink|magenta|cyan|teal|brown|beige|cream|ivory|ochre|terracotta|clay|moss|amber|crimson|sepia|gold|golden|silver|bronze|copper|navy|slate|charcoal|greyscale|grayscale|monochrome|saturated|desaturated|hue|oklch|rgb|hsl)\b/i;

const TYPE_LEAK_RE =
  /\b(?:serif|sans|sans-serif|grotesque|grotesk|monospace|typeface|font|lettering|headline type|display type|italic|tracking|kerning|leading|weight \d{3})\b/i;
...
export function validateImageComposition(
  entry: ImageComposition,
  seen: { ids: Set<string>; labels: Set<string> } = { ids: new Set(), labels: new Set() },
): string[] {
  const errors: string[] = [];
  ...
  if (!IMAGE_SURFACES.includes(entry?.surface)) {
    errors.push(`composition ${id} needs a surface of ${IMAGE_SURFACES.join(', ')}, got ${String(entry?.surface)}`);
  }
  if (!Array.isArray(entry?.roles) || entry.roles.length === 0) {
    errors.push(`composition ${id} must stage at least one role`);
  } else {
    for (const role of entry.roles) {
      if (!IMAGE_ROLES.includes(role)) errors.push(`composition ${id} names an unknown role: ${String(role)}`);
    }
  ...
  if (!Array.isArray(entry?.grammar) || entry.grammar.length !== COMPOSITION_GRAMMAR_PREFIXES.length) {
    errors.push(`composition ${id} needs exactly ${COMPOSITION_GRAMMAR_PREFIXES.length} grammar rules`);
  } else {
    entry.grammar.forEach((rule, i) => {
      const prefix = COMPOSITION_GRAMMAR_PREFIXES[i];
  ...
  if (!Array.isArray(zone) || zone.length !== 4 || zone.some((n) => typeof n !== 'number' || !Number.isFinite(n))) {
    ...
    const [x, y, w, h] = zone;
    if (x < 0 || y < 0 || w <= 0 || h <= 0) errors.push(`composition ${id} inkZone must have positive extent inside the frame`);
    if (x + w > 1.0001 || y + h > 1.0001) errors.push(`composition ${id} inkZone runs outside the frame`);
    if (w * h < 0.02) errors.push(`composition ${id} inkZone is too small to measure contrast in (${(w * h * 100).toFixed(1)}% of the frame)`);
  }
  ...
  if (entry?.id) seen.ids.add(entry.id);
  if (typeof entry?.label === 'string') seen.labels.add(entry.label.trim().toLowerCase());
  return errors;
}
 ...
 */
export function validateImageCompositionCatalog(
  catalog: readonly ImageComposition[] = IMAGE_COMPOSITION_CATALOG,
): CompositionValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  const seen = { ids: new Set<string>(), labels: new Set<string>() };

  if (catalog.length < 12) errors.push(`catalog needs at least 12 stagings, found ${catalog.length}`);
  for (const entry of catalog) errors.push(...validateImageComposition(entry, seen));

  for (const surface of IMAGE_SURFACES) {
    if (!catalog.some((c) => c.surface === surface)) errors.push(`no staging serves the ${surface} surface`);
  }
  for (const role of IMAGE_ROLES) {
    if (!catalog.some((c) => c.roles.includes(role))) errors.push(`no staging can stage the ${role} role`);
  }
  for (const surface of IMAGE_SURFACES) {
    const n = catalog.filter((c) => c.surface === surface).length;
    if (n < 2) warnings.push(`only ${n} staging(s) serve ${surface}; a resolution there has no spread to draw from`);
  }
...
export function compositionById(id: string): ImageComposition | undefined {
  return IMAGE_COMPOSITION_CATALOG.find((c) => c.id === id);
}

/** Every staging that can stage a role, home role first. */
export function compositionsForRole(role: ImageRole): ImageComposition[] {
  return IMAGE_COMPOSITION_CATALOG.filter((c) => c.roles.includes(role)).sort(
    (a, b) => Number(b.roles[0] === role) - Number(a.roles[0] === role) || a.id.localeCompare(b.id),
  );
}
 ...
 * unmeasurable one.
 */
export function inkZoneToPixels(
  zone: NormalizedRect,
  width: number,
  height: number,
