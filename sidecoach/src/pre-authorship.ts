...
function reqString(raw: unknown, label: string): string {
...
function reqHex(raw: unknown, label: string): string {
  const s = reqString(raw, label);
  if (!HEX_RE.test(s)) throw new Error(`brief.${label}: expected an #rgb or #rrggbb hex color, got ${JSON.stringify(raw)}`);
...
function reqFontStack(raw: unknown, label: string): string {
  const s = reqString(raw, label);
...
function parseSurface(raw: unknown): BriefSurface {
  if (!isPlainObject(raw)) throw new Error('brief.surface: expected an object');
  ...
  const sections: BriefSection[] = sectionsRaw.map((s, i) => {
    if (!isPlainObject(s)) throw new Error(`brief.surface.sections[${i}]: expected an object`);
...
function parsePalette(raw: unknown): BriefPalette {
  if (!isPlainObject(raw)) throw new Error('brief.palette: expected an object with six hex roles');
  return {
...
function parseType(raw: unknown): BriefType {
  if (!isPlainObject(raw)) throw new Error('brief.type: expected an object with display + body font stacks');
...
export function parseBrief(raw: unknown): Brief {
  if (!isPlainObject(raw)) throw new Error('brief input: expected a JSON object');
  ...
  if (raw.components !== undefined) {
    ...
    components = raw.components.map((c) => (c as string).trim());
  }
  return {
 ...
 */
function pageShell(title: string, brief: Brief, body: string): string {
...
function swatch(role: string, hex: string): string {
 ...
 */
export function buildBoardHtml(brief: Brief): string {
  const p = brief.palette;
 ...
 */
export function buildMockHtml(brief: Brief): string {
...
// ---------------------------------------------------------------------------
// The fail-closed gate (pure: audit verdicts in, proceed/halt out - no browser here)
// ---------------------------------------------------------------------------
 ...
 */
export function decidePreauthorGate(mock: ArtifactAuditLike, board: ArtifactAuditLike): PreauthorGate {
  if (mock.verdict === 'inconclusive') {
    return { decision: 'halt', exit: PREAUTHOR_EXIT.INCONCLUSIVE, reason: 'the mock did not render - a scan that did not run cannot certify the surface (fail-closed, never a silent proceed)' };
  }
  if (board.verdict === 'inconclusive') {
    return { decision: 'halt', exit: PREAUTHOR_EXIT.INCONCLUSIVE, reason: 'the design board did not render - both artifacts must render before the build proceeds' };
  }
  if (mock.verdict === 'blocked') {
    return { decision: 'halt', exit: PREAUTHOR_EXIT.BLOCKED, reason: `the mock has ${mock.severityCounts.blocking} blocking finding(s) - clear them before the build proceeds` };
  }
  ...
  return { decision: 'halt', exit: PREAUTHOR_EXIT.INCONCLUSIVE, reason: `the mock returned an unrecognized verdict ${JSON.stringify(mock.verdict)} - refusing to proceed (fail-closed)` };
}
