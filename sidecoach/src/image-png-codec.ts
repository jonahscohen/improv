 ...
 */
export function encodePng(width: number, height: number, rgba: Uint8Array, opts: EncodeOptions = {}): Buffer {
 ...
 */
const LEGAL_DEPTHS: Record<number, number[]> = {
  0: [1, 2, 4, 8, 16],
  2: [8, 16],
  3: [1, 2, 4, 8],
  4: [8, 16],
...
export function readPngHeader(
  buf: Uint8Array,
): { ok: true; width: number; height: number; bitDepth: number; colorType: number; interlace: number } | DecodeRefusal {
  if (buf.length < 8 || !PNG_SIGNATURE.equals(Buffer.from(buf.buffer, buf.byteOffset, 8))) {
    return { ok: false, reason: 'not-a-png', detail: 'signature mismatch' };
  }
  if (buf.length < 8 + 25) return { ok: false, reason: 'truncated', detail: 'shorter than signature + IHDR' };
  ...
  if (type !== 'IHDR' || len !== 13) return { ok: false, reason: 'missing-ihdr', detail: `first chunk was ${type} len ${len}` };
  return {
    ok: true,
 ...
 */
export function decodePng(buf: Uint8Array): DecodeResult {
  ...
  if (!channels) return { ok: false, reason: 'color-type-unsupported', detail: `color type ${colorType}` };
  ...
  if (width < 1 || height < 1) return { ok: false, reason: 'truncated', detail: `IHDR dimensions ${width}x${height}` };
  ...
  if (view[27] !== 0) return { ok: false, reason: 'bad-ihdr-fields', detail: `filter method ${view[27]}, only 0 is defined` };
  const idat: Buffer[] = [];
  ...
  while (pos + 8 <= view.length && !sawIend) {
    ...
    if (len > 0x7fffffff) return { ok: false, reason: 'bad-chunk-length', detail: `length ${len} at offset ${pos}` };
    const type = view.toString('latin1', pos + 4, pos + 8);
    const dataStart = pos + 8;
    const dataEnd = dataStart + len;
    if (dataEnd + 4 > view.length) return { ok: false, reason: 'truncated', detail: `chunk ${type} runs past end of file` };
    ...
    switch (type) {
      ...
      case 'PLTE':
        ...
        break;
      case 'tRNS':
        if (colorType === 3) paletteAlpha = Uint8Array.from(data);
        break;
      case 'tEXt': {
        const nul = data.indexOf(0);
        if (nul > 0) text[data.toString('latin1', 0, nul)] = data.toString('latin1', nul + 1);
        break;
      }
      case 'IEND':
        sawIend = true;
  ...
  if (idat.length === 0) return { ok: false, reason: 'missing-idat', detail: 'no IDAT chunk' };
  ...
  if (colorType === 3 && !palette) return { ok: false, reason: 'missing-palette', detail: 'color type 3 with no PLTE' };
  ...
  for (let y = 0; y < height; y++) {
    ...
    switch (filterType) {
      case 0:
        break;
      case 1:
        for (let i = filterOffset; i < stride; i++) cur[i] = (cur[i] + cur[i - filterOffset]) & 0xff;
        break;
      case 2:
