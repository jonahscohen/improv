...
import { FlowExecutionEngine, createExecutionEngine } from './sidecoach-orchestrator';
import { ProjectContext, ProductMetadata, DesignMetadata } from './project-context';

const testProductMetadata: ProductMetadata = {
  register: 'product',
...
const testDesignMetadata: DesignMetadata = {
  colors: {
    primary: '#0066FF',
    secondary: '#6B7280',
    success: '#10B981',
...
async function testPhase4Orchestration() {
  ...
  try {
    ...
    const result = await engine.process(
      'I need to design a new feature for our product. Start with brand verification, then research components and fonts, extract design tokens, and plan motion patterns.',
      {
        projectContext,
        projectPath: '/test/yes-and-demo',
        metadata: {
    ...
    if (result.flowResults && result.flowResults.length > 0) {
      ...
      result.flowResults.forEach((flowResult: any, idx: number) => {
    ...
    if (result.ambiguousCandidates && result.ambiguousCandidates.length > 1) {
      ...
      result.ambiguousCandidates.forEach((c: any) => {
    ...
    const executedFlows = (result.flowResults?.map((f: any) => f.flowName) || []) as string[];
    console.log(`   Executed: ${executedFlows.join(' → ')}`);

    // Verify phase detection
    console.log(`\n7. Phase Detection:`);
    const hasResearchFlows = executedFlows.some((f: string) => ['Component Research', 'Font Research', 'Design References', 'Motion Patterns'].includes(f));
    const hasExecutionFlows = executedFlows.some((f: string) => ['Design Tokens', 'Component Implementation', 'Motion Integration'].includes(f));
