...
function verifyAsset(buf, contract) {
    ...
    if (sniffed === 'png') {
        ...
        if (decoded.ok) {
            ...
            if (contract.alpha !== undefined)
                checks.push({ id: 'alpha-matches', status: 'unverified', detail: `pixels not decoded (${decoded.reason})` });
            if (contract.placement)
                checks.push({ id: 'contrast-at-placement', status: 'unverified', detail: `pixels not decoded (${decoded.reason})` });
        }
    }
    else {
        ...
        if (contract.alpha !== undefined)
            checks.push({ id: 'alpha-matches', status: 'unverified', detail: why });
        if (contract.placement)
            checks.push({ id: 'contrast-at-placement', status: 'unverified', detail: why });
    }
