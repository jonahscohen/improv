...
function isSubresourceAllowed(suppliedUrl, requestedUrl) {
    let supplied;
    let requested;
    try {
        supplied = new URL(suppliedUrl);
    }
    catch {
        return false;
    }
...
async function collectBrowserEvidence(renderUrl, signal) {
    ...
    try {
        ...
        await page.route('**/*', async (route) => {
            if (isSubresourceAllowed(renderUrl, route.request().url()))
                await route.continue();
            else
                await route.abort('blockedbyclient');
        });
        await race(page.goto(renderUrl, { waitUntil: 'domcontentloaded', timeout: 10000 }), 'navigation');
        const rawPromise = page.evaluate(() => {
            ...
            const backgroundFor = (el) => {
                ...
                for (const cur of chain) {
                    ...
                    if (parsed)
                        bg = blend(parsed, bg);
                    else if (rawBg && rawBg !== 'transparent' && rawBg !== 'rgba(0, 0, 0, 0)')
                        unsupported = true;
                }
            ...
            for (const el of textElements) {
                ...
                if (!fgRaw || bg.unsupported) {
                    contrastUnsupported = true;
                    continue;
                }
        ...
        if (signal?.aborted)
            return { available: false, reason: 'browser evidence collection aborted during collection' };
        ...
        const kinds = ['computed-style', 'dom'];
        if (raw.contrast.measurable)
            kinds.push('contrast');
        return {
            available: true,
            evidence: {
