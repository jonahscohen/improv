...
exports.ComponentGalleryReferenceImpl = void 0;
exports.createComponentGalleryReference = createComponentGalleryReference;
const reference_data_1 = require("./reference-data");
class ComponentGalleryReferenceImpl {
    constructor() {
    ...
    async getSemanticMarkup(componentType) {
        ...
        if (!component)
            return `<${componentType} role="region"></${componentType}>`;
    ...
    generateSemanticMarkup(componentName) {
        const templates = {
            button: '<button type="button" aria-label="Action">Label</button>',
            link: '<a href="/path" aria-label="Navigate">Link text</a>',
            input_field: '<label for="input">Label</label><input type="text" id="input" aria-invalid="false" />',
            checkbox: '<label><input type="checkbox" aria-checked="false" /> Label</label>',
            radio_button: '<fieldset><legend>Options</legend><label><input type="radio" name="group" /> Option</label></fieldset>',
            toggle_switch: '<button role="switch" aria-checked="false" aria-label="Toggle">Off</button>',
            select_dropdown: '<label for="select">Select</label><select id="select"><option>Choose</option></select>',
            card: '<article role="region" aria-label="Card"><h2>Title</h2><p>Content</p></article>',
            modal_dialog: '<div role="dialog" aria-modal="true" aria-labelledby="title"><h2 id="title">Title</h2><p>Content</p></div>',
            tabs: '<div role="tablist"><button role="tab" aria-selected="true">Tab 1</button></div><div role="tabpanel">Content</div>',
            accordion: '<button aria-expanded="false" aria-controls="panel">Header</button><div id="panel" role="region">Content</div>',
        };
        return (templates[componentName.toLowerCase().replace(/\s+/g, '_')] ||
            `<div role="region" aria-label="${componentName}"></div>`);
    }
    extractAriaRequirements(accessibility) {
        const requirements = accessibility.split(',').map((s) => s.trim());
        const ariaPatterns = requirements.filter((r) => /aria-|role=|semantic|label|heading|heading/.test(r));
        if (ariaPatterns.length === 0) {
            return ['aria-label', 'role', 'tabindex'];
        }
        return ariaPatterns.slice(0, 4);
    }
    extractKeyboardInteraction(variants) {
        const keyboardTerms = variants.filter((v) => /keyboard|focus|tab|arrow|enter|space|escape/.test(v.toLowerCase()));
    ...
    extractWcagRules(accessibility) {
        return [
...
exports.ComponentGalleryReferenceImpl = ComponentGalleryReferenceImpl;
function createComponentGalleryReference() {
    return new ComponentGalleryReferenceImpl();
}
//# sourceMappingURL=component-gallery-reference.js.map