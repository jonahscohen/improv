...
exports.MAX_EXAMPLES = exports.MAX_TAUGHT_NOTES = exports.BAN_RULE_PREFIX = exports.POLISH_FINDING_CRAFT = exports.POLISH_CRAFT = void 0;
exports.banCraftNote = banCraftNote;
exports.registryPolishRules = registryPolishRules;
exports.polishRuleKeyForNumber = polishRuleKeyForNumber;
exports.normalizeCraftKey = normalizeCraftKey;
exports.craftNote = craftNote;
exports.craftRemediation = craftRemediation;
exports.craftReason = craftReason;
exports.craftCoverageGaps = craftCoverageGaps;
 ...
 */
exports.BAN_RULE_PREFIX = 'anti-patterns:ban-';
const BAN_WHY = 'A named ban is a pattern that reads as the default answer rather than a decision, which is why ' +
...
function banCraftNote(rule) {
    if (!rule.startsWith(exports.BAN_RULE_PREFIX))
        return undefined;
    const name = rule.slice(exports.BAN_RULE_PREFIX.length);
 ...
 */
const SEVERITY_RANK = { blocker: 0, major: 1, minor: 2, advisory: 3 };
const UNRANKED = 4;
/**
 ...
 */
exports.MAX_TAUGHT_NOTES = 8;
exports.MAX_EXAMPLES = 3;
/**
...
function warnUnavailable(what, err) {
    if (_warned.has(what))
        return;
    _warned.add(what);
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`polish-craft: ${what} unavailable (${msg}). Craft notes for the affected rules will be omitted ` +
        'from the brief and the report will fall back to its generic template.\n');
}
let _registryRules = null;
/**
 ...
 */
function registryPolishRules() {
    if (_registryRules === null) {
        try {
            // eslint-disable-next-line @typescript-eslint/no-var-requires
            const { RULES } = require('./product-rule-registry');
            _registryRules = RULES
                .map((r) => {
                const aliases = r.sourceRuleAliases || [];
                ...
                .filter((x) => x !== null)
                .sort((a, b) => a.n - b.n);
            if (_registryRules.length === 0)
                warnUnavailable('product-rule-registry polish rules', 'registry exposed no polish-standard:N aliases');
        }
        catch (e) {
            _registryRules = [];
...
function polishRuleKeyForNumber(n) {
 ...
 */
function normalizeCraftKey(rule) {
    if (rule === null || rule === undefined)
        return undefined;
    if (typeof rule === 'number')
        return polishRuleKeyForNumber(rule);
    const raw = String(rule).trim();
    if (!raw)
        return undefined;
    if (exports.POLISH_CRAFT[raw] || exports.POLISH_FINDING_CRAFT[raw])
        return raw;
    const m = /^polish-standard:(?:rule-)?(\d+)$/.exec(raw);
    if (m)
        return polishRuleKeyForNumber(parseInt(m[1], 10));
    if (banCraftNote(raw))
        return raw;
    return undefined;
}
/** The note for a rule in any accepted shape, or undefined. */
function craftNote(rule) {
    const key = normalizeCraftKey(rule);
    if (!key)
        return undefined;
    return exports.POLISH_CRAFT[key] || exports.POLISH_FINDING_CRAFT[key] || banCraftNote(key);
}
 ...
 */
function craftCoverageGaps() {
    return registryPolishRules().filter((r) => !exports.POLISH_CRAFT[r.key]).map((r) => `${r.n}:${r.key}`);
}
/**
 ...
 */
function selectCraftNotes(rules, limit = exports.MAX_TAUGHT_NOTES) {
    ...
    for (const rule of rules) {
        ...
        if (!key || seen.has(key))
            continue;
        const note = exports.POLISH_CRAFT[key] || exports.POLISH_FINDING_CRAFT[key] || banCraftNote(key);
        if (!note)
            continue;
        seen.add(key);
 ...
 */
function craftBriefLines(rules, opts = {}) {
    const limit = opts.limit ?? exports.MAX_TAUGHT_NOTES;
    const exampleBudget = opts.examples ?? exports.MAX_EXAMPLES;
    const notes = selectCraftNotes(rules, limit);
    if (notes.length === 0)
        return [];
    const distinct = new Set(rules.map((r) => normalizeCraftKey(r)).filter((k) => !!k)).size;
    const lines = [];
    ...
    lines.push(`Selected from the ${distinct} rule${distinct === 1 ? '' : 's'} that failed on THIS page, hardest-first` +
...
//# sourceMappingURL=polish-craft.js.map