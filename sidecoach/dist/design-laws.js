...
exports.getDesignLawsForRegister = getDesignLawsForRegister;
exports.getSharedLawsForDomain = getSharedLawsForDomain;
// 27 Deterministic Anti-Patterns (Absolute Bans)
exports.ANTI_PATTERNS = {
    // Visual & Layout
    side_stripe_borders: {
        id: 'anti_001',
    ...
    gradient_text: {
        ...
        severity: 'critical',
        checker: (code) => /background-clip:\s*text|text\s+gradient/.test(code),
    },
    glassmorphism_default: {
        ...
        severity: 'high',
        checker: (code) => /backdrop-filter:|backdropFilter:|glass|blur\(\d+px\)/.test(code),
    },
    hero_metric_template: {
        ...
        severity: 'high',
        checker: (code) => /hero|metric|big\s*number|stats\s*grid/.test(code.toLowerCase()),
    },
    ...
    modal_as_first_thought: {
        ...
        severity: 'medium',
        checker: (code) => /modal|dialog/.test(code.toLowerCase()),
    },
    // Typography
    flat_scales: {
        id: 'anti_007',
        ...
        severity: 'high',
        checker: (code) => /font-size|--fs-|line-height/.test(code),
    },
    body_too_wide: {
        id: 'anti_008',
        ...
        severity: 'medium',
        checker: (code) => /max-width|width.*\d{3,}/i.test(code), // heuristic
    },
    // Color
    pure_black_white: {
        id: 'anti_009',
        ...
        severity: 'high',
        checker: (code) => /#000000|#fff|#ffffff|rgb\(0,\s*0,\s*0\)|rgb\(255,\s*255,\s*255\)/.test(code),
    },
    alpha_abuse: {
        id: 'anti_010',
        ...
        severity: 'medium',
        checker: (code) => /rgba\(.*,\s*0\.[1-4]\)|opacity:.*0\.[1-4]|--.*alpha/.test(code),
    },
    missing_wcag_contrast: {
        ...
        severity: 'critical',
        checker: (code) => /contrast|wcag|a11y|accessibility/.test(code.toLowerCase()),
    },
    // Spacing & Layout
    inconsistent_spacing: {
        ...
        severity: 'high',
        checker: (code) => /padding:|margin:|gap:|space-/.test(code),
    },
    nested_cards: {
        id: 'anti_013',
        name: 'Nested cards',
        ...
        severity: 'high',
        checker: (code) => /card.*card|border.*border/.test(code.toLowerCase()),
    },
    // Motion
    animated_layout_properties: {
        ...
        severity: 'high',
        checker: (code) => /animate.*(?:width|height|top|left|margin)|transition.*(?:width|height|top|left)/.test(code),
    },
    bounce_elastic_easing: {
        ...
        severity: 'high',
        checker: (code) => /bounce|elastic|cubic-bezier.*1\.\d/.test(code),
    },
    excessive_motion: {
        id: 'anti_016',
        ...
        severity: 'medium',
        checker: (code) => /animation-duration|transition-duration/.test(code),
    },
    // Interaction
    missing_focus_state: {
        id: 'anti_017',
        ...
        severity: 'critical',
        checker: (code) => /:focus|focus-visible|outline/.test(code),
    },
    placeholder_as_label: {
        ...
        severity: 'high',
        checker: (code) => /placeholder=(?!.*<label)/.test(code),
    },
    // Forms
    no_error_messages: {
        id: 'anti_019',
        ...
        severity: 'high',
        checker: (code) => /input|form|validation/.test(code.toLowerCase()),
    },
    // Copy & Content
    redundant_copy: {
        id: 'anti_020',
    ...
    desktop_first: {
        ...
        severity: 'high',
        checker: (code) => /max-width.*media/.test(code), // heuristic
    },
    touch_targets_small: {
        id: 'anti_023',
        ...
        severity: 'high',
        checker: (code) => false, // geometric check
    },
    // Images
    tinted_images: {
        id: 'anti_024',
        ...
        severity: 'medium',
        checker: (code) => /background-blend-mode|mix-blend-mode/.test(code),
    },
    // Components
    generic_component_names: {
        ...
        severity: 'low',
        checker: (code) => /button1|box|container\d/.test(code.toLowerCase()),
    },
    // Performance
    will_change_abuse: {
        id: 'anti_026',
        ...
        severity: 'low',
        checker: (code) => /will-change/.test(code),
    },
    transition_all: {
        id: 'anti_027',
        name: 'transition: all',
        ...
        severity: 'medium',
        checker: (code) => /transition:\s*all|transition-property:\s*all/.test(code),
    },
};
// Shared Design Laws by Domain
exports.SHARED_DESIGN_LAWS = {
    color: {
...
function getAntiPatternById(id) {
    return Object.values(exports.ANTI_PATTERNS).find((ap) => ap.id === id);
}
function getDesignLawsForRegister(register) {
    return exports.REGISTER_SPECIFIC_LAWS[register];
}
function getSharedLawsForDomain(domain) {
    return exports.SHARED_DESIGN_LAWS[domain];
}
//# sourceMappingURL=design-laws.js.map