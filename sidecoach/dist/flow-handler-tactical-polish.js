...
class FlowJTacticalPolishHandler extends flow_handler_1.BaseFlowHandler {
    ...
    async execute(context) {
        ...
        try {
            ...
            const extendedDomainBreakdown = extended_domain_validator_1.ExtendedDomainValidator.getDomains()
                .map((d) => `${d} (${extended_domain_validator_1.ExtendedDomainValidator.getRulesByDomain(d).length})`)
                .join(', ');
            ...
            if (absoluteBanScan.findings.length === 0) {
                ...
                absoluteBanGuidance.push(`ABSOLUTE BAN SCAN: 0 findings. The ${(0, absolute_ban_detector_1.scannedBanLabel)()} are clean.`);
            }
            else {
