...
exports.gatherReferencePreflightArtifacts = gatherReferencePreflightArtifacts;
const fs = __importStar(require("fs"));
const os = __importStar(require("os"));
const path = __importStar(require("path"));
const reference_system_preflight_1 = require("./reference-system-preflight");
const reference_systems_1 = require("./reference-systems");
...
function voiceWordsFrom(brandPersonality) {
    if (!brandPersonality)
        return [];
    return [...new Set(brandPersonality
            .toLowerCase()
            .split(/[\s,;/|]+/)
...
function readSkillArtifact(skillName, kind, title, maxChars = 1800) {
...
function componentHintFrom(target) {
    if (!target)
        return 'button';
    const lower = target.toLowerCase();
    for (const noun of COMPONENT_NOUNS) {
        if (new RegExp(`(?<![\\w-])${noun}s?(?![\\w-])`).test(lower))
            return noun;
    }
    return 'button';
}
async function gatherReferencePreflightArtifacts(opts) {
    const artifacts = [];
    const warnings = [];
    // Resolve register + voice from PRODUCT.md (soft - never throws out).
    let register = opts.register ?? 'product';
    let brandPersonality;
    try {
        const ctx = (0, context_loader_1.buildProjectContext)(opts.projectPath);
        register = (opts.register ?? ctx.register ?? 'product');
        // Read both camelCase and snake_case product shapes (handlers support both).
        brandPersonality = ctx.product?.brandPersonality ?? ctx.product?.brand_personality;
    }
    catch (e) {
    ...
    const preflight = (0, reference_system_preflight_1.createReferenceSystemPreFlight)(new reference_systems_1.ReferenceSystemsFactoryImpl());
    const probes = [
        (async () => {
        ...
        (async () => {
            ...
            const easings = await mr.getEasingCurves(intensity);
            if (easings.length) {
                artifacts.push({
        ...
        (async () => {
            const content = (0, icon_source_reference_1.buildIconSourceArtifactContent)((0, icon_source_reference_1.createIconSourceReference)());
        ...
        (async () => {
            ...
            if (a)
                ...
                warnings.push('visual-effects: skill not found (optional, read-only)');
        ...
        (async () => {
            ...
            if (a)
                ...
                warnings.push('tilt-lab: skill not found (optional, read-only)');
    ...
    const order = ['component', 'fonts', 'design-references', 'motion', 'icon-source', 'visual-effects', 'tilt-lab'];
...
//# sourceMappingURL=reference-preflight-artifacts.js.map