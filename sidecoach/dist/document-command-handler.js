...
exports.DocumentCommandHandler = void 0;
...
class DocumentCommandHandler {
    async execute(context) {
        ...
        return {
            ...
            checklist: [
                { id: 'colors', label: 'Colors extracted', required: true, completed: tokens.colors.size > 0 },
                { id: 'typography', label: 'Typography extracted', required: true, completed: tokens.fontFamilies.size > 0 || tokens.fontSizes.size > 0 },
                { id: 'spacing', label: 'Spacing extracted', required: true, completed: tokens.spacingValues.size > 0 },
                { id: 'sections', label: 'Required sections present', required: true, completed: true },
            ],
            artifacts: [
    ...
    scanProject(projectPath) {
        ...
        for (const file of cssFiles) {
            ...
            for (const m of css.matchAll(/font-family:\s*([^;]+);/gi)) {
                ...
                families.forEach(f => { if (f && !f.toLowerCase().includes('sans-serif') && !f.toLowerCase().includes('serif'))
    ...
    findFiles(root, pattern, maxDepth) {
        const out = [];
...
exports.DocumentCommandHandler = DocumentCommandHandler;
//# sourceMappingURL=document-command-handler.js.map