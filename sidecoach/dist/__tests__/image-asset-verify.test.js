...
// Fixture builders
// ---------------------------------------------------------------------------
function solid(width, height, r, g, b, a = 255, text) {
...
function halfSplit(width, height) {
    const rgba = new Uint8Array(width * height * 4);
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const i = (y * width + x) * 4;
...
const BASE = { format: 'png', width: 64, height: 64 };
// ---------------------------------------------------------------------------
...
function testColorMath() {
    ...
    assert(Math.abs((0, image_asset_verify_1.contrastRatio)(black, white) - 21) < 0.01, `black on white is 21:1, got ${(0, image_asset_verify_1.contrastRatio)(black, white)}`);
    assert(Math.abs((0, image_asset_verify_1.contrastRatio)(white, black) - 21) < 0.01, 'the ratio is symmetric');
    assert(Math.abs((0, image_asset_verify_1.contrastRatio)(white, white) - 1) < 1e-9, 'a color against itself is 1:1');
    // #767676 on white is the canonical 4.54:1 boundary color for WCAG AA normal text.
    const grey = (0, image_asset_verify_1.parseHexColor)('#767676');
    assert(grey !== null, '#767676 parses');
    assert(Math.abs((0, image_asset_verify_1.contrastRatio)(grey, white) - 4.54) < 0.02, `#767676 on white is about 4.54:1, got ${(0, image_asset_verify_1.contrastRatio)(grey, white)}`);
    assert((0, image_asset_verify_1.parseHexColor)('#abc') !== null && (0, image_asset_verify_1.parseHexColor)('#abc').r === 0xaa, 'three-digit hex expands');
    assert((0, image_asset_verify_1.parseHexColor)('not-a-color') === null, 'a bad hex returns null rather than a wrong color');
}
// ---------------------------------------------------------------------------
// 2. Blank and flat renders are caught
// ---------------------------------------------------------------------------
function testBlankDetection() {
    const white = (0, image_asset_verify_1.verifyAsset)(solid(64, 64, 255, 255, 255), BASE);
    ...
    const flatColor = (0, image_asset_verify_1.verifyAsset)(solid(64, 64, 30, 90, 160), BASE);
    assert(check(flatColor, 'rendered-not-blank').status === 'fail', 'a single flat color fails the blank check');
    const transparent = (0, image_asset_verify_1.verifyAsset)(solid(64, 64, 0, 0, 0, 0), BASE);
    assert(check(transparent, 'rendered-not-blank').status === 'fail', 'a fully transparent image fails the blank check');
    const real = (0, image_asset_verify_1.verifyAsset)(structured(64, 64), BASE);
    ...
    const tightened = (0, image_asset_verify_1.verifyAsset)(structured(64, 64), { ...BASE, blank: { minUniqueColors: 1000000 } });
    assert(check(tightened, 'rendered-not-blank').status === 'fail', 'a raised unique-color threshold is honored');
    assert(image_asset_verify_1.DEFAULT_BLANK_THRESHOLDS.minUniqueColors > 1, 'the default blank threshold demands more than one color');
}
// ---------------------------------------------------------------------------
// 3. Geometry and format come from the bytes
// ---------------------------------------------------------------------------
function testGeometryAndFormat() {
    const png = structured(64, 64);
    const wrongSize = (0, image_asset_verify_1.verifyAsset)(png, { ...BASE, width: 128, height: 128 });
    ...
    const wrongFormat = (0, image_asset_verify_1.verifyAsset)(png, { ...BASE, format: 'jpeg' });
    assert(check(wrongFormat, 'format-matches').status === 'fail', 'png bytes claimed as jpeg fail');
    const empty = (0, image_asset_verify_1.verifyAsset)(new Uint8Array(0), BASE);
    ...
    assert((0, image_asset_verify_1.sniffFormat)(minimalJpeg(300, 200)) === 'jpeg', 'jpeg is sniffed from its SOI marker');
    const jd = (0, image_asset_verify_1.readDimensions)(minimalJpeg(300, 200), 'jpeg');
    assert(jd !== null && jd.width === 300 && jd.height === 200, 'jpeg geometry comes from SOF0');
    assert((0, image_asset_verify_1.sniffFormat)(minimalWebp(640, 480)) === 'webp', 'webp is sniffed from RIFF/WEBP');
    const wd = (0, image_asset_verify_1.readDimensions)(minimalWebp(640, 480), 'webp');
    assert(wd !== null && wd.width === 640 && wd.height === 480, 'webp geometry comes from VP8X canvas fields');
    assert((0, image_asset_verify_1.sniffFormat)(minimalGif(12, 34)) === 'gif', 'gif is sniffed from its header');
    const gd = (0, image_asset_verify_1.readDimensions)(minimalGif(12, 34), 'gif');
    ...
    assert((0, image_asset_verify_1.sniffFormat)(svg) === 'svg', 'svg is sniffed from its root element');
    const sd = (0, image_asset_verify_1.readDimensions)(svg, 'svg');
    ...
    const vd = (0, image_asset_verify_1.readDimensions)(svgVb, 'svg');
    assert(vd !== null && vd.width === 200 && vd.height === 150, 'svg geometry falls back to viewBox');
    assert((0, image_asset_verify_1.sniffFormat)(Buffer.from('plain text, no image here')) === null, 'unknown bytes sniff as null');
}
// ---------------------------------------------------------------------------
// 4. The three-value rule
// ---------------------------------------------------------------------------
function testThreeValueRule() {
    const jpeg = minimalJpeg(64, 64);
    const asJpeg = (0, image_asset_verify_1.verifyAsset)(jpeg, { ...BASE, format: 'jpeg' });
    ...
    const wrongAndUndecodable = (0, image_asset_verify_1.verifyAsset)(jpeg, { ...BASE, format: 'jpeg', width: 999, height: 999 });
    ...
    const good = (0, image_asset_verify_1.verifyAsset)(structured(64, 64), BASE);
...
function testAlpha() {
    ...
    const withAlpha = (0, image_png_codec_1.encodePng)(64, 64, rgba);
    assert(check((0, image_asset_verify_1.verifyAsset)(opaque, { ...BASE, alpha: true }), 'alpha-matches').status === 'fail', 'an opaque image fails a transparency requirement');
    assert(check((0, image_asset_verify_1.verifyAsset)(withAlpha, { ...BASE, alpha: true }), 'alpha-matches').status === 'pass', 'a transparent image satisfies a transparency requirement');
    assert(check((0, image_asset_verify_1.verifyAsset)(withAlpha, { ...BASE, alpha: false }), 'alpha-matches').status === 'fail', 'a transparent image fails an opaque requirement');
    assert(check((0, image_asset_verify_1.verifyAsset)(opaque, { ...BASE, alpha: false }), 'alpha-matches').status === 'pass', 'an opaque image satisfies an opaque requirement');
    assert((0, image_asset_verify_1.verifyAsset)(opaque, BASE).checks.every((c) => c.id !== 'alpha-matches'), 'with no alpha clause the check is not run at all');
    ...
    const half = (0, image_png_codec_1.encodePng)(64, 64, halfOpaque);
    const halfStats = (0, image_asset_verify_1.verifyAsset)(half, BASE);
    ...
    assert(check((0, image_asset_verify_1.verifyAsset)(half, { ...BASE, alpha: false }), 'alpha-matches').status === 'fail', 'a uniformly half-opaque image FAILS an opaque requirement');
    assert(check((0, image_asset_verify_1.verifyAsset)(half, { ...BASE, alpha: true }), 'alpha-matches').status === 'pass', 'and it satisfies a transparency requirement');
    assert((0, image_asset_verify_1.verifyAsset)(opaque, BASE).pixels.nonOpaqueFraction === 0, 'a fully opaque image reports a zero non-opaque fraction');
}
// ---------------------------------------------------------------------------
...
function testHeaderBounds() {
    ...
    assert((0, image_asset_verify_1.sniffFormat)(lyingJpeg) === 'jpeg', 'the truncated jpeg still sniffs as jpeg');
    assert((0, image_asset_verify_1.readDimensions)(lyingJpeg, 'jpeg') === null, 'a jpeg segment that runs past the end of the file yields NO dimensions');
    ...
    assert((0, image_asset_verify_1.readDimensions)(shortSof, 'jpeg') === null, 'an SOF segment too short to hold the geometry yields NO dimensions');
    ...
    assert((0, image_asset_verify_1.sniffFormat)(lyingWebp) === 'webp', 'the lying webp still sniffs as webp');
    assert((0, image_asset_verify_1.readDimensions)(lyingWebp, 'webp') === null, 'a webp chunk claiming more bytes than the file holds yields NO dimensions');
    ...
    assert((0, image_asset_verify_1.readDimensions)(shortVp8x, 'webp') === null, 'a VP8X chunk too small for a canvas size yields NO dimensions');
    // And a well-formed one still answers, so the bounds checks did not simply disable the parsers.
    assert((0, image_asset_verify_1.readDimensions)(minimalWebp(640, 480), 'webp') !== null, 'a well-formed webp still yields dimensions');
    assert((0, image_asset_verify_1.readDimensions)(minimalJpeg(300, 200), 'jpeg') !== null, 'a well-formed jpeg still yields dimensions');
}
// ---------------------------------------------------------------------------
// 6. Contrast at placement, on real pixels
// ---------------------------------------------------------------------------
function testPlacementContrast() {
    const split = halfSplit(64, 64);
    // White ink over the WHOLE image: the near-white right half makes the worst case unreadable.
    const worstFail = (0, image_asset_verify_1.verifyAsset)(split, { ...BASE, placement: { inkHex: '#ffffff', minContrast: 4.5 } });
    ...
    const meanReport = (0, image_asset_verify_1.verifyAsset)(split, { ...BASE, placement: { inkHex: '#ffffff', minContrast: 4.5, mode: 'mean' } });
    ...
    const region = (0, image_asset_verify_1.verifyAsset)(split, { ...BASE, placement: { inkHex: '#ffffff', minContrast: 4.5, region: { x: 0, y: 0, width: 30, height: 64 } } });
    ...
    const inkFlip = (0, image_asset_verify_1.verifyAsset)(split, { ...BASE, placement: { inkHex: '#111111', minContrast: 4.5, region: { x: 0, y: 0, width: 30, height: 64 } } });
    ...
    const oob = (0, image_asset_verify_1.verifyAsset)(split, { ...BASE, placement: { inkHex: '#ffffff', minContrast: 4.5, region: { x: 40, y: 40, width: 100, height: 100 } } });
    ...
    const badInk = (0, image_asset_verify_1.verifyAsset)(split, { ...BASE, placement: { inkHex: 'chartreuse-ish', minContrast: 4.5 } });
    ...
    const onWhite = (0, image_asset_verify_1.verifyAsset)(clear, { format: 'png', width: 32, height: 32, placement: { inkHex: '#ffffff', minContrast: 1.5, backdropHex: '#ffffff' } });
    const onBlack = (0, image_asset_verify_1.verifyAsset)(clear, { format: 'png', width: 32, height: 32, placement: { inkHex: '#ffffff', minContrast: 1.5, backdropHex: '#000000' } });
    ...
    const whole = (0, image_asset_verify_1.analyzePixels)(new Uint8Array([0, 0, 0, 255, 255, 255, 255, 255]), 2, 1);
    const half = (0, image_asset_verify_1.analyzePixels)(new Uint8Array([0, 0, 0, 255, 255, 255, 255, 255]), 2, 1, { region: { x: 0, y: 0, width: 1, height: 1 } });
...
function testProvenance() {
    ...
    const passedOffAsReal = (0, image_asset_verify_1.verifyAsset)(marked, BASE);
    ...
    const honestOffline = (0, image_asset_verify_1.verifyAsset)(marked, { ...BASE, expectSynthetic: true });
    ...
    const realAsOffline = (0, image_asset_verify_1.verifyAsset)(plain, { ...BASE, expectSynthetic: true });
    assert(check(realAsOffline, 'provenance-matches').status === 'fail', 'a real render claimed as the placeholder fails');
    assert((0, image_asset_verify_1.verifyAsset)(plain, BASE).synthetic === false, 'an unmarked asset is not reported synthetic');
    const summary = (0, image_asset_verify_1.summarizeReport)(passedOffAsReal);
    assert(/provenance-matches/.test(summary) && /verdict=failed/.test(summary), `the summary names the failed check: ${summary}`);
}
function structuredWithMarker() {
    ...
    const out = (0, image_png_codec_1.encodePng)(64, 64, rgba, { text: [[image_asset_verify_1.SYNTHETIC_MARKER_KEY, 'offline-deterministic']] });
...
main();
//# sourceMappingURL=image-asset-verify.test.js.map