...
const pageInFamily = (family) => `<!doctype html><html><head><meta charset="utf-8"><style>@font-face{font-family:"${family}";src:url(data:font/woff2;base64,) format("woff2");}</style></head>` +
...
function makeProject(committedFamily) {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'dt-groundb-'));
    if (committedFamily !== null)
        fs.writeFileSync(path.join(dir, 'DESIGN.md'), designMd(committedFamily), 'utf8');
    return dir;
}
...
function capturingScan() {
    const calls = [];
    const scan = async (_url, _signal, opts) => {
        calls.push(opts);
...
async function run() {
    const dirs = [];
    const proj = (fam) => { const d = makeProject(fam); dirs.push(d); return d; };
    const browser = await playwright_1.chromium.launch({ headless: true });
    try {
        // ---------------------------------------------------------------------
        // PART 1: extraction (pure). committedFontFamilies + loadCommittedFontFamilies.
        // ---------------------------------------------------------------------
        eq((0, project_context_1.committedFontFamilies)(undefined), [], 'extract: undefined typography -> []');
        eq((0, project_context_1.committedFontFamilies)({ scale: { base: '16px' } }), [], 'extract: typography without any family -> []');
        eq((0, project_context_1.committedFontFamilies)({
        ...
        }), ['GT Sectra', 'Inter', 'JetBrains Mono'], 'extract: lead of each role family, quote-stripped, deduped');
        // Fail-closed at the source: a "commitment" to a pure system stack is NOT a chosen brand face.
        eq((0, project_context_1.committedFontFamilies)({ body: { family: 'system-ui, sans-serif' } }), [], 'extract: system-only lead dropped -> []');
        eq((0, project_context_1.committedFontFamilies)({ body: { family: 'Arial, sans-serif' } }), [], 'extract: websafe (Arial) lead dropped -> []');
        // A legal quoted family containing a comma survives whole (not truncated at the comma).
        eq((0, project_context_1.committedFontFamilies)({ body: { family: "'Arial, Grotesk', sans-serif" } }), ['Arial, Grotesk'], 'extract: quoted-comma family kept whole');
        eq((0, project_context_1.loadCommittedFontFamilies)(proj('\'Verge Serif\', Georgia, serif')), ['Verge Serif'], 'load: DESIGN.md committing Verge Serif');
        eq((0, project_context_1.loadCommittedFontFamilies)(proj(null)), [], 'load: project with NO DESIGN.md -> [] (fail-closed)');
        eq((0, project_context_1.loadCommittedFontFamilies)(''), [], 'load: empty path -> []');
        eq((0, project_context_1.loadCommittedFontFamilies)('/no/such/dir/xyz'), [], 'load: missing dir -> []');
        // ---------------------------------------------------------------------
        ...
        // ---------------------------------------------------------------------
        const collectStub = async () => ({ available: false, reason: 'test' });
        const rvMismatch = capturingScan();
        await (0, run_validator_1.runValidatorForTest)('polish-standard', { projectPath: proj('\'Verge Serif\', serif'), target: 'http://localhost:9/', renderUrl: 'http://localhost:9/' }, { collectBrowserEvidence: collectStub, scanRenderedLive: rvMismatch.scan });
        eq(rvMismatch.calls.length >= 1 ? rvMismatch.calls[0]?.typeface?.brandFamilies : 'NOT-CALLED', ['Verge Serif'], 'run-validator: DESIGN.md family forwarded to scanRenderedLive opts.typeface.brandFamilies');
        const rvNoBrand = capturingScan();
        await (0, run_validator_1.runValidatorForTest)('polish-standard', { projectPath: proj(null), target: 'http://localhost:9/', renderUrl: 'http://localhost:9/' }, { collectBrowserEvidence: collectStub, scanRenderedLive: rvNoBrand.scan });
        check(rvNoBrand.calls.length >= 1 && rvNoBrand.calls[0] === undefined, `run-validator: NO DESIGN.md -> scanRenderedLive opts undefined (fail-closed), got ${JSON.stringify(rvNoBrand.calls[0])}`);
        const rvInMemory = capturingScan();
        await (0, run_validator_1.runValidatorForTest)('polish-standard', { files: [], target: 'http://localhost:9/', renderUrl: 'http://localhost:9/' }, { collectBrowserEvidence: collectStub, scanRenderedLive: rvInMemory.scan });
        check(rvInMemory.calls.length >= 1 && rvInMemory.calls[0] === undefined, `run-validator: in-memory context (no projectPath) -> opts undefined (fail-closed), got ${JSON.stringify(rvInMemory.calls[0])}`);
        // ---------------------------------------------------------------------
        // PART 3: audit-rendered forwarding.
        // ---------------------------------------------------------------------
        const auMismatch = capturingScan();
        await (0, audit_rendered_1.runRenderedAudit)('localhost:9', { scan: auMismatch.scan, projectPath: proj('\'Verge Serif\', serif') });
        ...
        await (0, audit_rendered_1.runRenderedAudit)('localhost:9', { scan: auNoBrand.scan, projectPath: proj(null) });
        ...
        await (0, audit_rendered_1.runRenderedAudit)('localhost:9', { scan: auInject.scan, committedFamilies: ['Injected Face'] });
        ...
        const dtHits = async (brandFamilies) => {
            const opts = brandFamilies && brandFamilies.length ? { brandFamilies } : {};
            const found = await (0, subjective_rendered_scanner_1.analyzeHtmlOnBrowserSubjective)(browser, page, 30000, {}, opts);
        ...
        const committedMismatch = (0, project_context_1.loadCommittedFontFamilies)(proj('\'Verge Serif\', Georgia, serif'));
        ...
        const committedMatch = (0, project_context_1.loadCommittedFontFamilies)(proj('\'Alluvium Sans\', sans-serif'));
        ...
        const committedNone = (0, project_context_1.loadCommittedFontFamilies)(proj(null));
    ...
    if (failures.length)
        throw new Error(`default-typeface-ground-b-wiring FAILED (${failures.length}/${asserted}):\n  ${failures.join('\n  ')}`);
...
//# sourceMappingURL=default-typeface-ground-b-wiring.test.js.map