...
function run(args, env = baseEnv()) {
    const res = (0, child_process_1.spawnSync)('node', [BIN, ...args], { encoding: 'utf8', cwd: SC, env, stdio: ['ignore', 'pipe', 'pipe'] });
    if (res.error)
        throw res.error;
...
function fixturePngB64() {
    ...
    return (0, image_png_codec_1.encodePng)(w, w, rgba).toString('base64');
}
// ---------------------------------------------------------------------------
// 1. Offline: verified, and deterministic
// ---------------------------------------------------------------------------
function testOfflineHappyPath() {
    ...
    assert(failedJson.verification.checks.some((c) => c.id === 'dimensions-match' && c.status === 'fail'), 'the geometry check is the one that failed');
...
function testFailureClasses() {
    ...
    const httpFail = run(['generate', '--provider', 'nanobanana', '--prompt', 'x', '--out', path.join(TMP, 'f9.png'), '--size', '64x64', '--yes-spend', '--cache-dir', CACHE], baseEnv({ GEMINI_API_KEY: key, SIDECOACH_IMAGE_TEST_TRANSPORT: httpFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const refusal = run(['generate', '--provider', 'nanobanana', '--prompt', 'x', '--out', path.join(TMP, 'f10.png'), '--size', '64x64', '--yes-spend', '--cache-dir', CACHE], baseEnv({ GEMINI_API_KEY: key, SIDECOACH_IMAGE_TEST_TRANSPORT: refusalFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
...
function testUsageErrors() {
    ...
    const guarded = run(['generate', '--provider', 'nanobanana', '--prompt', 'x', '--out', path.join(TMP, 'u10.png'), '--size', '64x64', '--yes-spend', '--cache-dir', CACHE], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: writeFixture('unused.json', { status: 200, body: {} }) }));
...
function testCacheAndLedger() {
    ...
    const first = run(['generate', '--provider', 'nanobanana', '--prompt', prompt, '--out', out1, '--size', '64x64', '--yes-spend', '--cache-dir', cacheDir, '--quiet'], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: okFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const second = run(['generate', '--provider', 'nanobanana', '--prompt', prompt, '--out', out2, '--size', '64x64', '--yes-spend', '--cache-dir', cacheDir, '--quiet'], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: boomFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const bypass = run(['generate', '--provider', 'nanobanana', '--prompt', prompt, '--out', path.join(TMP, 'cached-3.png'), '--size', '64x64', '--yes-spend', '--no-cache', '--cache-dir', cacheDir, '--quiet'], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: boomFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const noCacheRun = run(['generate', '--provider', 'nanobanana', '--prompt', 'a bowl of cherries on slate', '--out', path.join(TMP, 'nocache.png'), '--size', '64x64', '--yes-spend', '--no-cache', '--cache-dir', noCacheDir, '--quiet'], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: okFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const cappedNoCache = run(['generate', '--provider', 'nanobanana', '--prompt', 'a bowl of cherries on slate, closer', '--out', path.join(TMP, 'nocache2.png'), '--size', '64x64', '--yes-spend', '--no-cache', '--budget-total-usd', '0.00001', '--cache-dir', noCacheDir], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: boomFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const overrun = run(['generate', '--provider', 'nanobanana', '--prompt', 'an expensive plate of light', '--out', path.join(TMP, 'overrun.png'), '--size', '64x64', '--yes-spend', '--budget-usd', '0.05', '--cache-dir', overrunDir], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: expensiveFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const unmetered = run(['generate', '--provider', 'nanobanana', '--prompt', 'a plate with no receipt', '--out', path.join(TMP, 'unmetered.png'), '--size', '64x64', '--yes-spend', '--cache-dir', unmeteredDir, '--quiet'], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: unmeteredFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const cappedOnCorrupt = run(['generate', '--provider', 'nanobanana', '--prompt', 'a plate against an unreadable ledger', '--out', path.join(TMP, 'corrupt1.png'), '--size', '64x64', '--yes-spend', '--budget-total-usd', '1.00', '--cache-dir', corruptDir], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: boomFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const proceedOnCorrupt = run(['generate', '--provider', 'nanobanana', '--prompt', 'a plate beside a quarantined ledger', '--out', path.join(TMP, 'corrupt2.png'), '--size', '64x64', '--yes-spend', '--cache-dir', corruptDir], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: okFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const attemptRun = run(['generate', '--provider', 'nanobanana', '--prompt', 'a plate the provider refuses', '--out', path.join(TMP, 'refused.png'), '--size', '64x64', '--yes-spend', '--cache-dir', attemptDir], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: refusedFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    let lockedStdout = '';
    try {
        const locked = run(['generate', '--provider', 'nanobanana', '--prompt', 'a plate nobody can account for', '--out', path.join(TMP, 'locked.png'), '--size', '64x64', '--yes-spend', '--cache-dir', lockedDir, '--quiet'], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: okFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
    ...
    const capped = run(['generate', '--provider', 'nanobanana', '--prompt', `${prompt} at dusk`, '--out', path.join(TMP, 'capped.png'), '--size', '64x64', '--yes-spend', '--budget-total-usd', '0.00001', '--cache-dir', cacheDir], baseEnv({ GEMINI_API_KEY: 'k', SIDECOACH_IMAGE_TEST_TRANSPORT: boomFixture, SIDECOACH_IMAGE_ALLOW_FIXTURE: '1' }));
...
function testVerifySubcommand() {
    ...
    assert(/provenance-matches/.test(JSON.parse(laundered.stdout).verification.checks.map((c) => c.id).join(',')), 'the provenance check is the one that fired');
    ...
    fs.writeFileSync(blank, (0, image_png_codec_1.encodePng)(32, 32, flat));
    const blankRun = run(['verify', blank, '--expect-size', '32x32', '--quiet']);
    assert(blankRun.code === 1, `a blank asset fails (got ${blankRun.code})`);
    const blankIds = JSON.parse(blankRun.stdout).verification.checks.filter((c) => c.status === 'fail').map((c) => c.id);
...
main();
//# sourceMappingURL=image-cli.test.js.map