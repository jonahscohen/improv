...
function run(args) {
    ...
    let stdout = '';
    try {
        stdout = (0, child_process_1.execFileSync)('node', [BIN, ...args], { encoding: 'utf8', cwd: SC, stdio: ['ignore', 'pipe', 'pipe'] });
    }
    catch (e) {
        const err = e;
    ...
    if (stdout.trim()) {
        ...
        catch { /* non-JSON (default human mode) - leave {} */ }
    }
...
function main() {
    ...
    const cats = new Set(d.json.drifted.map((x) => x.category));
    for (const c of ['color', 'radius', 'spacing', 'easing', 'duration']) {
        assert(cats.has(c), `drift fixture must surface a ${c} drift`);
    }
    const hover = d.json.drifted.find((x) => x.name === '--c-brand-red-hover');
    ...
    const names = d.json.drifted.map((x) => x.name);
    ...
    try {
        ...
        try {
            ...
            if (!stillReadable) {
                ...
                assert(rUnread.json.unreadableSources.some((s) => s.includes('secret.css')), 'the unreadable source must be named');
            }
        }
        finally {
        ...
        let symlinkOk = true;
        try {
            fs.symlinkSync(realTarget, path.join(symProj, 'linked.css'));
        }
        catch {
            symlinkOk = false;
        }
        if (symlinkOk) {
            ...
            assert(rSym.json.drifted.some((d) => d.name === '--c-off'), 'drift inside a symlinked stylesheet must be detected');
        }
        ...
        assert(rHidden.json.drifted.some((d) => d.name === '--c-sneaky'), 'drift in a hidden .css must be detected, not silently skipped');
        ...
        let symDirOk = true;
        try {
            fs.symlinkSync(realDir, path.join(symDirProj, 'shared'), 'dir');
        }
        catch {
            symDirOk = false;
        }
        if (symDirOk) {
            ...
            assert(rSymDir.json.drifted.some((d) => d.name === '--c-linked-dir'), 'drift inside a symlinked directory must be detected');
        }
        ...
        let cycleOk = true;
        try {
            fs.symlinkSync(cycleProj, path.join(cycleProj, 'loop'), 'dir');
        }
        catch {
            cycleOk = false;
        }
    ...
    const lime = inline.json.drifted.find((x) => x.name === '--c-accent-lime');
...
main();
//# sourceMappingURL=drift-cli.test.js.map