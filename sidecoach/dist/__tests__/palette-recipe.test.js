...
const palette_recipe_1 = require("../palette-recipe");
...
// ---------------------------------------------------------------------------
function testOklch() {
    const a = (0, palette_recipe_1.oklchToHex)({ L: 0.6, C: 0.15, H: 262 });
    const b = (0, palette_recipe_1.oklchToHex)({ L: 0.6, C: 0.15, H: 262 });
    ...
    const gray = (0, palette_recipe_1.oklchToHex)({ L: 0.5, C: 0.0, H: 0 });
    ...
    assert(HEX.test((0, palette_recipe_1.oklchToHex)({ L: 0.98, C: 0.3, H: 100 })), 'very light out-of-gamut must gamut-map, not NaN');
    assert(HEX.test((0, palette_recipe_1.oklchToHex)({ L: 0.05, C: 0.3, H: 300 })), 'very dark out-of-gamut must gamut-map, not NaN');
}
// ---------------------------------------------------------------------------
// 2. Brand-input validation (throws precisely; never silently accepts garbage).
// ---------------------------------------------------------------------------
function testBrandValidation() {
    const ok = (0, palette_recipe_1.parseBrandInput)(GOOD_BRAND);
    ...
    const throws = (raw, label) => {
        let threw = false;
        try {
            (0, palette_recipe_1.parseBrandInput)(raw);
        }
        catch {
            threw = true;
        }
...
function testPalette() {
    const ramp = (0, palette_recipe_1.buildRamp)({ hue: 262, chroma: 0.15 });
    for (const stop of palette_recipe_1.RAMP_STOPS)
        assert(HEX.test(ramp[stop]), `ramp stop ${stop} must be a valid hex`);
    // Lighter stops must be visibly lighter than darker stops (monotonic-ish luminance): 50 lighter than 900.
    const y = (hex) => parseInt(hex.slice(1, 3), 16) + parseInt(hex.slice(3, 5), 16) + parseInt(hex.slice(5, 7), 16);
    assert(y(ramp[50]) > y(ramp[900]), 'stop 50 must be lighter than stop 900');
    const pal = (0, palette_recipe_1.buildPalette)((0, palette_recipe_1.parseBrandInput)(GOOD_BRAND));
    assert(pal.solidStop.primary === 700, 'default solid stop is 700');
    const pinned = (0, palette_recipe_1.buildPalette)((0, palette_recipe_1.parseBrandInput)({ ...GOOD_BRAND, primary: { hue: 262, chroma: 0.15, solidStop: 500 } }));
    assert(pinned.solidStop.primary === 500, 'pinned solidStop is respected');
    assert(['neutral', 'primary', 'success', 'warning', 'danger', 'info'].every((f) => pal.ramps[f] !== undefined), 'all six families built');
}
// ---------------------------------------------------------------------------
// 4. Required pairs + swatch page.
// ---------------------------------------------------------------------------
function testPairsAndSwatches() {
    const pal = (0, palette_recipe_1.buildPalette)((0, palette_recipe_1.parseBrandInput)(GOOD_BRAND));
    const pairs = (0, palette_recipe_1.requiredPairs)(pal);
    ...
    const expectedCands = palette_recipe_1.ACCENT_FAMILIES.reduce((sum, f) => sum + (0, palette_recipe_1.accentSurfaces)(pal, f).length * 2, 0);
    assert(cands.length === expectedCands, `expected ${expectedCands} on-color candidates, got ${cands.length}`);
    assert((0, palette_recipe_1.accentSurfaces)(pal, 'primary').length === 3, 'primary emits solid+hover+active surfaces');
    assert((0, palette_recipe_1.accentSurfaces)(pal, 'success').length === 1, 'a semantic family emits only the solid surface');
    for (const p of pairs) {
        assert(HEX.test(p.fg), `pair ${p.id} fg hex`);
        assert(HEX.test(p.bg), `pair ${p.id} bg hex`);
    }
    const html = (0, palette_recipe_1.buildSwatchHtml)(pairs);
    for (const p of pairs)
        assert(html.includes(`id="${p.id}"`), `swatch html must contain a div for pair ${p.id}`);
...
// ---------------------------------------------------------------------------
function lowContrast(id, detail = '3.00:1 (need 4.5:1)') {
    return { rule: 'low-contrast', selector: `div#${id}`, detail };
}
function testResolveVerdict() {
    const pal = (0, palette_recipe_1.buildPalette)((0, palette_recipe_1.parseBrandInput)(GOOD_BRAND));
    const pairs = (0, palette_recipe_1.requiredPairs)(pal);
    // (a) No findings => nothing fails; every accent resolves to the preferred (paper) label.
    const clean = (0, palette_recipe_1.resolveVerdict)(pairs, []);
    ...
    assert(palette_recipe_1.ACCENT_FAMILIES.every((f) => clean.onColor[f] === 'paper'), 'clean => all accents prefer paper');
    // (b) A low-contrast finding on a HARD pair => that pair fails, named with the scanner detail.
    const hardFail = (0, palette_recipe_1.resolveVerdict)(pairs, [lowContrast('link-on-canvas', '2.90:1 (need 4.5:1)')]);
    ...
    const inkFallback = (0, palette_recipe_1.resolveVerdict)(pairs, [lowContrast('on-primary-solid-paper')]);
    ...
    const both = (0, palette_recipe_1.resolveVerdict)(pairs, [lowContrast('on-primary-solid-paper', '3.47:1 (need 4.5:1)'), lowContrast('on-primary-solid-ink', '4.00:1 (need 4.5:1)')]);
    ...
    const stateHole = (0, palette_recipe_1.resolveVerdict)(pairs, [
    ...
    const grayOnly = (0, palette_recipe_1.resolveVerdict)(pairs, [{ rule: 'gray-on-color', selector: 'div#link-on-canvas', detail: 'x' }]);
    ...
    const noId = (0, palette_recipe_1.resolveVerdict)(pairs, [{ rule: 'low-contrast', selector: 'div.some-class', detail: 'x' }]);
...
function testEmit() {
    const pal = (0, palette_recipe_1.buildPalette)((0, palette_recipe_1.parseBrandInput)(GOOD_BRAND));
    const pairs = (0, palette_recipe_1.requiredPairs)(pal);
    const verdict = (0, palette_recipe_1.resolveVerdict)(pairs, []); // clean verdict for a shape check
    const md1 = (0, palette_recipe_1.emitDesignMd)(pal, verdict);
    const md2 = (0, palette_recipe_1.emitDesignMd)(pal, verdict);
...
// ---------------------------------------------------------------------------
function testOverrideConsistency() {
    assert((0, palette_recipe_1.darkerStop)(700, 1) === 800 && (0, palette_recipe_1.darkerStop)(700, 2) === 900, 'darkerStop steps down the ramp');
    assert((0, palette_recipe_1.darkerStop)(900, 1) === 900, 'darkerStop clamps at 900');
    assert((0, palette_recipe_1.darkerStop)(500, 1) === 600, 'darkerStop relative to a pinned solid');
    const pal = (0, palette_recipe_1.buildPalette)((0, palette_recipe_1.parseBrandInput)({
    ...
    const scannedBg = (0, palette_recipe_1.requiredPairs)(pal).find((p) => p.id === 'on-success-solid-paper').bg;
    assert(scannedBg === verifiedSolid, 'the scanned on-success background is the 500 solid');
    const md = (0, palette_recipe_1.emitDesignMd)(pal, (0, palette_recipe_1.resolveVerdict)((0, palette_recipe_1.requiredPairs)(pal), []));
...
main();
//# sourceMappingURL=palette-recipe.test.js.map