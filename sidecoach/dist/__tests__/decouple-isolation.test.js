...
const SCAN = path_1.default.join(__dirname, '..', '..', 'eval', 'sidecoach-scan.mjs');
function buildPage() {
...
function runMode(file, mode, timeoutMs) {
    const out = (0, child_process_1.execFileSync)('node', [SCAN, file, mode], { encoding: 'utf8', timeout: timeoutMs, maxBuffer: 1 << 24 });
    const arr = JSON.parse(out);
    if (!Array.isArray(arr))
        throw new Error(`${mode} mode: stdout not a JSON array`);
    return arr;
}
function run() {
    const dir = (0, fs_1.mkdtempSync)(path_1.default.join((0, os_1.tmpdir)(), 'decouple-'));
    const file = path_1.default.join(dir, 'page.html');
    (0, fs_1.writeFileSync)(file, buildPage());
    const failures = [];
    ...
    if (objLeak.length)
        failures.push(`objective mode LEAKED static-subjective findings: [${objLeak.map((f) => f.source + ':' + f.rule).join(', ')}]`);
    ...
    if (subjLeak.length)
        failures.push(`subjective mode LEAKED rendered findings: [${subjLeak.map((f) => f.source + ':' + f.rule).join(', ')}]`);
    if (failures.length)
        throw new Error(`decouple-isolation FAILED (${failures.length}):\n  ${failures.join('\n  ')}`);
...
//# sourceMappingURL=decouple-isolation.test.js.map