...
const taste_map_types_1 = require("../taste-map-types");
let passed = 0;
const failures = [];
...
// ---------------------------------------------------------------------------
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(a, b), null, 'different axisSubject => null (unrelated concepts)');
}
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(a, b), null, 'empty axisSubject => null (no join key)');
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
{
    ...
    const r = (0, taste_map_types_1.classifyContradiction)(bolder, quieter);
...
// the gate wins even if a downstream author mislabeled them.)
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(a, b).type, 'direction-pair', 'direction gate beats polarity: bold vs restrained is never hard-vs-hard');
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
{
    ...
    const r = (0, taste_map_types_1.classifyContradiction)(ban, mandate);
...
// two SAME-polarity bans agree - not a contradiction.
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(a, b), null, 'two same-polarity bans agree => null (not a conflict)');
}
// a hard-prohibitive with no polarity cannot be asserted as opposing - fail toward NOT flagging.
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(a, b), null, 'missing polarity => null (never a fabricated hard-vs-hard)');
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
{
    ...
    const r = (0, taste_map_types_1.classifyContradiction)(a, b);
...
// identical measurements agree - not a calibration conflict.
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(a, b), null, 'identical measurements => null (agreement)');
}
// ---------------------------------------------------------------------------
// cross-type
// ---------------------------------------------------------------------------
// a direction vs a hard ban on the same axis (serif banned-on-UI vs serif-for-Elegant).
{
    ...
    const r = (0, taste_map_types_1.classifyContradiction)(dir, ban);
...
// a hard-prohibitive vs a standard-measurement on the same axis.
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(ban, std).type, 'cross-type', 'hard-prohibitive vs standard on the same axis => cross-type');
}
// a direction vs a SOFT principle is not a conflict.
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(dir, principle), null, 'a direction vs a soft principle => null (not a conflict)');
}
// two principles that share an axis are never a hard conflict.
{
    ...
    eq((0, taste_map_types_1.classifyContradiction)(a, b), null, 'two soft principles => null');
}
// ---------------------------------------------------------------------------
// seedConceptFromKey + normalizeConcept + buckets
// ---------------------------------------------------------------------------
{
    const s = (0, taste_map_types_1.seedConceptFromKey)({ canonicalRuleKey: 'anti-pattern/gradient-text' });
    ...
    const s2 = (0, taste_map_types_1.seedConceptFromKey)({ ruleId: 'theming.border-radius-consistency' });
    eq(s2.concept, 'theming-border-radius-consistency', 'seedConceptFromKey falls back to ruleId (dots/underscores -> hyphen)');
    eq((0, taste_map_types_1.normalizeConcept)('Border_Radius!! 8px'), 'border radius 8px', 'normalizeConcept collapses non-alphanumerics');
    const buckets = (0, taste_map_types_1.emptyContradictionsByType)();
    ok(taste_map_types_1.DISTILLED_RULE_TYPES.length === 4, 'four distilled rule types');
...
//# sourceMappingURL=taste-map-types.test.js.map