...
async function run() {
    ...
    pre.abort();
    const r0 = await (0, browser_evidence_collector_1.collectBrowserEvidence)(url, pre.signal);
    if (r0.available)
        throw new Error('already-aborted must not return available:true');
    if (!/aborted before launch/i.test(r0.reason))
        throw new Error(`already-aborted reason wrong: ${r0.reason}`);
    // Is a browser launchable here? If not, skip the mid-op assertions.
    const base = await (0, browser_evidence_collector_1.collectBrowserEvidence)(url);
    if (!base.available) {
        console.log(`browser-evidence-abort: SKIP (${base.reason})`);
        return;
    }
    ...
    for (let i = 0; i < 2; i++) {
        ...
        const p = (0, browser_evidence_collector_1.collectBrowserEvidence)(url, ac.signal);
        setTimeout(() => ac.abort(), 2);
        const r = await p;
        if (r.available)
            throw new Error('mid-launch abort must not return available:true');
        if (!/aborted during launch/i.test(r.reason))
            throw new Error(`mid-launch abort must be honored at the launch phase, got: ${r.reason}`);
    }
    // After aborted runs, a normal collection still succeeds (no hang, no leaked browser
    // starving subsequent launches).
    const after = await (0, browser_evidence_collector_1.collectBrowserEvidence)(url);
    if (!after.available)
        throw new Error('a normal collection must still succeed after aborted runs');
    console.log('browser-evidence-abort: OK');
}
run().catch((e) => { console.error(e); process.exit(1); });
//# sourceMappingURL=browser-evidence-abort.test.js.map