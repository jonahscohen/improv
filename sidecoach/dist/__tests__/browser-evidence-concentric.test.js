...
async function run() {
    ...
    const r = await (0, browser_evidence_collector_1.collectBrowserEvidence)(renderUrl);
    if (!r.available) {
        console.log(`browser-evidence-concentric: SKIP (${r.reason})`);
        return;
    }
    const e = r.evidence;
    ...
    if (polish_checks_1.POLISH_CHECKS['polish/concentric-radius'](ctx).status !== 'fail') {
...
//# sourceMappingURL=browser-evidence-concentric.test.js.map