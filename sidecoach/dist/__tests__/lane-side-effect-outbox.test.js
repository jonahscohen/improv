...
async function run() {
    ...
    const d = deps(proj);
    const s = await (0, lane_runner_1.startLane)('lane_build', 'hero', { projectPath: proj }, 'req-obx', d);
    const firstStepHistory = new flow_history_1.FlowHistory(proj).getLatestRun(`lane:lane_build:${s.currentVerb}`);
    if (!firstStepHistory?.laneLogicalKey?.endsWith(':flow-history'))
        throw new Error('first-step FINALIZE must publish a flow-history entry');
    ...
    await (0, lane_runner_1.advanceLane)(proj, s.checkpointId, { action: 'complete', report: rep('craft'), expectedRevision: cur.revision }, d);
    // after a clean commit + publish: the sink holds the entry, the outbox record is drained
    const sink = new lane_side_effect_sink_1.LaneSideEffectSink(proj);
    const key = `${s.checkpointId}:craft:0`;
    const rec = sink.get(key);
    if (!rec)
        throw new Error('sink must hold the committed step side effect');
    if (rec.fencingToken < 1)
        throw new Error('sink entry carries the fencing token');
    const after = d.store.read(s.checkpointId);
    if (after.sideEffectOutbox.length !== 0)
        throw new Error('fully-published outbox record is drained');
    ...
    if (sink.upsertSync(key, rec.fencingToken, { v: 1 }).status !== 'noop')
        throw new Error('same-token replay must be a no-op');
    if (sink.upsertSync(key, rec.fencingToken - 1, { v: 1 }).status !== 'rejected')
        throw new Error('lower token must be rejected');
    if (sink.upsertSync(key, rec.fencingToken + 1, { v: 2 }).status !== 'written')
        throw new Error('higher token must overwrite');
    ...
    d2.publishOutbox = async (...args) => {
        if (failOnce) {
            failOnce = false;
            throw new Error('injected publish crash');
        }
        return (0, lane_checkpoint_store_1.publishOutbox)(...args);
    };
    const s2 = await (0, lane_runner_1.startLane)('lane_build', 'hero2', { projectPath: proj }, 'req-crash', d2);
    await (0, lane_runner_1.advanceLane)(proj, s2.checkpointId, { action: 'complete', report: rep('shape'), expectedRevision: s2.revision }, d2);
    const cur2 = d2.store.read(s2.checkpointId);
    failOnce = true; // fail only after craft FINALIZE, not during start/shape publishing
    let crashed = false;
    try {
        await (0, lane_runner_1.advanceLane)(proj, s2.checkpointId, { action: 'complete', report: rep('craft'), expectedRevision: cur2.revision }, d2);
    }
    catch (e) {
        crashed = String(e).includes('injected publish crash');
    }
    if (!crashed)
        throw new Error('test must fail after FINALIZE at publish');
    const pending = d2.store.read(s2.checkpointId);
    if (!pending.completedStepIds.includes('craft') || pending.sideEffectOutbox.length !== 1)
        throw new Error('FINALIZE commit + pending outbox must survive publish crash');
    ...
    await (0, lane_checkpoint_store_1.publishOutbox)(d2.store, s2.checkpointId, proj, d2.now); // production publisher, no reseeding
    if (d2.store.read(s2.checkpointId).sideEffectOutbox.length !== 0)
        throw new Error('production replay must drain pending outbox');
    const sink2 = new lane_side_effect_sink_1.LaneSideEffectSink(proj);
    const k2 = `${s2.checkpointId}:craft:0`;
    if (sink2.get(k2)?.fencingToken !== pendingRecord.fencingToken)
        throw new Error('production replay publishes the committed fencing token');
    const historyAfterReplay = new flow_history_1.FlowHistory(proj);
    const replayedFlow = historyAfterReplay.getLatestRun('lane:lane_build:craft');
    if (replayedFlow?.fencingToken !== pendingRecord.fencingToken)
        throw new Error('crash replay must publish flow-history with the committed fencing token');
    ...
    await (0, lane_checkpoint_store_1.publishOutbox)(d2.store, s2.checkpointId, proj, d2.now);
    if (d2.store.read(s2.checkpointId).sideEffectOutbox.length !== 0) {
        throw new Error('same-token no-op replay must still ack both publishers');
    }
    if (new flow_history_1.FlowHistory(proj).getFlowCount('lane:lane_build:craft') !== countBeforeReplay) {
    ...
    if (d2.store.read(s2.checkpointId).sideEffectOutbox.length !== 1)
        throw new Error('reseeded crash artifact must be pending before entrypoint');
    const productionDeps = deps(proj); // no publish failure injection
    await (0, lane_runner_1.startLane)('lane_build', 'recovery-trigger', { projectPath: proj }, 'req-recovery-trigger', productionDeps);
    if (d2.store.read(s2.checkpointId).sideEffectOutbox.length !== 0)
        throw new Error('later production entrypoint must replay all pending project outbox');
...
async function runMultiPublisherAck() {
    ...
    await (0, lane_checkpoint_store_1.publishOutbox)(store, 'lane-multi', proj, () => '2026-01-01T00:00:10.000Z');
    const after = store.read('lane-multi');
    if (after.sideEffectOutbox.length !== 0)
        throw new Error('record must be removed only after both declared publishers ack');
    if (!new lane_side_effect_sink_1.LaneSideEffectSink(proj).get('lane-multi:craft:0'))
        throw new Error('sink must hold the published entry');
    const flowHistory = new flow_history_1.FlowHistory(proj);
    ...
    rejectedDelivery.entries = rejectedDelivery.entries.filter((entry) => entry.publisher === 'flow-history');
    rejectedDelivery.entries[0].logicalKey = 'lane-multi:craft:0:flow-history';
    store.write(baseCp('lane-rejected', [rejectedDelivery]));
    await (0, lane_checkpoint_store_1.publishOutbox)(store, 'lane-rejected', proj, () => '2026-01-01T00:00:10.250Z');
    if (store.read('lane-rejected').sideEffectOutbox.length !== 0)
        throw new Error('rejected lower-token delivery must still ack its publisher');
    if (new flow_history_1.FlowHistory(proj).getLatestRun('lane:lane_build:craft')?.fencingToken !== 2) {
    ...
    try {
        await (0, lane_checkpoint_store_1.publishOutbox)(store, 'lane-publisher-fail', proj, () => '2026-01-01T00:00:10.500Z');
    }
    catch {
        flowHistoryFailed = true;
    }
    if (!flowHistoryFailed)
        throw new Error('flow-history persistence failure must leave its publisher pending');
    ...
    if (!new lane_side_effect_sink_1.LaneSideEffectSink(proj).get('lane-publisher-fail:craft:0'))
        throw new Error('sink delivery must survive later publisher failure');
    process.env.HOME = home;
    const retained = rec('lane-retained', ['lane-side-effect-sink', 'unknown-publisher']);
    retained.entries = retained.entries.filter((entry) => entry.publisher === 'lane-side-effect-sink');
    store.write(baseCp('lane-retained', [retained]));
    const revisionBefore = store.read('lane-retained').revision;
    await (0, lane_checkpoint_store_1.publishOutbox)(store, 'lane-retained', proj, () => '2026-01-01T00:00:11.000Z');
    const retainedAfter = store.read('lane-retained');
    if (retainedAfter.revision !== revisionBefore)
        throw new Error('publisher acknowledgement must not change semantic revision');
    if (retainedAfter.sideEffectOutbox.length !== 1)
        throw new Error('record must survive while any publisher remains pending');
...
//# sourceMappingURL=lane-side-effect-outbox.test.js.map