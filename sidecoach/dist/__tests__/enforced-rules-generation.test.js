...
const enforced_rules_generation_1 = require("../validators/enforced-rules-generation");
let passed = 0;
const failures = [];
...
function validInputs() {
    ...
    const ledger = new Map([['mined.x', { ruleId: 'mined.x', content_digest: 'cd', precision_digest: PDIG, precision: 0.95 }]]);
    const precision = new Map([['mined.x', { precisionDigest: PDIG, buildStamp: STAMP, pass: true, precision: 0.95, threshold: 0.90, minHeldoutPositives: 8, minFires: 8, recomputedPrecisionDigest: PDIG }]]);
...
{
    const r = (0, enforced_rules_generation_1.deriveEnforcedRules)(validInputs());
    ...
    ok(r.certified[0]?.severity === enforced_rules_generation_1.ENFORCED_SEVERITY && enforced_rules_generation_1.ENFORCED_SEVERITY === 'major', 'certified rule is at blocking severity major');
...
function rejects(mutate, label, errNeedle) {
    const i = validInputs();
    mutate(i);
    const r = (0, enforced_rules_generation_1.deriveEnforcedRules)(i);
    ok(r.certified.length === 0 && r.errors.length > 0, `REJECT: ${label}`);
    if (errNeedle)
        ok(r.errors.some((e) => e.includes(errNeedle)), `REJECT: ${label} names "${errNeedle}"`);
}
...
rejects((i) => { i.records[0].obj.rule.ruleId = 'a-different-id'; }, 'body ruleId != filename', 'does not match filename');
rejects((i) => { i.records[0].obj.rule.sourceVocabulary = 'p012'; }, 'not mined-taste', 'not "mined-taste"');
rejects((i) => { delete i.records[0].obj.rule.patternSpec; }, 'no patternSpec', 'no patternSpec');
...
// a STRICTER floor than production is fine (more conservative) -> still certifies.
{
    ...
    p.threshold = 0.95;
    p.minHeldoutPositives = 20;
    p.minFires = 30;
    ok((0, enforced_rules_generation_1.deriveEnforcedRules)(i).certified.length === 1, 'a record measured at a STRICTER-than-production floor still certifies');
}
...
// floor accepts a record measured at that lower floor.
{
    ...
    const p = i.precisionByRuleId.get('mined.x');
    p.threshold = 0.90;
    p.minHeldoutPositives = 3;
    p.minFires = 3;
    ok((0, enforced_rules_generation_1.deriveEnforcedRules)({ ...i, requiredMinHeldoutPositives: 3, requiredMinFires: 3 }).certified.length === 1, 'with a test-lowered required floor, a record measured at that floor certifies');
    ok((0, enforced_rules_generation_1.deriveEnforcedRules)(i).certified.length === 0, 'the SAME record is REJECTED under the default production floor (8/8)');
}
// ---- a mix: one valid + one tampered -> the valid still certifies, the tampered is an error ----
{
    const i = validInputs();
    const bad = { fileStem: 'mined.bad', obj: { ruleId: 'mined.bad', rule: { ruleId: 'mined.bad', sourceVocabulary: 'mined-taste', patternSpec: { engine: 'static-css-regex' } } } };
...
// ---- render ----
{
    const empty = (0, enforced_rules_generation_1.renderEnforcedRulesModule)([]);
    ...
    const one = (0, enforced_rules_generation_1.renderEnforcedRulesModule)((0, enforced_rules_generation_1.deriveEnforcedRules)(validInputs()).certified);
...
//# sourceMappingURL=enforced-rules-generation.test.js.map