...
function findRule(pred, label) {
    const hit = design_laws_1.SHARED_DESIGN_LAWS.typography.rules.find(pred);
    if (!hit)
        throw new Error(`typography.rules is missing the ${label} rule`);
    return hit;
}
...
function assertEmitsTypographyRules(src, handlerName) {
    // Direct form: mapped array spread inline into a guidance array.
    if (/\.\.\.\s*SHARED_DESIGN_LAWS\.typography\.rules\.map\(/.test(src))
        return;
    ...
    if (domain) {
        ...
        if (mapped) {
            ...
            if (new RegExp(`\\.\\.\\.\\s*${guidanceLocal}\\b`).test(src))
                return; // spread into a guidance array
        }
    }
...
function run() {
    const rules = design_laws_1.SHARED_DESIGN_LAWS.typography.rules;
    if (!Array.isArray(rules) || rules.length === 0)
        throw new Error('SHARED_DESIGN_LAWS.typography.rules is empty');
    ...
    if (!/\brem\b/.test(clampRule))
        throw new Error('clamp rule must require a rem term (pure vw defeats zoom)');
    if (!/pure-?vw/i.test(clampRule))
        throw new Error('clamp rule must name the pure-vw failure mode');
    // Require the SPECIFIC criterion, not a bare "200%" that could appear incidentally (Codex Low).
    if (!/WCAG\s*1\.4\.4/.test(clampRule))
        throw new Error('clamp rule must cite WCAG 1.4.4 (resize text) by number');
    if (!/\bvw\b/.test(clampRule))
        throw new Error('clamp rule must show the viewport (vw) term');
    if (!/>=\s*1\.25|1\.25/.test(clampRule))
        throw new Error('clamp rule must preserve the >=1.25 scale ratio at both ends');
    ...
    if (!/serif/i.test(pairRule) || !/sans/i.test(pairRule))
        throw new Error('pairing rule must contrast serif with sans');
    ...
    if (named.length < 2)
        throw new Error(`pairing rule must name concrete faces; found only [${named.join(', ')}]`);
    ...
    const typefaceRule = findRule((r) => /system stack|deliberately picked/i.test(r) && ALLOWED_FACES.every((f) => r.includes(f)), 'typeface-selection (source of the sanctioned face vocabulary)');
    ...
    for (const h of handlers) {
        const src = (0, node_fs_1.readFileSync)(node_path_1.default.resolve(__dirname, '..', h), 'utf8');
        assertEmitsTypographyRules(src, h);
    }
    console.log(`typography-fluid-pairing: OK (pairing names [${named.join(', ')}], clamp carries rem+WCAG-1.4.4 caveat, ` +
        `consistency vs typeface rule intact, emission via ${handlers.length} handlers)`);
}
run();
//# sourceMappingURL=typography-fluid-pairing.test.js.map