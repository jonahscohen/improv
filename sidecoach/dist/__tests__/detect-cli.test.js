...
function runDetect(args) {
    ...
    let stdout = '';
    try {
        stdout = (0, child_process_1.execFileSync)('node', [BIN, ...args, '--quiet'], { encoding: 'utf8', cwd: SC, stdio: ['ignore', 'pipe', 'pipe'] });
    }
    catch (e) {
        const err = e;
    ...
    if (stdout.trim()) {
        try {
            ...
            throw new Error(`stdout was not JSON for [${args.join(' ')}]:\n${stdout.slice(0, 400)}`);
        }
    }
    return { code, stdout, json };
}
function lensesOf(r) {
    return r.json.lenses;
}
function rulesOf(r) {
    return r.json.findings.map((f) => f.rule);
}
function main() {
    // ---------------------------------------------------------------------
    ...
    assert(detect.decideVerdict({ a: ran, b: failed }, 0, 0) === 'inconclusive', 'partial scan with ZERO findings must be inconclusive - a lens that did not run cannot certify clean');
    assert(detect.decideVerdict({ a: ran, b: ran, c: ran, d: failed }, 0, 0) === 'inconclusive', 'three-of-four lenses with zero findings still cannot certify clean');
    ...
    assert(detect.decideVerdict({ a: ran, skipped: { attempted: false, available: false } }, 0, 0) === 'clean', 'a lens that was never attempted is not a coverage gap');
    // ---------------------------------------------------------------------
    ...
    assert(detect.exitCodeFor('inconclusive') !== detect.exitCodeFor('blocked'), 'inconclusive must be distinguishable from findings by exit code alone');
    // ---------------------------------------------------------------------
    ...
    assert(defectRules.some((r) => r.includes('gradient-text')), `the planted gradient-text finding must be emitted, got [${defectRules.join(', ')}]`);
    ...
    assert(clean.json.findings.length === 0, 'clean fixture must emit zero findings');
    ...
    assert(dead.json.unavailableReasons.length > 0, 'an inconclusive verdict must say WHY the lens did not run');
    // ---------------------------------------------------------------------
    ...
    for (const name of ['objective', 'subjective']) {
        ...
        assert(typeof noRenderLenses[name].reason === 'string', `a skipped ${name} lens must say WHY it was skipped`);
    }
    ...
    assert(noRender.json.unavailableReasons.length === 0, 'a skipped lens is not an unavailable lens');
    ...
    // ---------------------------------------------------------------------
    const heroRule = (0, product_rule_registry_1.getRuleById)('anti-pattern.hero-metric-template');
    assert(heroRule !== undefined, 'anti-pattern.hero-metric-template must exist in the registry');
    assert(heroRule.severity === 'minor', `this test encodes the registry override; hero-metric-template is now '${heroRule.severity}' - re-check banSeverity in the CLI`);
    const gradientRule = (0, product_rule_registry_1.getRuleById)('anti-pattern.gradient-text');
...
main();
//# sourceMappingURL=detect-cli.test.js.map