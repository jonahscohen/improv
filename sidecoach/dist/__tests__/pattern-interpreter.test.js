...
const pattern_spec_1 = require("../validators/pattern-spec");
let passed = 0;
const failures = [];
...
function ctxOf(css, markup = '') {
    const files = [];
    if (css)
        files.push({ path: 'styles.css', sourceKind: 'css', cssText: css, markup: '', evidenceKindsPresent: ['css'] });
    if (markup)
        files.push({ path: 'index.html', sourceKind: 'html', cssText: '', markup, evidenceKindsPresent: ['markup'] });
...
for (const bad of ['(a+)+$', '(a*)*', '(\\d+)+', '((a+))+', '(a(b+))+', '(a+){2,}', '(a)\\1', 'a'.repeat(1001)]) {
    ok((0, pattern_spec_1.screenRegexSource)(bad).safe === false, `screen REJECTS catastrophic/backref/overlong source: ${bad.slice(0, 24)}`);
}
for (const good of ['\\d+', '[a-z]+', 'font-family\\s*:\\s*[^;{}]+', 'cubic-bezier\\([^)]*\\)', '(a|b)+', '(?:foo|bar)+', '(a+)?', '<img\\b(?![^>]*\\balt=)']) {
    ok((0, pattern_spec_1.screenRegexSource)(good).safe === true, `screen ADMITS ordinary safe source: ${good}`);
}
// star-height >= 2 names the reason (so a preflight file is legible).
ok(/nested unbounded quantifier/.test((0, pattern_spec_1.screenRegexSource)('(a+)+').reason || ''), 'nested-quantifier rejection names star-height');
// ----------------------------------------------------------------------------
...
const firePos = (0, pattern_interpreter_1.interpretPatternSpec)(bounceSpec, ctxOf('.x { transition: transform .3s cubic-bezier(0.5, 1.6, 0.4, 1); }'));
...
const guardNotMet = (0, pattern_interpreter_1.interpretPatternSpec)(bounceSpec, ctxOf('.x { transition: transform .3s cubic-bezier(0.16, 1, 0.3, 1); }'));
eq(guardNotMet.status, 'pass', 'CLEAN when the defect regex matches but the numeric guard is NOT met (no overshoot)');
const noDefect = (0, pattern_interpreter_1.interpretPatternSpec)(bounceSpec, ctxOf('.x { transition: transform .3s ease; }'));
eq(noDefect.status, 'pass', 'CLEAN on the negative fixture (applicable, no cubic-bezier defect)');
const notApp = (0, pattern_interpreter_1.interpretPatternSpec)(bounceSpec, ctxOf('.x { color: #333; }'));
...
const unknownEngine = (0, pattern_interpreter_1.interpretPatternSpec)({ ...bounceSpec, engine: 'python-ast' }, ctxOf('.x { transition: transform .3s cubic-bezier(0.5, 1.6, 0.4, 1); }'));
...
const unknownPredicate = (0, pattern_interpreter_1.interpretPatternSpec)({ ...bounceSpec, defect: { anyOf: bounceSpec.defect.anyOf, numericGuard: { predicateId: 'not-a-real-predicate', threshold: 1 } } }, ctxOf('.x { transition: transform .3s cubic-bezier(0.5, 1.6, 0.4, 1); }'));
...
const noEvidence = (0, pattern_interpreter_1.interpretPatternSpec)(bounceSpec, { cssText: '', markup: '', files: [] });
...
eq((0, pattern_interpreter_1.interpretPatternSpec)(noGuardSpec, ctxOf('.x { color: red !important; }')).status, 'fail', 'guard-less presence spec FIRES');
eq((0, pattern_interpreter_1.interpretPatternSpec)(noGuardSpec, ctxOf('.x { color: red; }')).status, 'pass', 'guard-less presence spec is CLEAN when the pattern is absent');
// ----------------------------------------------------------------------------
...
const badFlagsNum = (0, pattern_interpreter_1.interpretPatternSpec)({ ...bounceSpec, defect: { anyOf: [{ pattern: 'cubic-bezier\\([^)]*\\)', flags: 123 }], numericGuard: bounceSpec.defect.numericGuard } }, ctxOf(overshootCss));
eq(badFlagsNum.status, 'inconclusive', 'a non-string flags value => inconclusive (no silent drop, no false pass)');
const badFlagsChar = (0, pattern_interpreter_1.interpretPatternSpec)({ ...bounceSpec, defect: { anyOf: [{ pattern: 'cubic-bezier\\([^)]*\\)', flags: 'g' }], numericGuard: bounceSpec.defect.numericGuard } }, ctxOf(overshootCss));
...
function runInterpChild(specLiteral, ctxLiteral, env = {}) {
    const script = `
  ...
  `;
    try {
        const out = (0, child_process_1.execFileSync)(process.execPath, ['-e', script], { timeout: 8000, encoding: 'utf8', env: { ...process.env, ...env } }).trim();
...
const ambiguous = runInterpChild(`{ specVersion:1, engine:'static-css-regex', applicability:{ anyOf:['a'], scope:'css' }, defect:{ anyOf:[{ pattern:'^(a|aa)*$' }] }, message:'ambiguous alternation' }`, `{ cssText: 'a'.repeat(40) + 'b', markup:'', files:[] }`);
...
const nested = runInterpChild(`{ specVersion:1, engine:'static-css-regex', applicability:{ anyOf:['a'], scope:'css' }, defect:{ anyOf:[{ pattern:'(a+)+$' }] }, message:'nested quantifier' }`, `{ cssText: 'a'.repeat(50000) + '!', markup:'', files:[] }`);
...
const noRe2 = runInterpChild(`{ specVersion:1, engine:'static-css-regex', applicability:{ anyOf:['transition\\\\s*:'], scope:'css' }, defect:{ anyOf:[{ pattern:'cubic-bezier\\\\([^)]*\\\\)', flags:'i' }], numericGuard:{ predicateId:'cubic-bezier-overshoot', threshold:0.1 } }, message:'overshoot', evidenceScope:'css' }`, `{ cssText: '.x { transition: transform .3s cubic-bezier(0.5, 1.6, 0.4, 1); }', markup:'', files:[] }`, { SIDECOACH_DISABLE_RE2: '1' });
...
//# sourceMappingURL=pattern-interpreter.test.js.map