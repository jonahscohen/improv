...
const fired = (html) => (0, taste_validator_1.validateTaste)(wrap(html)).some(v => v.ruleId === 'taste/fabricated-svg');
// The library outline convention, shared by every positive fixture below.
const ICON_ATTRS = 'viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"';
// ---------------------------------------------------------------------------
...
const HAMBURGER_LINES = '<button class="topbar-toggle" type="button" aria-label="Toggle sidebar">' +
    `<svg ${ICON_ATTRS}>` +
    '<line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="18" x2="20" y2="18"/></svg></button>';
const CHECKBOX_RECT_POLYLINE = `<svg ${ICON_ATTRS}><rect x="3" y="3" width="18" height="18" rx="2"/><polyline points="8 12 11 15 16 9"/></svg>`;
const CLOCK_CIRCLE_LINES = '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" aria-hidden="true">' +
...
check('primitive icon with data-icon-source marker is NOT flagged', !fired(HAMBURGER_LINES.replace('<svg ', '<svg data-icon-source="lucide" ')));
check('primitive icon with lucide- class marker is NOT flagged', !fired(HAMBURGER_LINES.replace('<svg ', '<svg class="lucide-menu" ')));
check('primitive icon with a <!-- source: --> comment is NOT flagged', !fired(HAMBURGER_LINES.replace('<line', '<!-- source: lucide/menu.svg --><line')));
// ---------------------------------------------------------------------------
...
check('  anchor: same shape on an icon grid, declared decorative, IS flagged', fired(`<svg ${ICON_ATTRS}><rect width="24" height="24" rx="12"/><circle cx="12" cy="12" r="4"/></svg>`));
// 2. A chart: many rects, a real data viewBox, axis text.
const BAR_CHART = `<svg viewBox="0 0 320 180" stroke="currentColor" stroke-linecap="round">` +
...
check('  anchor: the same bars on a 24 grid with no text IS flagged', fired(`<svg ${ICON_ATTRS}><rect x="2" y="8" width="4" height="12"/><rect x="10" y="4" width="4" height="16"/><rect x="18" y="12" width="4" height="8"/></svg>`));
...
const WORDMARK = '<svg viewBox="0 0 150 54" stroke="currentColor" stroke-linejoin="round"><polygon points="0,0 40,0 40,54 0,54"/>' +
    '<polyline points="50,10 90,10 90,44"/></svg>';
check('wordmark: non-square viewBox is NOT flagged', !fired(WORDMARK));
check('  anchor: the same primitives on a square 24 grid IS flagged', fired(`<svg ${ICON_ATTRS}><polygon points="0,0 8,0 8,8 0,8"/><polyline points="10,2 18,2 18,10"/></svg>`));
// 5. A decorative/brand shape on a large square grid.
const BRAND_SQUARE = '<svg viewBox="0 0 100 100" fill="#DC2618"><circle cx="50" cy="50" r="40"/><circle cx="50" cy="50" r="20"/></svg>';
check('brand mark: 100 grid with literal fill is NOT flagged', !fired(BRAND_SQUARE));
check('  anchor: the same circles on a 24 grid, declared decorative, IS flagged', fired(`<svg ${ICON_ATTRS}><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="5"/></svg>`));
// 6. CODEX REGRESSION BLOCK. These three broke the earlier draft of this rule. Kept verbatim.
const CODEX_SPARKLINE = '<svg viewBox="0 0 24 24" role="img" aria-label="Revenue trend" fill="none" stroke="currentColor">' +
    '<polyline points="1,20 6,12 11,16 16,6 23,9"/><circle cx="23" cy="9" r="1.5"/></svg>';
const CODEX_MICRO_BARCHART = '<svg viewBox="0 0 24 24" role="img" aria-label="7-day usage" fill="currentColor">' +
...
const CODEX_SMALL_LOGO = '<svg viewBox="0 0 24 24" aria-label="Acme" fill="currentColor">' +
...
const CODEX_SPARKLINE_ROUNDED = '<svg viewBox="0 0 24 24" role="img" aria-label="Revenue trend" fill="none" stroke="currentColor" ' +
...
const CODEX_LABELLED_CHART_WITH_HIDDEN_LAYER = '<svg viewBox="0 0 24 24" role="img" aria-label="Revenue rose from 42 to 57" fill="none" stroke="currentColor">' +
...
check('  anchor: the same drawing with the ROOT declared decorative IS flagged', fired('<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor">' +
    '<line x1="2" y1="20" x2="22" y2="20"/><polyline points="2 18 7 14 12 16 17 8 22 6"/></svg>'));
check('root scope: aria-hidden on a DESCENDANT does not make the graphic chrome', !fired('<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><g aria-hidden="true"><line x1="2" y1="2" x2="22" y2="2"/><line x1="2" y1="8" x2="22" y2="8"/></g></svg>'));
...
check('  anchor: the SAME sparkline declared decorative (aria-hidden) IS flagged', fired('<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">' +
    '<polyline points="1,20 6,12 11,16 16,6 23,9"/><circle cx="23" cy="9" r="1.5"/></svg>'));
// A single primitive is a shape, not an icon, even in the stroke-icon idiom.
check('sparkline: single polyline on an icon grid is NOT flagged', !fired('<svg viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><polyline points="1,20 6,12 11,16 16,6 23,9"/></svg>'));
check('  anchor: adding a second primitive to the same svg IS flagged', fired('<svg viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><polyline points="1,20 6,12 11,16 16,6 23,9"/><circle cx="23" cy="9" r="2"/></svg>'));
// 7. Animated / gradient illustration: brand motion, not UI chrome.
check('animation: <animate> child is NOT flagged', !fired(`<svg ${ICON_ATTRS}><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="4"><animate attributeName="r" values="4;8;4" dur="2s"/></circle></svg>`));
check('illustration: gradient/defs is NOT flagged', !fired(`<svg ${ICON_ATTRS}><defs><linearGradient id="g"/></defs><circle cx="12" cy="12" r="10" fill="url(#g)"/><rect x="2" y="2" width="6" height="6"/></svg>`));
// ---------------------------------------------------------------------------
// SCOPE LIMITS - accepted misses, asserted so a later loosening is deliberate.
// ---------------------------------------------------------------------------
check('scope limit: icon-grid primitives painted with literal colors are NOT flagged', !fired('<svg width="24" height="24" aria-hidden="true"><line x1="0" y1="0" x2="24" y2="24" stroke="#333"/><line x1="0" y1="24" x2="24" y2="0" stroke="#333"/></svg>'));
check('  anchor: the same two lines with currentColor IS flagged', fired('<svg width="24" height="24" aria-hidden="true"><line x1="0" y1="0" x2="24" y2="24" stroke="currentColor"/><line x1="0" y1="24" x2="24" y2="0" stroke="currentColor"/></svg>'));
check('scope limit: a primitive icon with NO aria-hidden is NOT flagged (accepted recall cost)', !fired('<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="20" y2="12"/></svg>'));
check('  anchor: the same icon declared decorative IS flagged', !!fired('<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="20" y2="12"/></svg>'));
check('scope limit: a FILLED primitive icon IS flagged when declared decorative', fired('<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="12" cy="12" r="9"/><rect x="10" y="6" width="4" height="8"/></svg>'));
// Grid boundary, side: a large SQUARE canvas is illustrative territory. Guarded by ICON_GRID_MAX.
check('scope limit: a 200x200 square canvas is NOT flagged', !fired('<svg viewBox="0 0 200 200" fill="none" stroke="currentColor" aria-hidden="true"><circle cx="100" cy="100" r="80"/><circle cx="100" cy="100" r="40"/></svg>'));
check('  anchor: the same two circles on a 24 grid IS flagged', fired(`<svg ${ICON_ATTRS}><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="5"/></svg>`));
// Grid boundary, aspect: a small NON-square box is an ornament or rule. Guarded by w === h.
check('scope limit: a small non-square 24x12 box is NOT flagged', !fired('<svg viewBox="0 0 24 12" fill="none" stroke="currentColor" aria-hidden="true"><line x1="0" y1="6" x2="24" y2="6"/><line x1="0" y1="9" x2="24" y2="9"/></svg>'));
check('  anchor: the same two lines in a square 24x24 box IS flagged', fired(`<svg ${ICON_ATTRS}><line x1="0" y1="6" x2="24" y2="6"/><line x1="0" y1="9" x2="24" y2="9"/></svg>`));
// viewBox parsing: comma-separated is legal and must not fall through to the width/height branch.
check('parsing: comma-separated viewBox="0,0,24,24" is recognised as an icon grid', fired('<svg viewBox="0,0,24,24" fill="none" stroke="currentColor" aria-hidden="true"><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="20" y2="12"/></svg>'));
check('  anchor: the same comma viewBox at 200 is NOT flagged', !fired('<svg viewBox="0,0,200,200" fill="none" stroke="currentColor" aria-hidden="true"><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="20" y2="12"/></svg>'));
// ---------------------------------------------------------------------------
// COMPOUND PATH - the second gap.
// ---------------------------------------------------------------------------
// Verbatim from sidecoach/reference/responsive-foundation.md at 8ae761a4.
const COMPOUND_HAMBURGER = '<svg width="24" height="24" aria-hidden="true">' +
...
check('  anchor: a single-subpath d="" of the same shape is NOT flagged', !fired('<svg width="24" height="24" aria-hidden="true"><path d="M3 6h18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>'));
// ---------------------------------------------------------------------------
...
const LOCKED_FIXTURE = '<svg viewBox="0 0 24 24"><path d="M3 12a9 9 0 1 0 18 0 9 9 0 0 0-18 0zoooooong"/>' +
...
check('regression: a single short unmarked path still does NOT fire', !fired(`<svg ${ICON_ATTRS}><path d="M5 12h14"/></svg>`));
check('regression: two unmarked paths still fire', fired(`<svg ${ICON_ATTRS}><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>`));
check('regression: a marked 2-path icon still does NOT fire', !fired('<svg class="lucide-arrow-right" viewBox="0 0 24 24"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>'));
...
check('mixed: one path plus one line on an icon grid IS flagged', fired(`<svg ${ICON_ATTRS}><path d="M5 12h14"/><line x1="4" y1="4" x2="20" y2="4"/></svg>`));
const CORPUS = [
...
for (const c of CORPUS) {
    ...
    if (c.icon && hit)
        ...
        fn++;
        mistakes.push(`  MISS (false negative): ${c.name}`);
    }
    else if (!c.icon && hit) {
        fp++;
        mistakes.push(`  FALSE POSITIVE: ${c.name}`);
    }
    else
        tn++;
}
...
//# sourceMappingURL=taste-validator-primitive-icons.test.js.map