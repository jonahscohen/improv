...
const PAGE_SCANNING_FLOWS = new Set(['flowK_multi_lens_audit']);
 ...
 */
function userFacingValidationFailures(validations) {
...
function isPageShapedTarget(resolved, target) {
 ...
 */
function unscannedAuditSummary(t) {
    return {
        renderUrl: null,
        lenses: {
...
function notRenderedNoticeFor(t) {
 ...
 */
function unscannedTargetForEarlyCommand(command, target, cwd) {
    const raw = (target ?? '').trim();
    if (!raw)
        return null;
    const resolved = (0, audit_rendered_1.resolveAuditTarget)(raw, { cwd });
    if (!isPageShapedTarget(resolved, raw))
        return null;
...
function unscannedPageTarget(command, target, resolved) {
    return {
...
class FlowExecutionEngine {
    ...
    recordFlowWithMemory(result, rendered) {
        const flowHistory = (0, flow_history_1.getFlowHistory)();
        const entry = {
    ...
    async process(utterance, context = {}) {
        ...
        if (commandMatch.isCommand) {
            ...
            // unchanged.
            let unrenderedAuditTarget = null;
            ...
            if ((commandMatch.target ?? '').trim()) {
                ...
                const projectPath = context.projectPath || process.cwd();
                const resolved = (0, audit_rendered_1.resolveAuditTarget)(commandMatch.target, { cwd: projectPath });
                if (commandMatch.command === 'audit') {
                    if (resolved.renderable) {
                        const audit = await (0, audit_rendered_1.runRenderedAudit)(resolved.renderUrl, { projectPath });
                    ...
                    if (resolved.kind === 'missing' && /\.(html?|xhtml)$/i.test(commandMatch.target)) {
                ...
                else if (commandMatch.command && isPageShapedTarget(resolved, commandMatch.target)) {
                    ...
                    unrenderedAuditTarget = unscannedPageTarget(commandMatch.command, commandMatch.target, resolved);
                }
            }
            ...
            for (const flowId of commandMatch.flowIds) {
                ...
                try {
                    ...
                    if (handler.canExecute(enrichedCtx)) {
                        ...
                        this.recordFlowWithMemory(result, unrenderedAuditTarget && PAGE_SCANNING_FLOWS.has(String(flowId)) ? false : undefined);
                        flowResults.push(result);
                        if (!detectedFlow) {
                            detectedFlow = {
                                flowId,
            ...
            return {
                ...
                panel: (0, panel_renderer_1.renderSidecoachPanel)((0, panel_model_1.assemblePanelModel)({
                    flowResults,
    ...
    toUnrenderableAuditResult(resolved, command) {
    ...
    toRenderedAuditResult(audit, command, resolved) {
        ...
        if (audit.verdict === 'inconclusive') {
            ...
            const emptyShell = !partial && audit.unavailableReasons.some((r) => r.includes('rendered document is empty') || r.includes('probe failed'));
        ...
        try {
            this.recordFlowWithMemory(flowResult, audit.rendered);
        }
        ...
        return {
            ...
            panel: (0, panel_renderer_1.renderSidecoachPanel)((0, panel_model_1.assemblePanelModel)({ flowResults: [flowResult], report, confidence: 1.0, auditPartial: audit.unavailableReasons.length > 0 })),
        };
    }
