...
exports.VERB_REGISTRY = {
    ...
    audit: {
        command: 'audit',
        description: 'Run systematic technical quality checks and generate a comprehensive report. Code-level audit across 5 dimensions; do not fix issues, document them.',
    ...
    critique: {
        command: 'critique',
        description: 'Design review via two independent assessments (LLM design director + automated detector). Combined into a single honest report with heuristic scoring, anti-patterns verdict, and persona red flags.',
    ...
    shape: {
        command: 'shape',
        description: 'Shape the UX and UI for a feature before any code is written. Produces a structured design brief that guides implementation through discovery, not guesswork.',
    ...
    onboard: {
        command: 'onboard',
        description: 'Design first-run flows that get users to first value as fast as possible. Onboarding\'s job is not to teach the product; it is to reach the aha moment that proves the product is worth the time.',
    ...
    animate: {
        command: 'animate',
        description: 'Add motion that conveys state, gives feedback, and clarifies hierarchy. Cut motion that exists only for decoration; animation fatigue is a real cost.',
    ...
    bolder: {
        command: 'bolder',
        description: 'Increase visual impact and personality through stronger hierarchy, committed scale, and decisive type. Reject the AI defaults (cyan-purple gradients, glassmorphism, neon accents) first, then amplify.',
    ...
    delight: {
        command: 'delight',
        description: 'Find the moments where personality and unexpected polish would turn a functional interface into one users remember. Add only where the moment earns it; delight everywhere reads as noise.',
    ...
    layout: {
        command: 'layout',
        description: 'Find the layout\'s actual problem (monotone spacing, weak hierarchy, identical card grids, the centered-stack default) and fix the structure, not the surface. Space is the most underused design tool.',
    ...
    overdrive: {
        command: 'overdrive',
        description: 'Push an interface past conventional limits. Use the full power of the browser to make any part of an interface feel extraordinary - a table that handles a million rows, a dialog that morphs from its trigger, a cinematic page transition.',
    ...
    quieter: {
        command: 'quieter',
        description: 'Reduce visual intensity in designs that are too loud, aggressive, or overstimulating without losing personality or making the result generic. Subtlety needs precision; quiet is harder than bold.',
    ...
    clarify: {
        command: 'clarify',
        description: 'Find the unclear, confusing, or poorly written interface text and rewrite it. Vague copy creates support tickets and abandonment; specific copy gets users through the task.',
    ...
    harden: {
        command: 'harden',
        description: 'Make a design production-ready against the inputs, errors, languages, and network conditions real users will throw at it. Designs that only work with perfect data are not done.',
    ...
    adapt: {
        command: 'adapt',
        description: 'Adapt an existing design to a different context: another screen size, device, platform, or use case. The trap is treating adaptation as scaling. The job is rethinking the experience for the new context.',
    ...
    distill: {
        command: 'distill',
        description: 'Strip a design to its essence. Remove anything that does not earn its place: redundant elements, repeated information, decorative noise, cosmetic complexity.',
    ...
    optimize: {
        command: 'optimize',
        description: 'Identify the actual performance bottleneck for THIS interface, fix it, then measure. Performance is a feature; do not optimize what is not slow.',
    ...
    extract: {
        command: 'extract',
        description: 'Identify reusable patterns, components, and design tokens, then extract and consolidate them into the design system for systematic reuse. Premature abstraction is worse than duplication; extract what is used 3+ times with the same intent.',
...
function getVerbEntry(command) {
    return exports.VERB_REGISTRY[command];
}
//# sourceMappingURL=verb-command-registry.js.map