...
exports.TRUTH_BAN_NO_STRING = exports.TRUTH_BAN_ONE_STRING = exports.TRUTH_BANS = exports.CALIBRATION_BANS = exports.MEDIUM_BANS = exports.OPENAI_SIZES = void 0;
exports.detectRole = detectRole;
exports.detectSurface = detectSurface;
exports.briefWantsRaster = briefWantsRaster;
exports.resolveConcept = resolveConcept;
exports.resolveComposition = resolveComposition;
exports.flattenPalette = flattenPalette;
exports.resolveInk = resolveInk;
exports.sizeForAspect = sizeForAspect;
exports.compileImageBrief = compileImageBrief;
exports.summarizeCompilation = summarizeCompilation;
...
function detectRole(text) {
    ...
    const insideCancelled = (start, end) => cancelled.some(([cs, ce]) => start >= cs && end <= ce);
    for (const [role, words] of ROLE_KEYWORDS) {
        for (const word of words) {
            ...
            while ((m = re.exec(t)) !== null) {
                ...
                if (!insideCancelled(start, end))
                    return { role, matched: word };
 ...
 */
function briefWantsRaster(text) {
...
function seedOf(text) {
    const hex = (0, crypto_1.createHash)('sha256').update(text).digest('hex').slice(0, 8);
 ...
 */
function resolveConcept(brief, ctx) {
    if (brief.directionId) {
        const explicit = (0, direction_deck_1.directionById)(brief.directionId);
        if (explicit)
            return { concept: explicit, basis: `explicit direction id ${explicit.id}` };
    }
    const approach = normalize(ctx.approach || '');
    if (approach) {
        const byId = direction_deck_1.DIRECTION_DECK.find((d) => d.id === approach.replace(/\s+/g, '-'));
        if (byId)
            return { concept: byId, basis: `project approach "${ctx.approach}" matched deck id ${byId.id}` };
    ...
    const pool = direction_deck_1.DIRECTION_DECK.filter((d) => d.id !== direction_deck_1.MODEL_DEFAULT_ID && d.axis !== 'motion');
    const picked = pool[seedOf(`${brief.text}|${ctx.register || ''}|${ctx.approach || ''}`) % pool.length];
    return {
        concept: picked,
        basis: `no world was named, so it was drawn deterministically from the ${pool.length} deck entries that are neither ${direction_deck_1.MODEL_DEFAULT_ID} nor motion-axis`,
    };
}
// ---------------------------------------------------------------------------
 ...
 */
function resolveComposition(brief, role, surface) {
    if (brief.compositionId) {
        const explicit = (0, image_composition_catalog_1.compositionById)(brief.compositionId);
        if (explicit)
            return { composition: explicit, basis: `explicit staging id ${explicit.id}` };
    }
    const forRole = (0, image_composition_catalog_1.compositionsForRole)(role);
    if (forRole.length === 0) {
        ...
        return {
            composition: image_composition_catalog_1.IMAGE_COMPOSITION_CATALOG[0],
 ...
 */
function flattenPalette(tokens, limit = 8) {
    const out = [];
    const walk = (node, trail) => {
        ...
        if (typeof node === 'string') {
            const v = node.trim();
            if (HEX_RE.test(v))
                out.push({ path: trail.join('.'), hex: v.toLowerCase() });
            return;
        }
...
exports.OPENAI_SIZES = ['1024x1024', '1536x1024', '1024x1536'];
...
function sizeForAspect(aspect, policy = {}) {
    ...
    if (policy.allowed && policy.allowed.length > 0) {
        ...
        for (const e of parsed) {
            ...
            if (Math.abs(r - ratio) < Math.abs(best.dims.width / best.dims.height - ratio))
                best = e;
        }
        return best.s;
    }
    ...
    if (ratio === null)
        return `${align8(maxEdge)}x${align8(maxEdge)}`;
    return ratio >= 1
...
exports.TRUTH_BAN_ONE_STRING = 'no lettering other than the one permitted string named in the TEXT section';
exports.TRUTH_BAN_NO_STRING = 'no lettering at all, anywhere, in any script';
// ---------------------------------------------------------------------------
// The compiler
// ---------------------------------------------------------------------------
const DIMENSIONS = [
 ...
 */
function compileImageBrief(brief, ctx = {}) {
    const detectedRole = detectRole(brief.text);
    const role = brief.role ?? detectedRole?.role ?? null;
    if (!role)
        return null;
    const fromBrief = [];
    const supplied = [];
    const missing = [];
    if (brief.role)
        fromBrief.push(`role: given explicitly as ${brief.role}`);
    else if (detectedRole)
        supplied.push(`role: read as ${role} from the word "${detectedRole.matched}" in the brief`);
    ...
    if (brief.directionId)
        fromBrief.push(`world: ${concept.id}`);
    else
        supplied.push(`world: ${concept.id} (${conceptBasis})`);
    const { composition, basis: compositionBasis } = resolveComposition(brief, role, surface);
    if (brief.compositionId)
        fromBrief.push(`staging: ${composition.id}`);
    else
        supplied.push(`staging: ${composition.id} (${compositionBasis})`);
    supplied.push(`focal: from the staging's Focal rule (${composition.id})`);
    const policy = ctx.sizePolicy || { allowed: exports.OPENAI_SIZES };
    const size = brief.size && parsePixels(brief.size) ? brief.size : sizeForAspect(composition.aspect, policy);
    if (brief.size && parsePixels(brief.size))
        fromBrief.push(`size: ${size} given explicitly`);
    else
        supplied.push(`size: ${size} derived from the staging's ${composition.aspect} aspect under the ${policy.allowed ? `${policy.allowed.length}-geometry provider list` : `${policy.maxEdge ?? 1024}px max-edge`} policy`);
    ...
    const inkRegion = pixels ? (0, image_composition_catalog_1.inkZoneToPixels)(composition.inkZone, pixels.width, pixels.height) : null;
    const contract = { size, format: 'png' };
    if (brief.alpha) {
    ...
    if (ink && inkRegion) {
        ...
        supplied.push(`contrast target: ${ink.hex} (from ${ink.path}) must clear 4.5:1 over the ${inkRegion.w}x${inkRegion.h} region at ${inkRegion.x},${inkRegion.y}, which is the staging's own ink zone`);
    }
    else if (!ink) {
        missing.push('DESIGN.md names no text colour, so the overlay-contrast check cannot be contracted; the asset will report contrast as not checked rather than as passed');
    }
    else {
    ...
    supplied.push(`negative constraints: ${exports.MEDIUM_BANS.length + (palette.length > 0 ? 0 : exports.CALIBRATION_BANS.length) + exports.TRUTH_BANS.length + 1} standing bans plus the world's own refusal and the staging's own refusal`);
    supplied.push(ctx.productName
    ...
    const digest = (0, crypto_1.createHash)('sha256').update(prompt).digest('hex').slice(0, 12);
...
function buildSections(input) {
    ...
    sections.push({
        ...
        body: [
            ...
            ...(palette.length > 0 ? [] : exports.CALIBRATION_BANS),
            ...exports.TRUTH_BANS,
            ctx.productName ? exports.TRUTH_BAN_ONE_STRING : exports.TRUTH_BAN_NO_STRING,
            ...antiRefs,
        ].join('; '),
    });
    sections.push({
...
function summarizeCompilation(c) {
    return `${c.role} for a ${c.surface} surface, staged as ${c.composition.id} in the ${c.concept.id} world, ${c.contract.size}${c.contract.ink ? `, contrast contracted at ${c.contract.minContrast}:1 for ${c.contract.ink}` : ', contrast NOT contracted'}`;
}
//# sourceMappingURL=image-brief-compiler.js.map