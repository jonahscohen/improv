---
name: sidecoach-session-20260801
...
**Guidance:**
- CRAFT BRIEF - what good looks like on the points this project missed on the five audit dimensions.
- Selected from the 8 rules that FAILED here, hardest-first, capped at 6 taught below. Rules that passed are omitted. Every finding is still enumerated under the brief.
- Read this before acting: the work is the principle, not the rule name.
- 
- 1. TOKEN-DRIVEN INTERACTIVE STATES
...
-    Do:   Replace the literal with a token reference - background: var(--c-brand-hover) rather than #2563eb. If the hover shade does not exist, add it to the ramp rather than inlining it once; a one-off hex in a state rule is how a second source of truth starts. The same applies to focus ring colour and disabled surfaces.
...
-    Do:   Set width and height attributes on every <img> to the intrinsic pixel size (CSS can still resize it), or declare aspect-ratio in CSS. Both approaches let the browser compute the box from the ratio before load. For a hero image add fetchpriority="high" as well so it is not queued behind lazy siblings.
-      <img src="hero.jpg" width="1600" height="900" alt="..." fetchpriority="high">
-    Source: src/validators/checks/page-quality-checks.ts
- 
...
-    Do:   Walk each control and add the reachable states. Do not signal disabled with opacity alone (it usually breaks contrast). Prefer a skeleton over a spinner for loading, since it also holds the layout.
-    Source: src/design-laws.ts (SHARED_DESIGN_LAWS.interaction)
- 
- 4. NO BOUNCE OR ELASTIC EASING
...
-    Do:   Split into logical groups (title, description, actions) and offset each by roughly 100ms; for a display title consider individual words at about 80ms. Combine opacity, a small translateY, and blur 4px to 0. Keep animated blur at or under 8px, with 2-4px the useful range.
-    Source: claude/skills/tactical-polish/animations.md
- 
- FINDINGS - what was actually measured on this project.
- SCOPE: 2 source file(s) inspected against the all rule classes from static source only. 12 rule(s) need a live render and are reported as inconclusive, not as passing.
- 
- 8 failing rule(s) in this flow's scope, each with its measured message and fix:
- - [blocker] [theming/token-driven-interactive-state] interactive state(s) use hardcoded hex while tokens exist: :hover/:active state uses hardcoded hex (#B71D11) while the file defines CSS custom properties. Derive the interactive state from a token (e.g. var(--c-brand-red-hover)) so theming and dark mode propagate. Selector: ".install-block__copy:hover" | :hover/:active state uses hardcoded hex (#B71D11) while the file defines CSS custom properties. Derive the interactive state from a token (e.g. var(--c-brand-red-hover)) so theming and dark mode propagate. Selector: ".install-block__copy:hover, .install-block__copy.is-copied" (background: #B71D11;, background: #B71D11;
    color: var(--c-text-inverse);) -> Derive the interactive state from a token, e.g. var(--c-brand-hover)
- - [major] [perf/image-dimensions] 2 image(s) lack width+height/aspect-ratio (causes layout shift) (index.html:32, index.html:1152) -> Set width and height (or aspect-ratio) on <img> so the browser reserves space before load
- - [major] [polish/state-completeness] 3/8 component states defined (index.html:10) -> Define all 8 component states
- - [minor] [anti-pattern/bounce-easing] bounce/elastic easing: overshoot easing cubic-bezier(0.34, 1.56, 0.64, 1) - real objects decelerate, they do not spring back (styles.css:90) -> Use a decelerating curve (ease-out quart/quint/expo, e.g. cubic-bezier(0.16, 1, 0.3, 1)) so motion settles instead of overshooting
- - [minor] [perf/image-lazy-load] 1 below-the-fold image(s) not lazy-loaded (index.html:1152) -> Add loading="lazy" to images past the first/hero (or fetchpriority="high" if intentionally eager) so below-the-fold images defer their fetch
- - [minor] [polish/staggered-enter] animations should use stagger delays (index.html:716) -> Apply animation-delay: calc(30ms * var(--index))
- - [minor] [theming/color-scheme-dark] dark mode without a color-scheme: dark declaration (index.html:31, index.html:1151, styles.css:1094) -> Add color-scheme: dark (or light dark) so native controls, scrollbars, and form fields adapt
- - [advisory] [polish/image-outline-neutral] image outlines should use neutral transparency (index.html:32) -> border: 1px solid rgba(0,0,0,0.1)
- 
...
**Guidance:**
- CRAFT BRIEF - what good looks like on the points this project missed on design critique.
- Selected from the 8 rules that FAILED here, hardest-first, capped at 6 taught below. Rules that passed are omitted. Every finding is still enumerated under the brief.
- Read this before acting: the work is the principle, not the rule name.
- 
- 1. TOKEN-DRIVEN INTERACTIVE STATES
...
**Guidance:**
- CRAFT BRIEF - the standard to build to on token-based iteration.
- This flow runs before there is a finished artifact to measure, so the 4 points below are the up-front standard - not selected by measuring this project. 2 rule(s) in this domain already fail on the current project and are listed under the brief.
- Read this before acting: the work is the principle, not the rule name.
- 
- 1. INTERACTIVE STATES ARE TOKEN-DRIVEN
...
-    Do:   Replace the literal with a token: background: var(--c-brand-hover) rather than #2563eb. If the hover shade does not exist yet, add it to the ramp rather than inlining it once. The same applies to focus ring colour and to disabled surfaces.
-    Source: src/validators/checks/theming-checks.ts
- 
- 2. ONE OR TWO RADIUS TOKENS
...
-    Do:   Name by role and tier: --text-body, --text-display, --surface-raised, --border-subtle, --shadow-md, --radius-card. Keep a two-layer system where semantic tokens reference primitives (--brand-500) rather than restating hex. Reference tokens from components as var(--token) so a theme change is one edit.
-    Source: src/design-laws.ts (SHARED_DESIGN_LAWS.typography)
- 
- FINDINGS - what was actually measured on this project.
- SCOPE: 2 source file(s) inspected against the theming rule classes from static source only. 12 rule(s) need a live render and are reported as inconclusive, not as passing.
- 
- 2 failing rule(s) in this flow's scope, each with its measured message and fix:
- - [blocker] [theming/token-driven-interactive-state] interactive state(s) use hardcoded hex while tokens exist: :hover/:active state uses hardcoded hex (#B71D11) while the file defines CSS custom properties. Derive the interactive state from a token (e.g. var(--c-brand-red-hover)) so theming and dark mode propagate. Selector: ".install-block__copy:hover" | :hover/:active state uses hardcoded hex (#B71D11) while the file defines CSS custom properties. Derive the interactive state from a token (e.g. var(--c-brand-red-hover)) so theming and dark mode propagate. Selector: ".install-block__copy:hover, .install-block__copy.is-copied" (background: #B71D11;, background: #B71D11;
    color: var(--c-text-inverse);) -> Derive the interactive state from a token, e.g. var(--c-brand-hover)
- - [minor] [theming/color-scheme-dark] dark mode without a color-scheme: dark declaration (index.html:31, index.html:1151, styles.css:1094) -> Add color-scheme: dark (or light dark) so native controls, scrollbars, and form fields adapt
- 
...
- Raster asset produced and verified - the brief names no raster asset (no backdrop, texture, plate, portrait, object, thumbnail, scene, or social card), so no image was generated
...
Recorded: 2026-08-01T13:20:49.486Z