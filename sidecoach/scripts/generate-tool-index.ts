...
const DESCRIPTIONS: Record<string, Desc> = {
  ...
  'sidecoach-mine': {
    ...
    exits: '0 success, 2 usage, 3 registry unavailable (build first), 4 write failure, 5 bad --findings file',
    reachedBy: '/sidecoach mine, and the scheduled sidecoach-mine-daily launchd job (precheck/advance are its SRR run/skip gate + cursor on the shared research spine)',
  },
  'sidecoach-consolidate': {
    ...
    exits: '0 success, 1 --check drift, 2 usage, 3 build first (registry/types), 4 write failure, 5 bad --distilled',
    reachedBy: '/sidecoach consolidate (a live session flow)',
  },
  'sidecoach-taste-promote': {
    ...
    exits: '0 ok, 2 usage, 3 no candidate, 4 bad candidate, 5 no token, 6 no TTY, 8 ledger tampered, 9 audit discrepancy, 10 io, 11 replay',
    reachedBy: 'the human review gate (the user types promote-confirm in their REPL, then runs promote)',
  },
  'sidecoach-taste-enforce': {
    ...
    exits: '0 ok, 2 usage, 3 no rule, 4 bad rule, 5 no token, 6 no TTY, 7 precision refused, 8 ledger tampered, 9 audit discrepancy, 10 io, 11 replay, 12 not guidance-promoted, 13 build failed',
    reachedBy: 'the human enforce gate (the user types enforce-confirm in their REPL, then runs enforce)',
  },
  'sidecoach-qa-plan': {
    purpose:
