...
4. **Register the new classes** in `product-rule-registry.ts` (ids, aliases, severities, evidence reqs), regenerate `src/validators.generated.ts` (`scripts/generate-validators.ts`), and update the golden `src/__tests__/product-rule-registry.test.ts` (rows + count). THE PATTERN FOR THIS IS ALREADY PROVEN - see [[session_2026-06-24_sidecoach-tactical-polish-gap-implement]] (registry + codegen + golden + count-assertion tests for the #4/#13 additions). Reuse it.
