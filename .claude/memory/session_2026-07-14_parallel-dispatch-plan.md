...
## Follow-ups factored into the plan (2026-07-14, Jonah directive)
Jonah: "the follow-ups the wave surfaced need to get factored into our work." Updated docs/plans/2026-07-14-parallel-dispatch-plan.md (in the branch):
- Wave 2 is now PARALLEL (disjoint files) with U7 (guardrails) + 3 NEW units:
  - U7b harness false-positives (owns memory-nudge.sh, verify-before-done.sh, bash-guard.sh): the memory-nudge `install`-substring write-token; the memory commit-gate false-blocking worktree executors + re-dirty race + a Bash `cp` of a beat not being recognized as a beat-write clear (all hit repeatedly this session incl. at integration); the verify-before-done exemption missing the repo-source `claude/hooks/` dir (fires on hook-source edits).
  - U11 dependency-map correction (owns docs/dependency-map/): the stale :4832 page (Codex flagged 3x) + reclassify the two false "islands" - test-site-1 and cmux/settings.json are LIVE.
  - U12 test-site-1 fixture-repoint-then-removal: relocate landing.css off test-site-1 into the tests' own fixtures so U2's correctly-blocked removal can complete.
- U8 (cmux research) expanded to also cover the cmux/settings.json retire question U1 surfaced (install.sh:2181 live-symlink + cmux active-detection; the "legacy commented-out" premise was false).
- File-ownership table updated (7b/11/12 added; still collision-free - all disjoint).
These ride into the wave1-debt-burndown branch with the plan doc.

## Wave 2 plan Codex-reviewed (GO) - 2026-07-14
Jonah: "Have Codex review the wave 2 plan before you deploy agents to work on wave 2." 3 rounds via codex-review.py (real Codex):
- Round 1 (151s): 6 findings, all folded. CRITICAL - U11/U12 state-collision (U11 would document test-site-1 LIVE while U12 deletes it in the same wave); HIGH - U12's removal gate still snags on sprint1-integration.test.js.map + a test-site-1 task-list mention in install.sh:203; HIGH - U7 ownership loose around the team-dir orphan fix (the "new hooks" wording didn't cover the existing team-reaper.sh it edits); MEDIUM - U7b's gate-fix wording could be read as weakening the gate; MEDIUM - Wave 2 must base worktrees on the post-Wave-1 MERGE commit not 7eb21eca; LOW - cmux/settings.json bookkeeping still implied U1 island retirement.
- Round 2 (49s): the fixes were in the unit BODIES but the DISPATCH PROMPTS (what agents receive) + U11's body still lagged - 3 alignment gaps. Aligned all (U11 body+prompt: test-site-1 RETIRED/removed-from-map, cmux/settings.json live-pending-U8, runs after U12; U12 prompt: names .js.map + install.sh:203 + the final grep; U7 prompt: exact owned files incl. team-reaper.sh, no other hook).
- Round 3 (20s): GO. Wave 2 dispatch-safe: U7/U7b/U12 in parallel from the post-Wave-1 merge commit, then U11 after U12 is integrated.
Structural outcome: Wave 2 is NOT fully parallel - U11 is sequenced last (state-coupled to U12). U7b carries a 4-part don't-weaken-the-gate invariant. Wave 2 worktrees base = the post-Wave-1 merge commit (so Wave 2 deploys only AFTER Jonah merges Wave 1). Committed to wave1-debt-burndown.
