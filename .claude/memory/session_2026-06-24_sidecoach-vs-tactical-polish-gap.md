---
name: sidecoach-vs-tactical-polish-gap
description: Gap analysis - Sidecoach's references vs tactical-polish; coverage is strong (19 polish detectors operationalize 14/16), gaps = #4 interruptible + partial #13 + stale "14-point" doc label
type: project
relates_to: []
---
...
Q: gap between Sidecoach's references vs tactical-polish.

## What exists (not a gap)
- EXTRACTED COPY current: sidecoach/reference/_extracted/tactical-polish/ (SKILL.md + animations/performance/surfaces/typography.md), dated May 25-26; SOURCE skill last modified Apr 30. So the copy is NEWER than the source's last change = NOT stale, and AUGMENTED (each principle tagged "- extension"). All 16 source principles present in the extracted copy.
- OPERATIONALIZED, not just docs: Sidecoach has a polish/ validator family - 19 registered product rules (product-rule-registry) + the checks in src/validators/checks/polish-checks.ts. So tactical-polish is wired into the AUDIT gate as detectors, not only cited as build-guidance.
...
## STATUS UPDATE (2026-07-03, verified by grep of polish-checks.ts + validators.generated.ts)
All three gaps above are CLOSED: checkInterruptibleAnimations exists (gap 1, registered as polish.interruptible-animations), checkSkipLoadAnimation exists alongside checkAnimatePresenceInitial (gap 2, CSS-only mount case), and CLAUDE.md now says "16-point checklist" (gap 3). Current coverage: 21 distinct check functions = all 16 tactical-polish principles detected + genericity/state-completeness/typography-rhythm/reduced-motion/shadow-hierarchy beyond the source. 16/16 fully operationalized.
