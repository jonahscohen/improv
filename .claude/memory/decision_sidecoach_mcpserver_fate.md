...
> **REVERSAL (2026-07-24, Jonah): RETIRE after all.** The 2026-07-15 wire-up ruling was NEVER EXECUTED, and the revisit condition IT set - "a concrete external MCP client appears" - was not met in the ~2 weeks since. Given the full origin re-read (the goal was a host-callable external surface; no consumer materialized), Jonah reversed to RETIRE. This is an eyes-open reversal of his own prior ruling, not a drift. The RETIRE analysis and the "Safe removal sequence" below are now the CHOSEN path - but that sequence was authored against commit `0b65e983` and HEAD has moved FAR since (simplify-cut deletions, vocab-collapse, groundb's run-tests.ts suite add), so EVERY current-state claim in it (the tether line number in run-tests.ts, the docstring locations, the file paths) MUST be re-verified against current HEAD before executing. A drifted removal sequence is a hypothesis. NOTE: retiring the dir inherently collapses the TRIPLICATED classifier -> DUPLICATED (the dead 3rd copy `keyword-resolver.ts` lives inside the dir). modes.ts is NOT deleted by this retirement (the CLI still resolves retired-mode words via getMode per vocab-collapse's back-compat); its full deletion is a SEPARATE follow-up that must first migrate mode resolution into PHASE_ALIASES.
>
> **PRIOR RULING (2026-07-15, Jonah): WIRE IT UP, not retire.** SUPERSEDED by the 2026-07-24 reversal above. The archaeology (section 0) showed the external MCP surface was a real, sustained intent, not an accident - so Jonah chose to realize it: register the server portably and make it live, rather than delete it. Everything below from section 1 on is the RETIRE analysis, retained as the record of the alternative that was weighed and (in 2026-07-15) rejected, then (in 2026-07-24) chosen. Wire-up work was specced in docs/plans/2026-07-15-cmux-fable-scoping-and-sidecoach-wireup.md and is now abandoned.
