...
- **Round 4** (confirmation, exit 0, real Codex): **CLEAN** - no new correctness bugs. Explicitly confirmed all three round-3 folds (bigram pre-dedupe stream producing newyork/yorknew/newjersey without false adjacency; N>top_n rejection at score.py; corpus derivation matching validate.py's HERE.parent.parent) and rechecked that all prior folds remain intact (stale-set consumption, machine-stale-leak abort, index exclusion, deterministic head/dangling/cycle resolution, bidirectional expansion). Codex gate CLOSED.
