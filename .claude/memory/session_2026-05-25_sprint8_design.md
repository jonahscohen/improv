...
description: Sprint 8 (Sidecoach Oracle parity + Teach rebuild) brainstorming + design spec. Surfaced by dogfood failure 2026-05-24 - sidecoach claimed full oracle replacement but teach was a stub and 20+ oracle verb commands had no slash routes.
type: project
...
2. Sidecoach has 15 slash commands using a PHASE vocabulary (research, craft, implement, review, etc), not oracle's 23 verb vocabulary (animate, colorize, polish, audit, document, etc).
3. Prior session memories (`session_2026-05-23_sidecoach_100_complete.md`, etc) claimed sidecoach fully replaced oracle. That claim was inaccurate.
...
- Fix all the gaps; make sidecoach BETTER than oracle
...
- **Vocabulary strategy:** ADD oracle's 22 verb commands alongside sidecoach's existing 15 phase commands. Both vocabularies remain valid. No breaking changes.
- **Teach mode:** hybrid - parse brief first, ask follow-up questions only for fields the brief did not answer with confidence.
- **Acceptance bar:** parity-plus per command. Each must include oracle's checklist items PLUS at least one sidecoach-specific addition (validator, BuildReport hook, memory tracking).
...
`/Users/spare3/Documents/Github/claude-dotfiles/docs/superpowers/specs/2026-05-25-sidecoach-sprint8-oracle-parity-design.md`
...
- High: oracle source path locally available so parityChecklist can be derived from real skill files.
- Medium: `live` verb depends on Improv MCP - may need to ship as stub-with-error if integration is incomplete.
- Medium: 22 entries × parity-plus = many literal strings to maintain. Mitigation: registry entries reference `oracleSkillPath` for future drift-audit.
...
Implementation plan saved to `/Users/spare3/Documents/Github/claude-dotfiles/docs/superpowers/plans/2026-05-25-sprint8-oracle-parity.md`.
