...
- **Global stale rule:** a query is a MISS if any stale beat ranks strictly above the satisfaction rank. Stale beats are (a) the machine-derived set - every corpus beat with `superseded_by` frontmatter, emitted by `validate.py --stale-set`, which scorers MUST consume rather than re-derive - and (b) the case's `stale` entries, which trap UNMARKED stale beats (q21's endow decision beat claims "final name is endow" with no supersession marker; do not "fix" that frontmatter, the trap is the point).
